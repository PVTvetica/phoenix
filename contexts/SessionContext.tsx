...
export const SessionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // never resolved setupCompleted (server still warming / dev API not started),
    // retry a few times then lift the splash so LoginView is reachable.
    useEffect(() => {
        ...
        const scheduleRetry = () => {
            if (cancelled || bootResolvedRef.current) return;
            ...
            const t = setTimeout(() => {
                if (cancelled || bootResolvedRef.current) return;
                bootRetriesRef.current += 1;
                void refreshUser().finally(() => {
                    if (!cancelled && !bootResolvedRef.current) scheduleRetry();
                });
            }, 1500);
            return t;
        };

        const firstTimer = scheduleRetry();
        return () => {
            cancelled = true;
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`auth-alerts`, { config: { private: true } })
            ...
            .on('broadcast', { event: 'system_broadcast' }, (payload: any) => {
                ...
                if (msg) {
                    playSound(brandingConfig.newRequestSoundUrl);
                    addToast("Systemübertragung", <i className="fa-solid fa-bullhorn"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: msg, silent: true });
                }
            })
    ...
    const login = useCallback(() => {
        ...
        if (!clientId) {
            addToast("Discord Nicht konfiguriert", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Discord OAuth has not been set up. Contact your administrator." });
            return;
        }
