...
const RadioSettingsTab: React.FC = () => {
    ...
    const handleUpdateConfig = async () => {
        ...
        try {
            ...
            console.error(err);
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Funkkonfiguration konnte nicht gespeichert werden." });
        } finally {
    ...
    const handleUpdateChannel = async (id: string, name: string, color: string) => {
        ...
        try {
            ...
            console.error(err);
            addToast("Aktualisierung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Funkkanal konnte nicht aktualisiert werden." });
        }
    };
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="amber"
                subtitle="Konfiguriere Sprachfunkkanäle und LiveKit-Raumzuordnungen."
            />
            ...
            <div className="bg-slate-900/50 rounded-lg p-6 border border-slate-700/50">
                <h2 className="text-xl text-white flex items-center mb-4">
                    <i className="fa-solid fa-walkie-talkie h-6 w-6 mr-3 text-slate-400"></i>Konfiguration des Funksystems</h2>
                <p className="text-sm text-slate-400 mb-6">Manage default boot frequencies and global comms settings.</p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label htmlFor="channelName" className="block text-sm font-medium text-slate-300 mb-2">Standard-Startfrequenz-ID</label>
                        <select
                            id="channelName"
                        ...
                        <p className="text-xs text-slate-500 mt-1">Neue Nutzer stellen sich beim Einschalten auf diese Frequenz ein.</p>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2">Push-to-Talk-Taste</label>
                        <div className="w-full bg-slate-700/50 border border-slate-600 rounded-md p-2.5 text-slate-400 font-mono text-sm cursor-not-allowed">Richtige Kontrolle</div>
                    </div>
                </div>
            ...
            <div className={`bg-slate-900/50 rounded-lg p-6 border border-slate-700/50`}>
                <h2 className="text-xl text-white flex items-center mb-4">
                    <i className="fa-solid fa-tower-broadcast h-6 w-6 mr-3 text-slate-400"></i>Frequenzmatrix</h2>
                <p className="text-sm text-slate-400 mb-6">Taktische Preset-Frequenzen verwalten. Zeilen zum Sortieren ziehen.</p>
                ...
                <form onSubmit={handleAddChannel} className="bg-slate-800/40 border border-slate-700 p-4 rounded-lg mb-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label className="block text-[10px] text-slate-500 uppercase font-black mb-1">Neue Kanal-ID</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="block text-[10px] text-slate-500 uppercase font-black mb-1">Anzeigename</label>
                        <input
                            type="text"
                    ...
                    <div className="flex space-x-2">
                        <div className="flex-1">
                            <label className="block text-[10px] text-slate-500 uppercase font-black mb-1">Farbe</label>
                ...
                <div className="space-y-3">
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex flex-col relative pl-6">
                                    ...
                                    <label className="block text-[10px] text-slate-500 uppercase font-bold mb-1">ID / Schnecke</label>
                                    <code className="bg-black/30 px-2 py-1 rounded-sm text-slate-300 text-xs w-fit">{channel.id}</code>
                                </div>
                                <div>
                                    <label className="block text-[10px] text-slate-500 uppercase font-bold mb-1">Anzeigename</label>
                                    <input
                                        type="text"
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="w-32">
                                        <label className="block text-[10px] text-slate-500 uppercase font-bold mb-1">Farbe</label>
                                        <input
                                            type="color"
                                    ...
                                    <div className="text-right flex flex-col items-end">
                                        <label className="block text-[10px] text-slate-500 uppercase font-bold mb-1">Typ</label>
                    ...
                    {localChannels.length === 0 && (
                        <div className="text-center py-12 border-2 border-dashed border-slate-800 rounded-lg">
                            ...
                            <p className="text-slate-500 italic text-sm">Keine Funkfrequenzen konfiguriert.</p>
                        </div>
                    )}
                </div>
            </div>
