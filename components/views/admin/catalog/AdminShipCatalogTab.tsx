            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-3">
                        <i className="fa-solid fa-rocket text-purple-400"></i>Schiffskatalog<span className="text-sm font-bold text-slate-500 bg-slate-800 px-2 py-0.5 rounded-sm">{ships.length}</span>
                    </h2>
            ...
            <div className="bg-slate-900 border border-white/10 rounded-xl p-4 mb-6">
                <div className="flex flex-col lg:flex-row gap-3 items-end">
                    ...
                    {(search || filterManufacturer || filterSize || filterStatus) && (
                        <button onClick={clearFilters} className="text-xs text-slate-400 hover:text-white underline whitespace-nowrap">Klar</button>
                    )}
                </div>
            ...
            <div className="bg-slate-900 border border-white/10 rounded-xl overflow-hidden mb-6">
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-black/30 text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-white/5">
                            <tr>
                                ...
                                <th className="p-3 cursor-pointer hover:text-white" onClick={() => handleSort('manufacturer')}>Hersteller<SortIcon field="manufacturer" />
                                </th>
                                <th className="p-3">Größe</th>
                                <th className="p-3">Rolle</th>
                                <th className="p-3">Status</th>
                                <th className="p-3 cursor-pointer hover:text-white text-right" onClick={() => handleSort('msrp')}>UVP<SortIcon field="msrp" />
                                </th>
                                <th className="p-3 cursor-pointer hover:text-white text-center" onClick={() => handleSort('usageCount')}>Verwendung<SortIcon field="usageCount" />
                                </th>
                                ...
                                <tr key={ship.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-3 text-right">
                                        <div className="flex justify-end gap-1">
                                            <button onClick={() => openEdit(ship)} className="p-1.5 text-purple-400 hover:bg-purple-500/10 rounded-sm transition-colors" title="Bearbeiten">
                                            ...
                                            <button onClick={() => handleDelete(ship)} className="p-1.5 text-red-400 hover:bg-red-500/10 rounded-sm transition-colors" title="Löschen">
                    ...
                    {pageShips.map(ship => (
                        <div key={ship.id} className="p-4 space-y-3">
                            ...
                            <div className="flex justify-end gap-2">
                                <button onClick={() => openEdit(ship)} className="text-xs text-purple-400 font-bold uppercase">Bearbeiten</button>
                                {ship.usageCount > 0 && (
                                    <button onClick={() => { setMergeSource(ship); setMergeSearch(''); }} className="text-xs text-amber-400 font-bold uppercase">Verschmelzen</button>
                                )}
                                <button onClick={() => handleDelete(ship)} className="text-xs text-red-400 font-bold uppercase">Löschen</button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {filtered.length === 0 && (
                    <div className="p-12 text-center text-slate-600">
                        ...
                        <p className="text-sm font-bold">Keine Schiffe gefunden</p>
            ...
            {totalPages > 1 && (
                <div className="flex justify-between items-center text-sm text-slate-400 mb-8">
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            <i className="fa-solid fa-chevron-left mr-1"></i>Vorher</button>
                        ...
                        <button
                            ...
                            className="px-3 py-1.5 bg-slate-800 border border-white/10 rounded-sm text-xs font-bold disabled:opacity-30 hover:bg-slate-700 transition-colors"
                        >Nächste<i className="fa-solid fa-chevron-right ml-1"></i>
                        </button>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-150 bg-black/90 backdrop-blur-sm flex items-start justify-center animate-fade-in overflow-y-auto p-4">
                    <div className="bg-slate-900 border border-purple-500/30 shadow-2xl shadow-purple-900/20 rounded-2xl max-w-3xl w-full my-8 relative">
                        ...
                        <div className="p-6 border-b border-white/10 flex justify-between items-center">
                            <div>
                                <h3 className="text-lg font-bold text-white">Schiff bearbeiten</h3>
                        ...
                        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-purple-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <i className="fa-solid fa-tag"></i>Grundlegende Informationen</h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        <label className={labelClass}>Herstellercode</label>
                                    ...
                                    <div>
                                        <label className={labelClass}>Karriere</label>
                                    ...
                                    <div>
                                        <label className={labelClass}>Produktionsstatus</label>
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-sky-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <i className="fa-solid fa-gauge-high"></i>Spezifikationen</h4>
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-green-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                                    <i className="fa-solid fa-link"></i>Medien & Links</h4>
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        ...
                                        <div>
                                            <label className={labelClass}>Promise URL</label>
                        ...
                        <div className="p-4 border-t border-white/10 flex justify-end gap-3">
                            <button onClick={() => setEditingShip(null)} className="px-4 py-2 text-xs font-bold uppercase text-slate-400 hover:text-white transition-colors">Abbrechen</button>
                            <button
                                onClick={handleSave}
                                disabled={isSaving}
                    ...
                    <div className="bg-slate-900 border border-amber-500/30 shadow-2xl shadow-amber-900/20 rounded-2xl max-w-lg w-full relative">
                        ...
                        <div className="p-6 border-b border-white/10">
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                <i className="fa-solid fa-code-merge text-amber-400"></i>Schiff zusammenführen</h3>
                            <p className="text-xs text-slate-500 mt-1">Alles neu zuweisen<strong className="text-amber-400">{mergeSource.usageCount}</strong> user reference(s) from "<strong className="text-white">{mergeSource.name}</strong>" to another ship, then delete it.
                            </p>
                        </div>

                        <div className="p-6">
                            <label className={labelClass}>Wähle das zu behaltende Zielschiff aus</label>
                            <input
                                type="text"
                                value={mergeSearch}
                        ...
                        <div className="p-4 border-t border-white/10 flex justify-end">
                            <button onClick={() => { setMergeSource(null); setMergeSearch(''); }} className="px-4 py-2 text-xs font-bold uppercase text-slate-400 hover:text-white transition-colors">Abbrechen</button>
                        </div>
                    </div>
                </div>
            )}
