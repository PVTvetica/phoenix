...
const FLAG_FIELDS: Array<{ key: keyof WarehousePlatformCommodityWithUsage; label: string }> = [
    ...
    { key: 'isHarvestable', label: 'Erntebar' },
        ...
        <div className="animate-fade-in-up p-4 md:p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-3">
                        <i className="fa-solid fa-flask text-purple-400"></i>Warenkatalog<span className="text-sm font-bold text-slate-500 bg-slate-800 px-2 py-0.5 rounded-sm">{totalCount}</span>
                    </h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 gap-3 mb-6">
                <div className="bg-slate-900 border border-white/10 rounded-xl p-4 text-center">
                    ...
                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Passende Rohstoffe</p>
                </div>
                <div className="bg-slate-900 border border-white/10 rounded-xl p-4 text-center">
                    <i className="fa-solid fa-tags text-sky-400 text-lg mb-1"></i>
                    <p className="text-xl font-black text-white">{categories.length}</p>
                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider">Kategorien</p>
                </div>
            </div>
            ...
            <div className="bg-slate-900 border border-white/10 rounded-xl p-4 mb-6">
                <div className="flex flex-col lg:flex-row gap-3 items-end">
                    ...
                    <select value={filterIllegal} onChange={(e) => setFilterIllegal(e.target.value as any)} className={selectClass}>
                        <option value="all">Legal und illegal</option>
                        <option value="legal">Nur legal</option>
                        <option value="illegal">Nur illegal</option>
                    </select>
                </div>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-black/30 text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-white/5">
                            <tr>
                                ...
                                <th className="p-3 text-right">Verkaufen</th>
                                <th className="p-3">Flaggen</th>
                                    ...
                                    <tr key={c.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="p-3 text-right">
                                            <div className="flex justify-end gap-1">
                                                <button onClick={() => openEdit(c)} className="p-1.5 text-purple-400 hover:bg-purple-500/10 rounded-sm transition-colors" title="Bearbeiten">
                                                    <i className="fa-solid fa-pen-to-square"></i>
                                                </button>
                                                <button onClick={() => handleDelete(c)} className="p-1.5 text-red-400 hover:bg-red-500/10 rounded-sm transition-colors" title="Löschen">
                ...
                <div className="fixed inset-0 z-150 bg-black/90 backdrop-blur-sm flex items-start justify-center animate-fade-in overflow-y-auto p-4">
                    <div className="bg-slate-900 border border-purple-500/30 shadow-2xl shadow-purple-900/20 rounded-2xl max-w-3xl w-full my-8 relative">
                        ...
                        <div className="p-6 border-b border-white/10 flex justify-between items-center">
                            <div>
                                <h3 className="text-lg font-bold text-white">Ware bearbeiten</h3>
                        ...
                        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            {/* Numerics */}
                            <div>
                                <h4 className="text-xs font-bold text-sky-400 uppercase tracking-wider mb-3">Preise und Gewicht</h4>
                                <div className="grid grid-cols-3 gap-3">
                                    <div>
                                        <label className={labelClass}>Gewicht (SCU)</label>
                                    ...
                                    <div>
                                        <label className={labelClass}>Kaufen (Durchschnitt/SCU)</label>
                                    ...
                                    <div>
                                        <label className={labelClass}>Verkaufen (Durchschnitt/SCU)</label>
                            ...
                            {/* Flags */}
                            <div>
                                <h4 className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-3">Klassifizierungsflaggen</h4>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm">
                    <thead className="text-[10px] font-bold uppercase tracking-wider text-slate-500 border-b border-white/5">
                        <tr>
                            <th className="text-left py-2 px-2">UEX Quelle</th>
                            <th className="text-left py-2 px-2">Anzeigename</th>
                            <th className="text-left py-2 px-2 w-24">Sortieren</th>
                            <th className="text-left py-2 px-2 w-24">Versteckt</th>
                            <th className="text-right py-2 px-2 w-28">Aktionen</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                        {categories.map(cat => {
                            ...
                            return (
                                <tr key={cat.id}>
                                    ...
                                    <td className="py-2 px-2 text-right">
                                        ...
                                        >Speichern</button>
                                        <button onClick={() => handleDelete(cat)} className="text-[10px] font-bold uppercase text-red-400 hover:text-red-300">Löschen</button>
                                    </td>
                                </tr>
                            );
                        })}
                        {categories.length === 0 && (
                            <tr><td colSpan={5} className="py-6 text-center text-slate-600 text-xs">Noch keine Kategorien. Führen du zuerst die Synchronisierung aus.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
