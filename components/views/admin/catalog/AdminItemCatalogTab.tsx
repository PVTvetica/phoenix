        ...
        <div className="animate-fade-in-up p-4 md:p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-3">
                        <i className="fa-solid fa-box-open text-purple-400"></i>Artikelkatalog<span className="text-sm font-bold text-slate-500 bg-slate-800 px-2 py-0.5 rounded-sm">{totalCount}</span>
                    </h2>
                    <p className="text-sm text-slate-500 mt-1">Plattformweite Gegenstandsdatenbank, synchronisiert von uexcorp.space.</p>
                </div>
            ...
            <div className="bg-slate-900 border border-white/10 rounded-xl p-4 mb-6">
                <div className="flex flex-col lg:flex-row gap-3 items-end">
                    ...
                    <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer whitespace-nowrap">
                        <input type="checkbox" checked={hideVehicles} onChange={(e) => setHideVehicles(e.target.checked)} className="accent-purple-500" />Fahrzeuggegenstände ausblenden</label>
                </div>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-black/30 text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-white/5">
                            <tr>
                                ...
                                <th className="p-3">Unterkategorie</th>
                                <th className="p-3">Unternehmen</th>
                                <th className="p-3">Größe</th>
                                <th className="p-3 text-center">Flaggen</th>
                                <th className="p-3 text-center">Verwendung</th>
                                <th className="p-3 text-right">Aktionen</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {loading && (
                                <tr><td colSpan={8} className="px-3 py-6 text-center text-xs text-slate-500"><i className="fa-solid fa-spinner animate-spin mr-2" />Laden…</td></tr>
                            )}
                                    ...
                                    <tr key={item.id} className="hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="p-3 text-right">
                                            <div className="flex justify-end gap-1">
                                                <button onClick={() => openEdit(item)} className="p-1.5 text-purple-400 hover:bg-purple-500/10 rounded-sm" title="Bearbeiten">
                                                    <i className="fa-solid fa-pen-to-square"></i>
                                                </button>
                                                <button onClick={() => handleDelete(item)} className="p-1.5 text-red-400 hover:bg-red-500/10 rounded-sm" title="Löschen">
            ...
            <div className="flex justify-between items-center text-sm text-slate-400 mb-8">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => setPage(p => p + 1)} disabled={page >= totalPages - 1} className="px-3 py-1.5 bg-slate-800 border border-white/10 rounded-sm text-xs font-bold disabled:opacity-30 hover:bg-slate-700">Nächste<i className="fa-solid fa-chevron-right ml-1"></i>
                    </button>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-150 bg-black/90 backdrop-blur-sm flex items-start justify-center animate-fade-in overflow-y-auto p-4">
                    <div className="bg-slate-900 border border-purple-500/30 shadow-2xl shadow-purple-900/20 rounded-2xl max-w-3xl w-full my-8 relative">
                        ...
                        <div className="p-6 border-b border-white/10 flex justify-between items-center">
                            <div>
                                <h3 className="text-lg font-bold text-white">Element bearbeiten</h3>
                        ...
                        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-purple-400 uppercase tracking-wider mb-3">Grundlegende Informationen</h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        <label className={labelClass}>Fahrzeugname (falls beigefügt)</label>
                                    ...
                                    <div>
                                        <label className={labelClass}>Qualität</label>
                                    ...
                                    <div>
                                        <label className={labelClass}>Spielversion</label>
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-3">Flaggen</h4>
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                                    <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                                        <input type="checkbox" checked={!!editForm.isVehicleItem} onChange={(e) => setEditForm({ ...editForm, isVehicleItem: e.target.checked })} className="accent-purple-500" />Fahrzeugartikel</label>
                                    <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                                        <input type="checkbox" checked={!!editForm.isCommodity} onChange={(e) => setEditForm({ ...editForm, isCommodity: e.target.checked })} className="accent-purple-500" />Ware</label>
                                    <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                                        <input type="checkbox" checked={!!editForm.isHarvestable} onChange={(e) => setEditForm({ ...editForm, isHarvestable: e.target.checked })} className="accent-purple-500" />Erntebar</label>
                                </div>
                            </div>

                            <div>
                                <h4 className="text-xs font-bold text-green-400 uppercase tracking-wider mb-3">Medien & Links</h4>
                                <div className="space-y-3">
                                    <div>
                                        <label className={labelClass}>Miniaturbild URL</label>
                        ...
                        <div className="p-4 border-t border-white/10 flex justify-end gap-3">
                            <button onClick={() => setEditing(null)} className="px-4 py-2 text-xs font-bold uppercase text-slate-400 hover:text-white">Abbrechen</button>
