        ...
        <div className="animate-fade-in-up p-4 md:p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <div>
                    <h2 className="text-2xl font-black text-white flex items-center gap-3">
                        <i className="fa-solid fa-globe text-purple-400"></i>Standortkatalog<span className="text-sm font-bold text-slate-500 bg-slate-800 px-2 py-0.5 rounded-sm">{counts.total}</span>
                    </h2>
                    <p className="text-sm text-slate-500 mt-1">Star-Citizen-Universum – synchronisiert von uexcorp.space.</p>
                </div>
            ...
            <div className="bg-slate-900 border border-white/10 rounded-xl p-4 mb-6 space-y-3">
                ...
                <div className="flex flex-wrap gap-4">
                    <label className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-slate-400 cursor-pointer">
                        <input type="checkbox" checked={includeInternal} onChange={(e) => setIncludeInternal(e.target.checked)} className="accent-purple-500" />Interne (Umlaufbahnen) einbeziehen</label>
                    <label className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-slate-400 cursor-pointer">
                        <input type="checkbox" checked={includeHidden} onChange={(e) => setIncludeHidden(e.target.checked)} className="accent-purple-500" />Verborgenes einbeziehen</label>
                    <label className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-slate-400 cursor-pointer">
                        <input type="checkbox" checked={includeDecommissioned} onChange={(e) => setIncludeDecommissioned(e.target.checked)} className="accent-purple-500" />Stillgelegte einbeziehen</label>
                </div>
            </div>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-black/30 text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-white/5">
                            <tr>
                                ...
                                <th className="p-3">Weg</th>
                                <th className="p-3">Art</th>
                                <th className="p-3">Fraktion</th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {loading && (
                                <tr><td colSpan={6} className="px-3 py-6 text-center text-xs text-slate-500"><i className="fa-solid fa-spinner animate-spin mr-2" />Laden…</td></tr>
                            )}
                                ...
                                <tr key={loc.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-3 text-right">
                                        <div className="flex justify-end gap-1">
                                            <button onClick={() => openEdit(loc)} className="p-1.5 text-purple-400 hover:bg-purple-500/10 rounded-sm" title="Bearbeiten">
                                                <i className="fa-solid fa-pen-to-square"></i>
                                            </button>
                                            <button onClick={() => handleDelete(loc)} className="p-1.5 text-red-400 hover:bg-red-500/10 rounded-sm" title="Löschen">
            ...
            <div className="flex justify-between items-center text-sm text-slate-400 mb-8">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => setPage(p => p + 1)} disabled={rows.length < PAGE_SIZE} className="px-3 py-1.5 bg-slate-800 border border-white/10 rounded-sm text-xs font-bold disabled:opacity-30 hover:bg-slate-700">Nächste<i className="fa-solid fa-chevron-right ml-1"></i>
                    </button>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-150 bg-black/90 backdrop-blur-sm flex items-start justify-center animate-fade-in overflow-y-auto p-4">
                    <div className="bg-slate-900 border border-purple-500/30 shadow-2xl shadow-purple-900/20 rounded-2xl max-w-xl w-full my-8 relative">
                        ...
                        <div className="p-6 border-b border-white/10 flex justify-between items-center">
                            <div>
                                <h3 className="text-lg font-bold text-white">Standort bearbeiten</h3>
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                <label className={labelClass}>Spitzname (Administratorüberschreibung)</label>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <label className="flex items-start gap-2 cursor-pointer">
                                    ...
                                    <span>
                                        <span className="text-sm text-white block">Versteckt</span>
                                        <span className="text-[10px] text-slate-500">Von der Suche ausgeschlossen.</span>
                                    </span>
                                </label>
                                <label className="flex items-start gap-2 cursor-pointer">
                                    ...
                                    <span>
                                        <span className="text-sm text-white block">Intern</span>
