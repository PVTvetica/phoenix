    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="amber"
                subtitle="Initialisiere diese Self-Hosted-Instanz mit einem Voll-Export der Organisation (.ndjson) aus dem gehosteten myrsi.org-Kundenportal. Nur einmal — abgelehnt, wenn die Instanz bereits Daten hat."
            />
            ...
            <div className="bg-slate-900/40 rounded-xl border border-slate-700/50 overflow-hidden">
                <div className="px-6 py-4 bg-slate-800/50 border-b border-slate-700/50 flex justify-between items-center">
                    <h3 className="text-sm font-bold text-white uppercase tracking-wider">Datei exportieren</h3>
                    ...
                    >
                        <i className="fa-solid fa-folder-open mr-1"></i>Wähle Datei</button>
                </div>
                ...
                <div className="p-6 space-y-4">
                    ...
                    {header && (
                        <>
                            <div className="rounded-lg border border-slate-700/60 bg-slate-900/60 px-3 py-2 text-xs text-slate-300 space-y-1">
                                <div><span className="text-slate-500 uppercase tracking-wider mr-2">Datei:</span>{filename}</div>
                                <div><span className="text-slate-500 uppercase tracking-wider mr-2">Quellorganisation:</span>{header.sourceOrg?.name || '—'}</div>
                                <div><span className="text-slate-500 uppercase tracking-wider mr-2">Gesamtzahl der Zeilen:</span>{totalRows.toLocaleString()}</div>
                                <div><span className="text-slate-500 uppercase tracking-wider mr-2">Tische:</span>{nonEmptyTables.length}</div>
                            </div>
                            ...
                            {users.length > 0 && (
                                <div className="rounded-lg border border-sky-500/20 bg-sky-500/10 px-3 py-2">
                                    <label className="block text-[11px] font-semibold text-sky-300 mb-1 uppercase tracking-wider">Wer davon bist du?</label>
                            ...
                            <div className="flex gap-2 pt-2">
                                ...
                                >
                                    {isImporting ? <><i className="fa-solid fa-spinner animate-spin mr-1"></i>Importieren…</> : <><i className="fa-solid fa-upload mr-1"></i>Führen du den Import aus</>}
                                </button>
                                <button onClick={clear} disabled={isImporting} className="text-[10px] bg-slate-700 hover:bg-slate-600 border border-slate-600 text-white px-4 py-2 rounded-sm font-bold uppercase transition-colors disabled:opacity-50">Klar</button>
                            </div>
                    ...
                    {result && (
                        <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-200 space-y-1">
                            ...
                            <div className="text-slate-300 pt-1">Laden du die App neu, um die importierten Daten zu laden.</div>
                        </div>
                    )}
                </div>
            </div>
