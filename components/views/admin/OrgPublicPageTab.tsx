        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-slate-900 border border-slate-700 rounded-xl max-w-3xl w-full max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="p-4 border-b border-slate-700 flex items-center justify-between">
                    <div>
                        <h3 className="text-sm font-black uppercase tracking-widest text-slate-100">Wähle Erfahrungsberichte</h3>
                    ...
                    <button onClick={onClose} className="text-slate-400 hover:text-white" aria-label="Schließen">
                ...
                <form onSubmit={handleSearch} className="p-4 border-b border-slate-800 flex gap-2">
                    <input
                        ...
                        className={inputCls}
                    />
                    <button type="submit" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white text-xs font-bold uppercase tracking-widest rounded-lg">Suchen</button>
                </form>
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {loading && <p className="text-slate-400 text-sm text-center py-6">Laden…</p>}
                    {!loading && loadError && (
                        <p className="text-red-400 text-sm text-center py-6">{loadError}</p>
                    )}
                    {!loading && !loadError && items.length === 0 && (
                        <p className="text-slate-500 text-sm text-center py-6">Noch keine bewerteten Serviceanfragen mit schriftlicher Rückmeldung.</p>
                    )}
                    {items.map((t) => {
...
const OrgPublicPageTab: React.FC = () => {
    ...
    const handleSave = async () => {
        const err = validate();
        if (err) {
            addToast("Ungültige Konfiguration der öffentlichen Seite", <i className="fa-solid fa-triangle-exclamation" />, 'bg-amber-500/10 text-amber-400 border-amber-500/50', { description: err });
            return;
        }
        setIsSaving(true);
        try {
            ...
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: e?.message || 'Failed to save public page config.' });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="sky"
                subtitle="Ersetze den Standard-Discord-Login durch eine gebrandete Landingpage für nicht angemeldete Besucher."
            />


            <SectionPanel title="Sichtweite" icon="fa-solid fa-eye">
                <Toggle
            ...
            <SectionPanel title="Inhalt" icon="fa-solid fa-pen">
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2">Klappentext<span className="text-[10px] text-slate-500 font-mono">({blurbLen}/4000)</span></label>
                ...
                <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                        <div className="md:col-span-3 space-y-2">
                            <label className="block text-sm font-medium text-slate-300">Heldenbannerbild URL</label>
                            <input
                                type="url"
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-slate-300 mb-2 text-center">Bannervorschau</label>
                            <div className="w-full aspect-4/1 bg-slate-950/60 rounded-xl border border-slate-700 flex items-center justify-center overflow-hidden shadow-inner">
                                {config.heroImageUrl ? (
                                    ...
                                    <span className="text-[10px] text-slate-500 font-mono uppercase tracking-widest">Kein Bildsatz</span>
                                )}
                            </div>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                        <div className="md:col-span-3 space-y-2">
                            <label className="block text-sm font-medium text-slate-300">Profilbild URL</label>
                            <input
                                type="url"
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-slate-300 mb-2 text-center">Profilvorschau</label>
                            <div className="w-full aspect-square max-w-[140px] mx-auto bg-slate-950/60 rounded-full border-4 border-slate-700 flex items-center justify-center overflow-hidden shadow-inner">
                                {config.profileImageUrl ? (
                                    ...
                                    <span className="text-[9px] text-slate-500 font-mono uppercase tracking-widest text-center p-2">Fällt auf das Logo der Organisation zurück</span>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <SectionPanel title="Featured Testimonials" icon="fa-solid fa-comment-dots" note="Select 3–6 quotes to show publicly. All testimonials are displayed anonymously — no client names are ever published.">
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            >
                                <i className="fa-solid fa-plus mr-1.5" />Testimonial hinzufügen</button>
                        </div>
                        {config.featuredTestimonialIds.length === 0 && (
                            <p className="text-sm text-slate-500 italic">Noch keine Erfahrungsberichte ausgewählt.</p>
                        )}
            ...
            {config.modules.links && (
                <SectionPanel title="External Links" icon="fa-solid fa-link" note="Up to 10 links. URLs must start with https:// or discord://">
                    <div className="space-y-3">
                        {config.links.length === 0 && <p className="text-sm text-slate-500 italic">Noch keine Links hinzugefügt.</p>}
                        {config.links.map((link, idx) => (
                            <div key={link.id} className="p-3 rounded-lg border border-slate-700 bg-slate-800/40 flex items-start gap-3">
                                ...
                                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-2">
                                    <input type="text" placeholder="Etikett" maxLength={40} value={link.label} onChange={(e) => updateLink(idx, { label: e.target.value })} className={inputCls} />
                                ...
                                <button type="button" onClick={() => removeLink(idx)} className="px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest rounded-sm border border-red-500/40 text-red-300 hover:bg-red-500/10 shrink-0">Entfernen</button>
                            </div>
                        ))}
                        {config.links.length < 10 && (
                            <button type="button" onClick={addLink} className="px-3 py-2 text-xs font-bold uppercase tracking-widest rounded-sm border border-sky-500/40 text-sky-300 hover:bg-sky-500/10">
                                <i className="fa-solid fa-plus mr-1.5" />Link hinzufügen</button>
                        )}
                    </div>
                </SectionPanel>
            )}
            ...
            <div className="flex justify-end pt-4">
                ...
                >
                    {isSaving ? <><i className="fa-solid fa-spinner animate-spin mr-2" />Sparen</> : isSaved ? <><i className="fa-solid fa-check mr-2" />Gespeichert</> : 'Save Public Page'}
                </button>
            </div>
