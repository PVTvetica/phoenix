...
const ClearanceManagementTab: React.FC = () => {
    ...
    const saveEdit = async () => {
        ...
        try {
            ...
            addToast("Aktualisierung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Freigabestufe konnte nicht aktualisiert werden." });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const saveMarker = async () => {
        ...
        try {
            ...
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Limitierender Marker konnte nicht gespeichert werden." });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const deleteMarker = async (id: number) => {
        ...
        try {
            ...
            console.error(error);
            addToast("Löschen fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Limitierender Marker konnte nicht gelöscht werden." });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="red"
                subtitle="Sicherheitsstufen und kompartmentierte Informationskontrollen verwalten."
                actions={canBulkAssign ? (
                    ...
                    >
                        <i className="fa-solid fa-users-gear" />Massenzuweisungsfreigabe</button>
                ) : undefined}
            />
            ...
            <div className="hidden md:grid grid-cols-3 gap-3">
                ...
                <HeroStat icon="fa-tags" label="Begrenzungsmarkierungen" value={limitingMarkers.length} accent="amber" emphasize={limitingMarkers.length > 0} />
                <HeroStat icon="fa-ban" label="Synchronisierung eingeschränkt" value={syncRestrictedCount} accent="red" emphasize={syncRestrictedCount > 0} />
            </div>
                ...
                <SectionCard
                    ...
                    actions={
                        <button onClick={() => startEditMarker()}
                            ...
                            <i className="fa-solid fa-plus mr-1"></i>Markierung hinzufügen</button>
                    }
                >
                    {/* New marker form */}
                    {editingMarkerId === -1 && (
                        <div className="p-4 bg-amber-500/4 border-b border-amber-500/15">
                            ...
                            <label className="flex items-center gap-2 cursor-pointer mb-3">
                                ...
                                <span className="text-xs text-red-300 font-bold uppercase tracking-wide">
                                    <i className="fa-solid fa-ban mr-1"></i>Von externer Synchronisierung einschränken</span>
                            </label>
                            <div className="flex justify-end gap-2">
                                <button onClick={cancelEditMarker} disabled={isLoading} className="text-xs font-bold text-slate-400 hover:text-white bg-slate-800/60 hover:bg-slate-700/60 border border-slate-700 px-3 py-1.5 rounded-sm uppercase tracking-wider transition-colors">Abbrechen</button>
                                <button onClick={saveMarker} disabled={isLoading} className="text-xs font-bold text-amber-300 hover:text-amber-200 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 px-3 py-1.5 rounded-sm uppercase tracking-wider transition-colors">Speichern</button>
                            </div>
                        </div>
                    )}
                                        ...
                                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                                            ...
                                            {m.syncRestricted && (
                                                <span className="text-[9px] bg-red-500/10 text-red-300 border border-red-500/30 px-1.5 py-0.5 rounded-sm uppercase font-black tracking-wider flex items-center gap-1">
                                                    <i className="fa-solid fa-ban"></i>Keine Synchronisierung</span>
                                            )}
                                        </div>
