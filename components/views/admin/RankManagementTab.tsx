...
const RankManagementTab: React.FC = () => {
    ...
    const handleDrop = async (e: React.DragEvent, targetRank: Rank) => {
        ...
        addToast("Neuordnung der Ränge", <i className="fa-solid fa-spinner animate-spin"></i>, 'bg-slate-500/10 text-slate-300 border-slate-500/50', { description: 'Neue Rangfolge wird gespeichert…' });
        ...
        try {
            ...
            addToast("Ränge neu geordnet", <i className="fa-solid fa-check"></i>, 'bg-green-500/10 text-green-400 border-green-500/50', { description: 'Rangfolge erfolgreich aktualisiert.' });
        } catch (err) {
            console.error("Failed to reorder ranks:", err);
            addToast("Nachbestellung fehlgeschlagen", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Neue Rangfolge konnte nicht gespeichert werden." });
        } finally {
            setIsUpdating(false);
        }
    };
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="amber"
                subtitle="Konfiguriere Hierarchie und Abzeichen. Ziehe Zeilen, um die Rangfolge zu ändern."
                ...
                actions={
                    <div className="flex gap-2 w-full md:w-auto">
                        ...
                        >
                            <i className="fa-solid fa-plus mr-2" />Rang erstellen</button>
                    </div>
                }
            />
            ...
            <div className={`bg-slate-900/40 rounded-xl border border-slate-700/50 overflow-hidden ${isUpdating ? 'opacity-60 pointer-events-none' : ''}`}>
                <div className="flex bg-slate-800/60 p-4 border-b border-slate-700/50 text-xs font-black text-slate-500 uppercase tracking-widest">
                    ...
                    <div className="w-16">Insignien</div>
                    <div className="w-24 text-center">Vorrang</div>
                    <div className="flex-1">Rangname</div>
                    <div className="w-32 hidden md:block">System-ID</div>
                    <div className="w-32 text-center">Personal</div>
                ...
                <div className="divide-y divide-slate-700/50">
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="w-32 text-right flex justify-end gap-2">
                                    <button onClick={() => openRankModal(rank)} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-700 rounded-sm transition-colors" title="Bearbeiten">
                                        <i className="fa-solid fa-pencil"></i>
                                    </button>
                                    <button onClick={() => handleDelete(rank)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-900/20 rounded-sm transition-colors" title="Löschen">
                    ...
                    {displayedRanks.length === 0 && (
                        <div className="p-12 text-center">
                            <p className="text-slate-500 font-medium italic">Keine Ränge gefunden.</p>
                        </div>
                    )}
                </div>
            </div>
            <p className="text-center text-xs text-slate-600 mt-4 italic">
                {isUpdating ? (
                    <span className="text-slate-300"><i className="fa-solid fa-circle-notch animate-spin mr-2"></i>Rangfolge neu ordnen…</span>
                ) : (
                    <>
                        <i className="fa-solid fa-lightbulb mr-1 text-amber-500/50"></i>Tipp: Ziehe Zeilen per Drag-and-Drop, um die Rangfolge zu ändern.</>
                )}
            </p>
        </div>
    );
