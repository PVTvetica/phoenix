    ...
    const handleDelete = async (unit: UnitNode) => {
        ...
        if (confirmed) {
            try {
                ...
                addToast("Einheit gelöscht", <i className="fa-solid fa-trash-can"></i>, "bg-slate-500/10 text-slate-300 border-slate-500/50", { description: `${unit.name} has been permanently deleted.` });
            } catch (err: any) {
                addToast("Löschen fehlgeschlagen", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: err?.message || "Failed to delete the unit." });
            }
        }
    };
    ...
    const handleDrop = async (e: React.DragEvent, targetUnit: UnitNode) => {
        ...
        try {
            ...
            if (dropTarget.position === 'inside') {
                ...
                addToast("Einheit verschoben", <i className="fa-solid fa-folder-tree"></i>, "bg-slate-500/10 text-slate-300 border-slate-500/50", { description: `Moved ${draggedUnit.name} into ${targetUnit.name}.` });
            }
            // Case 2: Reordering (Before/After)
                ...
                addToast("Einheit nachbestellt", <i className="fa-solid fa-sort"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: `${draggedUnit.name} has been repositioned successfully.` });
            }
        } catch (err) {
            console.error("Move failed:", err);
            addToast("Verschiebung fehlgeschlagen", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Einheit konnte in der Hierarchie nicht verschoben werden." });
        } finally {
            setIsUpdating(false);
            setDropTarget(null);
            setDraggedUnit(null);
            ...
            <React.Fragment key={unit.id}>
                ...
                >
                    ...
                    <div className="flex-1 flex items-center min-w-0" style={{ paddingLeft: `${level * (window.innerWidth < 768 ? 1 : 2)}rem` }}>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                                ...
                                {level === 0 && <span className="text-[10px] text-slate-400 font-black uppercase tracking-wider">Oberbefehlshaber</span>}
                    ...
                    <div className="w-24 md:w-32 text-right md:opacity-0 md:group-hover:opacity-100 transition-opacity flex justify-end gap-2">
                        <button onClick={() => openUnitModal(unit)} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-700 rounded-sm transition-colors" title="Bearbeiten">
                            <i className="fa-solid fa-pencil"></i>
                        </button>
                        <button onClick={() => handleDelete(unit)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-900/20 rounded-sm transition-colors" title="Löschen">
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="cyan"
                subtitle="Einheitsstruktur und Hierarchie verwalten. Per Drag-and-Drop sortieren oder umhängen."
                ...
                actions={
                    <div className="flex gap-2 w-full md:w-auto">
                        ...
                        >
                            <i className="fa-solid fa-plus mr-2" />Einheit erstellen</button>
                    </div>
                }
            />
            ...
            <div className={`bg-slate-900/40 rounded-xl border border-slate-700/50 overflow-hidden ${isUpdating ? 'opacity-60 pointer-events-none' : ''}`}>
                <div className="flex bg-slate-800/60 p-4 border-b border-slate-700/50 text-xs font-black text-slate-500 uppercase tracking-widest">
                    ...
                    <div className="flex-1">Einheitenhierarchie</div>
                    <div className="w-24 text-center hidden md:block">Befehl</div>
                    <div className="w-32 text-center hidden md:block">Personal</div>
                    <div className="w-24 md:w-32 text-right">Aktionen</div>
                </div>
                <div className="divide-y divide-slate-700/50">
                    {filteredTree.length > 0 ? (
                        ...
                        <div className="p-12 text-center">
                            <p className="text-slate-500 font-medium italic">Keine Einheiten gefunden.</p>
                        </div>
                    )}
                </div>
            </div>
            {searchTerm ? (
                <p className="text-xs text-amber-500 italic text-center mt-4">Die Neuordnung per Drag-and-Drop ist während der Suche deaktiviert.</p>
            ) : (
                <p className="text-center text-xs text-slate-600 mt-4 italic">
                    {isUpdating ? (
                        <span className="text-slate-300"><i className="fa-solid fa-circle-notch animate-spin mr-2"></i>Einheitenstruktur wird aktualisiert…</span>
                    ) : (
                        <>
