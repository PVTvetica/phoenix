...
const AdminClientDetailView: React.FC<AdminClientDetailViewProps> = ({ user, onBack, openAdjustReputationModal, openReputationHistoryModal }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.error(err);
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Änderungen am Client-Datensatz konnten nicht gespeichert werden." });
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="h-full flex flex-col overflow-hidden animate-fade-in">
            <HeroShell
                ...
                actions={<>
                    ...
                    >
                        <i className="fa-solid fa-arrow-left"></i>Kehren du zur Registrierung zurück</button>
                    ...
                    {isPromoted && (
                        <span className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-500/10 text-emerald-300 border border-emerald-500/30 text-[10px] font-black uppercase tracking-wider">
                            <i className="fa-solid fa-check"></i>Gefördert</span>
                    )}
                </>}
                statsCols={4}
                stats={<>
                    ...
                    <HeroStat icon="fa-star" label="Ruf" value={userToDisplay.reputation ?? 0} accent={userToDisplay.reputation < 25 ? 'red' : userToDisplay.reputation < 50 ? 'amber' : 'emerald'} />
                        ...
                        {activeTab === 'profile' && (
                            <SectionCard title="Account Integrity" icon="fa-id-card" accent="amber">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    ...
                                    <div>
                                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Discord Kennung</p>
                                        <p className="text-white font-mono text-sm">{userToDisplay.discordId}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Beigetreten</p>
                                        <p className="text-white font-mono text-sm">{fmt(userToDisplay.createdAt)}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Rolle</p>
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="bg-slate-950/40 border-b border-slate-800 text-slate-400 text-[10px] uppercase tracking-widest font-black">
                                            <th className="p-3 sm:p-4">Referenz</th>
                                            <th className="p-3 sm:p-4">Typ</th>
                                            <th className="p-3 sm:p-4">Datum</th>
                                            <th className="p-3 sm:p-4 text-right">Ergebnis</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-800">
                                        ...
                                        {clientStats.requests.length === 0 && (
                                            <tr><td colSpan={4} className="p-10 text-center text-slate-500 italic">Keine historischen Anfragen gefunden.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </SectionCard>
                            ...
                            <SectionCard title="Confidential Archive Notes" icon="fa-pen-to-square" accent="amber">
                                ...
                                <div className="flex justify-end">
                                    <button onClick={handleSave} disabled={isSaving || isSaved || !hasPermission('admin:user:update')}
                                        ...
                                        {isSaving ? <><i className="fa-solid fa-spinner animate-spin mr-1"></i>Sparen…</> : isSaved ? <><i className="fa-solid fa-check mr-1"></i>Gespeichert</> : <><i className="fa-solid fa-floppy-disk mr-1"></i>Commit-Notizen</>}
                                    </button>
                                </div>
                            </SectionCard>
                        )}
                    ...
                    <div className="space-y-6 lg:sticky lg:top-0 lg:self-start">
                        <SectionCard title="Reputation Profile" icon="fa-star" accent="amber">
                            <div className="text-center py-2">
                                ...
                                <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Aktive Punktzahl</div>
                            </div>
                            <div className="space-y-2 pt-2">
                                ...
                                >
                                    <i className="fa-solid fa-sliders mr-1.5"></i>Bewertung anpassen</button>
                                <button
                                ...
                                >
                                    <i className="fa-solid fa-clock-rotate-left mr-1.5"></i>Audit-Historie</button>
                            </div>
                        </SectionCard>
                    </div>
                </div>
