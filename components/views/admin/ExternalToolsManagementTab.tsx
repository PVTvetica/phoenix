    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="cyan"
                subtitle="Drittanbieter-Ressourcen und Links verwalten. Tools nach Kategorien gruppieren; innerhalb einer Kategorie mit den Pfeilen sortieren."
                actions={
                    <div className="flex gap-2 w-full md:w-auto">
                        ...
                        >
                            <i className="fa-solid fa-plus mr-2" />Werkzeug erstellen</button>
                    </div>
                }
            />
            ...
            {grouped.length === 0 ? (
                <div className="bg-slate-900/40 rounded-xl border border-slate-700/50 p-12 text-center">
                    <p className="text-slate-500 font-medium italic">Kein Werkzeug entspricht Deinen Kriterien.</p>
                </div>
            ) : grouped.map(group => (
                            ...
                            return (
                                <div key={tool.id} className="flex items-center p-4 hover:bg-slate-800/50 transition-colors group">
                                    ...
                                    <div className="w-24 text-right flex justify-end gap-2">
                                        <button onClick={() => openExternalToolModal(tool)} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-700 rounded-sm transition-colors" title="Bearbeiten">
                                            <i className="fa-solid fa-pencil"></i>
                                        </button>
                                        <button onClick={() => handleDelete(tool)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-900/20 rounded-sm transition-colors" title="Löschen">
