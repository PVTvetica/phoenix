...
const RoleDetailView: React.FC<RoleDetailViewProps> = ({ roleId, onBack }) => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            try {
                ...
                addToast("Laden fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Rollendetails konnten nicht vom Server geladen werden." });
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            addToast("Berechtigungen gespeichert", <i className="fa-solid fa-check"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: "Rollenberechtigungen erfolgreich aktualisiert." });
        } catch (error) {
            console.error("Failed to save permissions:", error);
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Speichern der Rollenberechtigungen ist ein Fehler aufgetreten." });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    if (isLoading) {
        return (
             <div className="flex flex-col items-center justify-center h-64 text-slate-500">
                ...
                <p>Sicherheitsprofile werden geladen…</p>
            </div>
        );
    }

    if (!role) {
        return <div className="p-6 text-center text-red-400">Rolle nicht gefunden.</div>;
    }
        ...
        <div className="p-6 h-full flex flex-col">
            ...
            <div className="flex justify-between items-start mb-6">
                <div>
                     <button onClick={onBack} className="flex items-center text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-white transition-colors mb-2">
                        <i className="fa-solid fa-arrow-left mr-2" />Zurück zu Levels</button>
            ...
            <div className="flex-1 bg-slate-900/40 rounded-xl border border-slate-700/50 overflow-hidden flex flex-col">
                <div className="overflow-y-auto custom-scrollbar p-6 space-y-8">
                     ...
                     {categories.length === 0 && (
                         <div className="text-center py-12 text-slate-500 italic">Keine Berechtigungen entsprechen Deinem Filter.</div>
                     )}
                </div>
            </div>
        </div>
