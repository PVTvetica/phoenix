...
const DiscordSettingsTab: React.FC = () => {
    ...
    const handleUpdateConfig = async () => {
        ...
        try {
            ...
            console.error(error);
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Discord-Konfiguration konnte nicht gespeichert werden. Details in der Konsole." });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleFetchRoles = async () => {
        ...
        try {
            ...
            addToast("Rollen abgerufen", <i className="fa-solid fa-check"></i>, "bg-emerald-500/10 text-emerald-400 border-emerald-500/50", { description: "Discord-Serverrollen erfolgreich synchronisiert." });
        } catch (error: any) {
            console.error('[DiscordSettingsTab] Fetch roles failed:', error);
            addToast("Abruf fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: error?.message || "Failed to fetch Discord roles. Check that DISCORD_BOT_TOKEN and DISCORD_GUILD_ID are set in your server's .env file." });
        } finally {
            setIsFetching(false);
        }
    };
    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="indigo"
                subtitle="Verbinde deinen Discord-Server für Bot, Kanalrouting und Rollensync."
            />
            ...
            {!discordConfig?.clientId && (
                <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-4 flex items-start gap-3">
                    ...
                    <div>
                        <p className="text-sm font-bold text-amber-400">Discord Bot nicht konfiguriert</p>
                        <p className="text-xs text-slate-400 mt-1">Discord-Integration erfordert Bot-Zugangsdaten. Setze<strong className="text-slate-300">DISCORD_CLIENT_ID</strong>, <strong className="text-slate-300">DISCORD_CLIENT_SECRET</strong>, <strong className="text-slate-300">DISCORD_BOT_TOKEN</strong>, and <strong className="text-slate-300">DISCORD_GUILD_ID</strong> in your server's <strong className="text-slate-300">.env</strong> file. Channel settings below will have no effect until the bot is set up.
                        </p>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'channels' && (
                <div className="bg-slate-900/50 rounded-lg p-6 border border-slate-700/50">
                    <h2 className="text-xl text-white flex items-center mb-4">
                        <i className="fa-brands fa-discord h-6 w-6 mr-3 text-[#5865F2]" />Discord Kanalkonfiguration</h2>

                    <div className="bg-[#5865F2]/10 border border-[#5865F2]/30 p-4 rounded-md mb-6 text-sm text-slate-300">
                        <p className="font-bold text-white mb-2"><i className="fa-solid fa-circle-info mr-2"></i>So erhalten du Kanal-IDs:</p>
                        <ol className="list-decimal list-inside space-y-1 ml-2">
                            <li>Discord öffnen<strong>Benutzereinstellungen</strong> &gt; <strong>Fortschrittlich</strong>.</li>
                            <li>Aktivieren<strong>Entwicklermodus</strong>.</li>
                            <li>Klicke mit der rechten Maustaste auf den gewünschten Textkanal auf Deinem Server.</li>
                            <li>Klicken<strong>Kanal-ID kopieren</strong> (e.g., 123456789012345678).</li>
                        </ol>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        <div>
                            <label htmlFor="newRequestChannelId" className="block text-sm font-medium text-slate-300 mb-2">Neue Anforderungskanal-ID</label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label htmlFor="intelChannelId" className="block text-sm font-medium text-slate-300 mb-2">Intel Bulletin-Kanal-ID</label>
                            <input
                                type="text"
                                id="intelChannelId"
                                name="intelChannelId"
                        ...
                        <div>
                            <label htmlFor="eamChannelId" className="block text-sm font-medium text-slate-300 mb-2">EAM Sendekanal-ID</label>
                            <input
                                type="text"
                                id="eamChannelId"
                                name="eamChannelId"
                        ...
                        <div>
                            <label htmlFor="defaultOperationAnnounceChannelId" className="block text-sm font-medium text-slate-300 mb-2">Kanal-ID für Betriebsankündigung<span className="text-slate-500 font-normal">(default)</span></label>
                            <input
                                type="text"
                ...
                <div className="space-y-6">
                    <div className="bg-slate-900/50 rounded-lg p-6 border border-slate-700/50">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 flex-wrap gap-4">
                            <div>
                                <h2 className="text-xl text-white">Discord Rollenzuordnung</h2>
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <button onClick={() => setIsSyncUsersModalOpen(true)} className="flex items-center px-4 py-2 text-sm font-semibold text-white bg-slate-700 border border-slate-600 rounded-md hover:bg-slate-600 transition-colors disabled:bg-slate-800">
                                    <i className="fa-solid fa-users-gear h-4 w-4 mr-2" />Alle Benutzer synchronisieren</button>
                            </div>
                        </div>

                        <div className="bg-amber-500/10 border border-amber-500/30 p-3 rounded-md mb-4 text-sm text-amber-300">
                            <i className="fa-solid fa-arrows-rotate mr-2"></i>
                            <strong>Bidirektionale Synchronisierung:</strong> When a member's rank or role is changed on the platform, their Discord roles will be updated automatically to match.
                        </div>
                        ...
                        {syncedDiscordRoles.length > 0 ? (
                            <>
                                ...
                                <div className="hidden sm:grid sm:grid-cols-3 gap-3 px-3 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-700/50 mb-2">
                                    <div>Discord Rolle</div>
                                    <div>Plattformrang</div>
                                ...
                                <div className="space-y-2">
                                    {syncedDiscordRoles.map(role => (
                                        <div key={role.id} className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center bg-slate-800/70 p-3 rounded-md">
                                            ...
                                            <div>
                                                <label className="sm:hidden text-xs text-slate-400 mb-1 block">Rang</label>
                                                <select
                                            ...
                                            <div>
                                                <label className="sm:hidden text-xs text-slate-400 mb-1 block">Berechtigungsstufe</label>
                                                <select
                            ...
                            <p className="text-center text-slate-500 py-8">Keine Rollen abgerufen. Klicke zum Starten auf „Serverrollen abrufen“.</p>
                        )}
                    </div>

                    <div className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/30 text-sm text-slate-400 space-y-2">
                        <p><i className="fa-solid fa-shield-halved mr-2 text-slate-500"></i><strong className="text-slate-300">Sicherheitshinweis:</strong> The Admin permission level cannot be assigned via Discord sync. Only Member, Dispatcher, and custom roles are available.</p>
                        ...
                        <p><i className="fa-solid fa-robot mr-2 text-slate-500"></i><strong className="text-slate-300">Bot-Anforderung:</strong>Für die bidirektionale Synchronisierung benötigt Dein Discord-Bot das<code className="bg-slate-700 px-1.5 py-0.5 rounded-sm text-xs">MANAGE_ROLES</code> permission and must have a role positioned higher than the roles it manages.</p>
                    </div>
                </div>
            )}
        </div>
