    ...
    return (
        <div className="p-4 md:p-8 space-y-6 animate-fade-in">
            <TabPageHeader
                ...
                accent="orange"
                subtitle="Systemweite Rundfunkmeldungen und Informationen verwalten."
                ...
                actions={
                    <div className="flex gap-2 w-full md:w-auto">
                        ...
                        >
                            <i className="fa-solid fa-plus mr-2" />Mitteilung erstellen</button>
                    </div>
                }
            />
            ...
            <div className="bg-slate-900/40 rounded-xl border border-slate-700/50 overflow-hidden">
                <div className="flex bg-slate-800/60 p-4 border-b border-slate-700/50 text-xs font-black text-slate-500 uppercase tracking-widest">
                    <div className="flex-1">Ankündigungsdetails</div>
                    <div className="w-48 hidden md:block">Publikum</div>
                    <div className="w-32 hidden sm:block text-right">Läuft ab</div>
                ...
                <div className="divide-y divide-slate-700/50">
                        ...
                        return (
                            <div key={notice.id} className="flex items-center p-4 hover:bg-slate-800/50 transition-colors group">
                                ...
                                <div className="w-24 text-right flex justify-end gap-2">
                                    <button onClick={() => openNoticeModal(notice)} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-700 rounded-sm transition-colors" title="Bearbeiten">
                                        <i className="fa-solid fa-pencil"></i>
                                    </button>
                                    <button onClick={() => handleDelete(notice)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-900/20 rounded-sm transition-colors" title="Löschen">
                    ...
                    {sortedAndFilteredItems.length === 0 && (
                        <div className="p-12 text-center">
                            <p className="text-slate-500 font-medium italic">Keine Benachrichtigungen entsprechen Deinen Kriterien.</p>
                        </div>
                    )}
                </div>
            </div>
