    ...
    return (
        <div className="p-4 md:p-8 space-y-6">
            <TabPageHeader
                ...
                accent="indigo"
                subtitle="Sichere Föderation mit verbündeten Organisationen. Pairing server-zu-server mit verschlüsselten Peer-Keys — keine Daten bis zum abgeschlossenen Handshake."
            ...
            <form onSubmit={handleSaveProfile} className="bg-slate-900/40 rounded-xl border border-slate-700/50 p-6 space-y-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center">
                    <i className="fa-solid fa-id-card mr-3 text-slate-300"></i>Deine Verzeichniskarte</h3>
                <p className="text-xs text-slate-500">This is what allied orgs see for you in their directory after pairing.</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Organisationsname</label>
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Symbol URL</label>
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Kontakt (Discord)</label>
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Klappentext</label>
                ...
                <div className="flex items-center justify-between">
                    <label className="flex items-center gap-2 text-sm text-slate-300 cursor-pointer">
                        <input type="checkbox" checked={profile.directoryVisible} onChange={e => setProfile(p => ({ ...p, directoryVisible: e.target.checked }))}
                            className="accent-slate-400 w-4 h-4" disabled={!canManage} />Tragen du uns öffentlich in alliierten Verzeichnissen ein</label>
                    <button type="submit" disabled={busy || !canManage}
                        className="bg-slate-700 hover:bg-slate-600 border border-slate-600 text-white font-bold py-2 px-4 rounded-sm text-sm transition-colors disabled:opacity-50">Profil speichern</button>
                </div>
            </form>
            ...
            <div className="bg-slate-900/40 rounded-xl border border-slate-700/50 p-6 space-y-4">
                <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center">
                    <i className="fa-solid fa-key mr-3 text-slate-300"></i>Verbinde dich mit einem Verbündeten</h3>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <button onClick={handleGenerateCode} disabled={busy || !canManage}
                        ...
                        <i className="fa-solid fa-dice mr-2"></i>Pairing-Code generieren</button>
                    {pairingCode && (
                        <div className="flex items-center gap-2 bg-slate-800/60 border border-slate-600 rounded-sm px-3 py-2">
                            ...
                            <button onClick={() => navigator.clipboard?.writeText(pairingCode.code)} title="Kopie"
                ...
                <form onSubmit={handleAddPartner} className="grid grid-cols-1 md:grid-cols-7 gap-3 items-end pt-2 border-t border-slate-700/50">
                    <div className="md:col-span-2">
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Partnername</label>
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Deine Dashboard URL</label>
                    ...
                    <div className="md:col-span-2">
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Dein Pairing-Code</label>
                    ...
                    <div className="md:col-span-7">
                        <button type="submit" disabled={busy || !canManage}
                            className="bg-slate-700 hover:bg-slate-600 border border-slate-600 text-white font-bold py-2 px-4 rounded-sm text-sm transition-colors disabled:opacity-50">Partner hinzufügen</button>
                    </div>
                </form>
            </div>
            ...
            <div className="bg-slate-900/40 rounded-xl border border-slate-700/50 overflow-hidden">
                <div className="px-6 py-4 bg-slate-800/50 border-b border-slate-700/50 flex justify-between items-center">
                    <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center">
                        <i className="fa-solid fa-people-arrows mr-3 text-slate-300"></i>Allianzpartner</h3>
                    <button onClick={load} className="text-xs text-slate-400 hover:text-white"><i className="fa-solid fa-rotate mr-1"></i>Aktualisieren</button>
                </div>
                <div className="p-6 space-y-2">
                    {loading && <p className="text-center text-slate-500 py-4">Laden…</p>}
                    {!loading && peers.length === 0 && <p className="text-center text-slate-500 py-4 italic">Noch keine Allianzpartner.</p>}
                    {peers.map(peer => (
                                ...
                                <div className="flex items-center gap-2 shrink-0">
                                    {peer.status !== 'Active' && (
                                        <button onClick={() => handleConnect(peer.id)} disabled={busy || !canManage}
                                            ...
                                            <i className="fa-solid fa-link mr-1"></i>Verbinden</button>
                                    )}
                            ...
                            {peer.status === 'Active' && (
                                <div className="mt-3 pt-3 border-t border-slate-700/50 space-y-2">
                                    <div className="flex flex-wrap items-center gap-3">
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Kanäle teilen</span>
                                    ...
                                    <div className="flex flex-wrap items-center gap-4">
                                        <label className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Ausgehend max<select value={peer.outboundMaxClearance ?? 0} disabled={busy || !canManage}
            ...
            <div className="bg-slate-900/40 rounded-xl border border-slate-700/50 overflow-hidden">
                <div className="px-6 py-4 bg-slate-800/50 border-b border-slate-700/50 flex justify-between items-center">
                    <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center">
                        <i className="fa-solid fa-satellite-dish mr-3 text-slate-300"></i>Nur-Empfangs-Feeds</h3>
                ...
                <div className="p-6 space-y-4">
                    <p className="text-xs text-slate-500">
                        ...
                        Intel from feeds (and allied peers) is pulled in <span className="text-slate-300">Intel → Wartung → Futteraufnahme</span>.
                    </p>
                    <form onSubmit={handleAddFeed} className="grid grid-cols-1 md:grid-cols-7 gap-3 items-end pt-2 border-t border-slate-700/50">
                        <div className="md:col-span-2">
                            <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Quellname</label>
                        ...
                        <div className="md:col-span-2">
                            <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Dein API-Schlüssel</label>
                        ...
                        <div>
                            <button type="submit" disabled={busy || !canManage}
                                className="w-full bg-slate-700 hover:bg-slate-600 border border-slate-600 text-white font-bold py-2 rounded-sm text-sm transition-colors disabled:opacity-50">Feed hinzufügen</button>
                        </div>
                    </form>
                    <div className="space-y-2">
                        {feeds.length === 0 && <p className="text-center text-slate-500 py-4 italic">Keine Nur-Empfangs-Feeds konfiguriert.</p>}
                        {feeds.map(feed => {
                            ...
                            return (
                                <div key={f.id} className="p-3 bg-slate-800/30 rounded-sm border border-slate-700">
                                    ...
                                    <div className="mt-3 pt-3 border-t border-slate-700/50 flex flex-wrap items-center gap-3">
                                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Synchronisierungsfilter</span>
