...
const AdminUserDetailView: React.FC<AdminUserDetailViewProps> = ({
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.error(err);
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Änderungen am Nutzerdatensatz konnten nicht gespeichert werden." });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const tabBar = (
        <div className="flex">
            {([
                ...
                { id: 'conduct', label: 'Verhaltensprotokoll', icon: 'fa-gavel' },
        ...
        <div className="h-full flex flex-col overflow-hidden animate-fade-in">
            <HeroShell
                ...
                actions={<>
                    ...
                    >
                        <i className="fa-solid fa-arrow-left"></i>Zurück zum Dienstplan</button>
                    <HeroActionButton
                        onClick={handleSave}
                ...
                statsCols={4}
                stats={<>
                    <HeroStat icon="fa-medal" label="Rang" value={userToDisplay.rank?.name || 'Unranked'} accent="sky" />
                    <HeroStat icon="fa-people-group" label="Einheit" value={userToDisplay.unit?.name || 'Unassigned'} accent="indigo" />
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar p-4 sm:p-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-w-0">
                                ...
                                <div className="relative">
                                    {isSaving && (
                                        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-xs z-10 rounded-lg flex items-center justify-center -m-2 p-2">
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <span className="text-xs font-bold text-sky-300 uppercase tracking-widest">Änderungen speichern</span>
                                            </div>
                                        </div>
                                    )}
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Systemrolle</label>
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Rang</label>
                                            <select value={rankId} onChange={e => setRankId(e.target.value)} disabled={!hasPermission('admin:user:update') || isSaving} className="w-full bg-slate-950/60 border border-slate-700 rounded-lg p-2.5 text-white focus:border-sky-500/40 focus:ring-1 focus:ring-sky-500/30 outline-hidden text-sm">
                                                <option value="">Ohne Rang</option>
                                                {sortedRanks.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Einheitenzuweisung</label>
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Hauptposition</label>
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Sekundäre Position (optional)</label>
                                    ...
                                    <div className="mt-5 bg-slate-950/40 p-4 rounded-lg border border-red-500/15">
                                        <h4 className="text-[10px] font-black text-red-300 uppercase tracking-widest mb-3 flex items-center">
                                            <i className="fa-solid fa-lock mr-2"></i>Sicherheitsprofil</h4>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Freigabeebene</label>
                                                <select
                                            ...
                                            <div>
                                                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Begrenzungsmarkierungen</label>
                                                <div className="flex flex-wrap gap-2">
                                                    ...
                                                    {limitingMarkers.length === 0 && <span className="text-xs text-slate-600 italic">Keine Markierungen definiert.</span>}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-5">
                                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Personalnotizen<span className="text-slate-600 font-normal normal-case ml-2">(Visible to member & command)</span>
                                        </label>
                                    ...
                                    <div className="mt-5">
                                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Beginndatum der Amtszeit<span className="text-slate-600 font-normal normal-case ml-2">(Override for "member since" — leave blank to use account creation date)</span>
                                        </label>
                                        <input
                                            type="date"
                                ...
                                <SectionCard
                                    ...
                                    actions={
                                        <button onClick={() => openAwardSingleCertModal(userToDisplay)}
                                            ...
                                            <i className="fa-solid fa-plus mr-1"></i>Auszeichnungsurkunde</button>
                                    }
                                >
                                    ...
                                    ) : <p className="text-slate-500 italic text-sm">Es liegen keine Zertifizierungen vor.</p>}
                                </SectionCard>
                                ...
                                <SectionCard
                                    ...
                                    actions={
                                        <button onClick={() => openAwardSingleCommendModal(userToDisplay)}
                                            ...
                                            <i className="fa-solid fa-plus mr-1"></i>Auszeichnungsmedaille</button>
                                    }
                                >
                                    ...
                                    ) : <p className="text-slate-500 italic text-sm">Keine Auszeichnungen vergeben.</p>}
                                </SectionCard>
                            </div>
                        )}
                            ...
                            <SectionCard
                                ...
                                actions={
                                    <button onClick={() => openAddConductEntryModal(userToDisplay)}
                                        ...
                                        <i className="fa-solid fa-plus mr-1"></i>Eintrag hinzufügen</button>
                                }
                            >
                                ...
                                ) : <p className="text-slate-500 italic text-sm">Es wurden keine Verhaltenseinträge erfasst.</p>}
                            </SectionCard>
                        )}
                            ...
                            <SectionCard title="HR Case History" icon="fa-folder-tree" accent="sky">
                                {linkedHRFiles.length > 0 ? (
                                    ...
                                    <div className="text-center py-12 border-2 border-dashed border-slate-800 rounded-lg">
                                        <p className="text-slate-500 font-medium italic">Mit diesem Mitglied sind keine HR-Fallakten verknüpft.</p>
                                    </div>
                                )}
                            </SectionCard>
                        )}
                    ...
                    <div className="space-y-6 lg:sticky lg:top-0 lg:self-start">
                        <SectionCard title="Ruf" icon="fa-star" accent="amber">
                            <div className="text-center py-2">
                                <div className="text-3xl font-black text-white mb-1">{userToDisplay.reputation}</div>
                                <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Aktueller Punktestand</div>
                            </div>
                            <div className="space-y-2 pt-2">
                                <button onClick={() => openAdjustReputationModal(userToDisplay)} className="w-full bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 text-xs font-bold py-2.5 rounded-lg transition-colors uppercase tracking-wider border border-amber-500/30">
                                    <i className="fa-solid fa-sliders mr-1.5"></i>Manuell anpassen</button>
                                <button onClick={() => openReputationHistoryModal(userToDisplay)} className="w-full bg-slate-800/60 hover:bg-slate-700/60 text-slate-300 hover:text-white text-xs font-bold py-2.5 rounded-lg transition-colors border border-slate-700 uppercase tracking-wider">
                                    <i className="fa-solid fa-clock-rotate-left mr-1.5"></i>Verlauf anzeigen</button>
                                <button onClick={() => openRatingHistoryModal(userToDisplay)} className="w-full bg-slate-800/60 hover:bg-slate-700/60 text-slate-300 hover:text-white text-xs font-bold py-2.5 rounded-lg transition-colors border border-slate-700 uppercase tracking-wider">
                                    <i className="fa-solid fa-comments mr-1.5"></i>Kundenbewertungen</button>
                            </div>
                        </SectionCard>
                        ...
                        <SectionCard title="Admin Notes (Private)" icon="fa-lock" accent="red">
                            ...
                            <div className="flex justify-end">
                                <button onClick={handleSave} disabled={isSaving || isSaved} className="text-[10px] font-bold text-slate-400 hover:text-white uppercase tracking-wider disabled:opacity-50">
                                    {isSaved ? <><i className="fa-solid fa-check mr-1"></i>Gespeichert</> : <><i className="fa-solid fa-floppy-disk mr-1"></i>Notiz speichern</>}
                                </button>
                            </div>
                        </SectionCard>
                    </div>
