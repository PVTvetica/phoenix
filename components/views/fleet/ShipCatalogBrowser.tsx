...
const ShipCatalogBrowser: React.FC<ShipCatalogBrowserProps> = ({ isOpen, onSelect, onClose }) => {
    ...
    return (
        <WindowFrame
            title="Schiffskatalog"
            subtitle="Schiffe durchsuchen & auswählen"
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                    {shipCatalog.length === 0 ? (
                        <EmptyState
                            icon="fa-database"
                            accent="orange"
                            heading="Schiffskatalog ist leer"
                            description="Ein Administrator muss den Schiffskatalog aus dem Star Citizen Wiki synchronisieren."
                        />
                    ) : (
                        <>
                            ...
                            {filtered.length === 0 && (
                                <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/30 mt-4">
                                    <EmptyState
                                        ...
                                        description="Setze Filter zurück oder passe die Suche an."
                                        compact
                                    />
                                </div>
                            )}
                    ...
                    <div className="text-sm text-slate-400">
                        {selectedIds.size > 0 ? (
                            <>
                                ...
                                <button onClick={() => setSelectedIds(new Set())} className="ml-3 text-xs text-slate-500 hover:text-white underline uppercase tracking-widest">Klar</button>
                            </>
                        ) : (
                            <span className="text-slate-500 text-xs uppercase tracking-widest font-mono">Klicke zum Auswählen auf „Versand“.</span>
                        )}
                    </div>
                    <div className="flex gap-3">
                        <button onClick={onClose} className="px-4 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white transition-colors">Abbrechen</button>
                        <button
