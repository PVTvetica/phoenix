...
const TABS: readonly { key: Tab; label: string; icon: string; managerOnly?: boolean }[] = [
    ...
    { key: 'locations',   label: 'Standorte',   icon: 'fa-map-location-dot', managerOnly: true },
];
...
export default function WarehouseView() {
    ...
    const loadMovements = useCallback(async () => {
        ...
        try {
            ...
            addToast("Bewegungen konnten nicht geladen werden", <i className="fa-solid fa-xmark" />,
        ...
        return (
            <div className="p-8">
                <EmptyState
                    icon="fa-lock"
                    accent="cyan"
                    heading="Du hast keinen Zugriff auf das Lager"
                    description="Bitte einen Admin, dir die Berechtigung warehouse:view zu gewähren."
                />
            </div>
        );
    }
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden bg-slate-950 text-white animate-fade-in">
            <HeroShell
                ...
                chipAccent="cyan"
                title="Org Warehouse"
                subtitle="Verfolge Massengüter — Erz, Raffinat, Treibstoff, RMC, Raketen — über deine Lager. Bestände aus einem Append-only-Ledger, damit parallele Änderungen konsistent bleiben."
                actions={canManage && !firstRun && (
                    <HeroActionButton onClick={() => setStockCreateOpen(true)} accent="cyan" icon="fa-plus">Lagerbestand hinzufügen</HeroActionButton>
                )}
                ...
                {initialLoading ? (
                    ...
                    <EmptyState
                        ...
                        action={canAdmin && (
                            ...
                            >
                                <i className="fa-solid fa-plus" />Ware hinzufügen</button>
                        )}
                    />
                ) : tab === 'stock' ? (
                    <WhStockTab
