...
const REASONS: { key: WarehouseReasonCategory; label: string; description: string }[] = [
    { key: 'sale',      label: 'Sale',                description: 'Bestand für aUEC verkaufen.' },
    { key: 'craft',     label: 'Crafting',            description: 'Material verbraucht bei Crafting / Fertigung.' },
    { key: 'transport', label: 'Transport',           description: 'Transport zwischen Lagern oder zu einem Partner.' },
    { key: 'other',     label: 'Other',               description: 'Sonstiges — erkläre es in den Notizen.' },
];
...
export default function WhWithdrawalRequestDialog({ isOpen, stock, onClose, onSubmitted }: Props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast("Antrag eingereicht",
                <i className="fa-solid fa-check" />,
                'bg-emerald-500/10 text-emerald-400 border-emerald-500/50',
                { description: 'Ein Offizier prüft und erledigt sie.' });
            onSubmitted();
            onClose();
        } catch (err: any) {
            addToast("Die Anfrage ist fehlgeschlagen",
        ...
        >
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Menge</label>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    ...
                    <button onClick={handleSubmit} disabled={submitting || !!validationError}
                        ...
                        {submitting ? <><i className="fa-solid fa-spinner fa-spin" />Einreichen…</> : <><i className="fa-solid fa-paper-plane" />Anfrage senden</>}
                    </button>
                </div>
            </div>
        </WindowFrame>
