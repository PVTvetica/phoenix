...
export default function WhCatalogEditDialog({ isOpen, target, onClose, onSubmitted }: Props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast("Speichern fehlgeschlagen",
    ...
    const handleArchive = async () => {
        ...
        try {
            ...
            addToast("Archivierung fehlgeschlagen",
                <i className="fa-solid fa-xmark" />,
                'bg-red-500/10 text-red-400 border-red-500/50',
                { description: err?.message });
        } finally {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            addToast("Ware gelöscht",
                <i className="fa-solid fa-check" />,
                'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            onSubmitted();
            onClose();
        } catch (err: any) {
            addToast("Das Löschen ist fehlgeschlagen",
                <i className="fa-solid fa-xmark" />,
                'bg-red-500/10 text-red-400 border-red-500/50',
                { description: err?.message });
        } finally {
            ...
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Kategorie</label>
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Einheit</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Qualitätssiegel (optional)</label>
                        ...
                        <p className="text-[10px] text-slate-500 mt-1 font-mono uppercase tracking-widest">Freitext: Wähle ein Streifenschema, das zu Deiner Organisation passt.</p>
                    </div>

                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Beschreibung (optional)</label>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex items-center justify-between gap-3 rounded-b-xl">
                    {existing ? (
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={handleDelete} disabled={submitting}
                                ...
                                <i className="fa-solid fa-trash mr-1.5" />Löschen</button>
                        </div>
                    ) : <span />}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={handleSubmit} disabled={submitting || !!validationError}
                            ...
                            {submitting ? <><i className="fa-solid fa-spinner fa-spin" />Sparen…</> : <><i className="fa-solid fa-check" />{existing ? 'Save' : 'Create'}</>}
                        </button>
                    </div>
                </div>
            </div>
