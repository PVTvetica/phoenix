...
export default function WhStockCreateDialog({ isOpen, onClose, onSubmitted }: Props) {
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            rpcAction('warehouse:list_locations', {})
                ...
                .catch((err: any) => {
                    addToast("Standorte konnten nicht geladen werden",
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast("Bestand erstellt",
                <i className="fa-solid fa-check" />,
                'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            onSubmitted();
            onClose();
        } catch (err: any) {
            addToast("Bestand konnte nicht erstellt werden",
                <i className="fa-solid fa-xmark" />,
                'bg-red-500/10 text-red-400 border-red-500/50',
                { description: err?.message });
        } finally {
        ...
        >
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Ware</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Standort</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Anfangsmenge (optional)</label>
                        ...
                        <p className="text-[10px] text-slate-500 mt-1 font-mono uppercase tracking-widest">Aufzeichnungen als „erste“ Bewegung.</p>
                    </div>

                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Notizen (optional)</label>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    ...
                    <button onClick={handleSubmit} disabled={submitting || !!validationError}
                        ...
                        {submitting ? <><i className="fa-solid fa-spinner fa-spin" />Sparen…</> : <><i className="fa-solid fa-plus" />Erstellen</>}
                    </button>
                </div>
            </div>
        </WindowFrame>
