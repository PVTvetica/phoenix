...
const WhCatalogImportExportModal: React.FC<WhCatalogImportExportModalProps> = ({ isOpen, onClose, onImported }) => {
    ...
    const handleStartExport = async () => {
        ...
        try {
            ...
            if (exportCancelRef.current) {
                addToast("Export abgebrochen", <i className="fa-solid fa-ban" />,
            ...
            addToast("Katalog exportiert", <i className="fa-solid fa-check" />,
                'bg-emerald-500/10 text-emerald-400 border-emerald-500/30',
                { description: `${collected.length} commodit${collected.length === 1 ? 'y' : 'ies'} written to ${defaultName}.` });
        } catch (err: any) {
            addToast("Der Export ist fehlgeschlagen", <i className="fa-solid fa-xmark" />,
    ...
    const loadPreview = useCallback(async (parsedItems: any[]) => {
        ...
        try {
            ...
            addToast("Vorschau fehlgeschlagen", <i className="fa-solid fa-xmark" />,
                'bg-red-500/10 text-red-400 border-red-500/50',
    ...
    const handleFilePick = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = () => {
            try {
                ...
                addToast("Ungültige Datei", <i className="fa-solid fa-xmark" />,
                    'bg-red-500/10 text-red-400 border-red-500/50',
    ...
    return (
        <WindowFrame
            ...
            subtitle="Nur Rohstoffe — Bestand und Bewegungen sind nicht enthalten."
                ...
                {tab === 'export' && (
                    <div className="p-5 space-y-4">
                        ...
                        <div className="bg-slate-900/40 rounded-lg p-4 border border-slate-700/30">
                            <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-2">Dateiinhalte</p>
                        ...
                        <div className="flex justify-end gap-3">
                            {exporting ? (
                                ...
                                >
                                    <i className="fa-solid fa-ban" />Abbrechen</button>
                            ) : (
                                <button
                                ...
                                >
                                    <i className="fa-solid fa-download" />Laden du JSON herunter</button>
                            )}
                        </div>
                    </div>
                )}
                ...
                {tab === 'import' && canImport && (
                    <div className="p-5 space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <i className="fa-solid fa-folder-open" />Wähle JSON-Datei</button>
                        ...
                        {loadingPreview && (
                            <div className="bg-slate-900/40 rounded-lg p-4 border border-slate-700/30 flex items-center gap-2 text-xs text-slate-400">
                                <i className="fa-solid fa-circle-notch animate-spin" />Rechenunterschiede …</div>
                        )}
                        ...
                        {preview && !loadingPreview && (
                            <div className="bg-slate-900/40 rounded-lg p-4 border border-slate-700/30 space-y-3">
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                    ...
                                    <Stat label="Aktualisieren" value={preview.updateCount} color="text-amber-400" />
                                    <Stat label="Überspringen" value={preview.skipCount} color="text-slate-500" />
                                </div>
                        ...
                        {importProgress && (
                            <div className="bg-slate-900/40 rounded-lg p-4 border border-slate-700/30 space-y-2">
                                ...
                                {importProgress.errors.length > 0 && (
                                    <details className="text-xs">
                                        <summary className="text-amber-400 cursor-pointer">Fehler anzeigen</summary>
                                        <ul className="mt-2 space-y-1 max-h-40 overflow-y-auto custom-scrollbar text-slate-400">
                        ...
                        <div className="flex justify-end gap-3">
                            {importing ? (
                                ...
                                >
                                    <i className="fa-solid fa-play" />Starten du den Import</button>
                            )}
                        </div>
                    </div>
                )}
