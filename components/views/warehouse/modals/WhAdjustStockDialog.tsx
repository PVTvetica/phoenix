...
const REASON_OPTIONS: ReasonOption[] = [
    { key: 'restock',     label: 'Restock',           description: 'Neuer Bestand eingetroffen (Mining, Raffinerie, Kauf).', serverReason: 'restock',     deltaSign: 'positive', destructive: false, notesRequired: false },
    { key: 'adjust',      label: 'Adjust (correction)', description: 'Zählfehler korrigieren. Kann nach oben oder unten gehen.',                       serverReason: 'adjust',      deltaSign: 'either',   destructive: false, notesRequired: false },
    { key: 'loss',        label: 'Loss / shrinkage',  description: 'Bestand ohne Erklärung fehlend.',                          serverReason: 'loss',        deltaSign: 'negative', destructive: true,  notesRequired: true  },
    { key: 'destruction', label: 'Destruction',       description: 'Bestand vernichtet (Kampf, Unfall, Verfall).',                 serverReason: 'destruction', deltaSign: 'negative', destructive: true,  notesRequired: true  },
];
...
export default function WhAdjustStockDialog({ isOpen, stock, onClose, onSubmitted }: Props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast("Lagerbestand angepasst",
            ...
            addToast("Die Anpassung ist fehlgeschlagen",
            ...
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                    <div className="flex items-center justify-between rounded-lg bg-slate-900/60 border border-white/5 px-4 py-3">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500">Aktuell vorhanden</span>
                    ...
                    <div className="flex items-center gap-1 bg-slate-900 rounded-lg border border-white/10 p-1">
                        ...
                        <button onClick={() => setMode('set')}
                            className={`flex-1 px-3 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest transition ${mode === 'set' ? 'bg-sky-500/20 text-sky-200' : 'text-slate-400 hover:text-slate-200'}`}>Neue Summe festlegen</button>
                    </div>

                    {mode === 'delta' ? (
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Ändern um (verwenden du - zum Verringern)</label>
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Neue Summe</label>
                    ...
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Grund</label>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    <button onClick={onClose} disabled={submitting}
                        className="px-4 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white transition-colors disabled:opacity-50">Abbrechen</button>
                    <button onClick={handleSubmit} disabled={submitting || !!validationError}
                        ...
                        {submitting ? <><i className="fa-solid fa-spinner fa-spin" />Sparen…</> : <><i className="fa-solid fa-check" />Anwenden</>}
                    </button>
                </div>
            </div>
        </WindowFrame>
