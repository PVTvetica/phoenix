...
export default function WhTransferStockDialog({ isOpen, fromStock, onClose, onSubmitted }: Props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast("Lagerbestand übertragen",
            ...
            addToast("Die Übertragung ist fehlgeschlagen",
            ...
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar flex-1">
                    {loadingCandidates ? (
                        <div className="rounded-lg border border-white/5 bg-slate-900/30 px-3 py-3 text-xs text-slate-400">
                            <i className="fa-solid fa-spinner fa-spin mr-2" />Ziele werden geladen…</div>
                    ) : candidates.length === 0 ? (
                        ...
                        <>
                            <div>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Ziel</label>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    ...
                    <button onClick={handleSubmit} disabled={submitting || loadingCandidates || !!validationError || candidates.length === 0}
                        ...
                        {submitting ? <><i className="fa-solid fa-spinner fa-spin" />Übertragen…</> : <><i className="fa-solid fa-arrow-right-arrow-left" />Überweisen</>}
                    </button>
                </div>
            </div>
        </WindowFrame>
