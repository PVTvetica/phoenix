...
const UnifiedCaseFileView: React.FC<UnifiedCaseFileViewProps> = ({ applicationId, onBack }) => {
    ...
    const caseConfig = useMemo(() => {
        ...
        if (source === 'INTERNAL_CASE') {
            return { type: 'INTERNAL', color: 'red', icon: 'fa-folder-closed', label: 'Interne Angelegenheiten' };
        }
        ...
        try {
            ...
            if (decision === 'Approve' && caseFile.referralSource === 'WEBSITE_APPLICATION' && promoteTargetId) {
                try {
                    ...
                    console.error(err);
                    addToast("Die Beförderung ist fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: err?.message || "Failed to promote the user to Member." });
                    setIsSaving(false);
            ...
            console.error(err);
            addToast("Entscheidung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Entscheidung konnte nicht verarbeitet werden." });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleResetStatus = async () => {
        ...
        try {
            ...
            console.error(e);
            addToast("Zurücksetzen fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Aktenstatus konnte nicht zurückgesetzt werden." });
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleReassignInterview = async (interviewId: string) => {
        ...
        try {
            ...
            addToast("Neuzuweisung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Interview konnte nicht neu zugewiesen werden." });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDeleteInterview = async (interviewId: string) => {
        ...
        try {
            ...
            addToast("Löschen fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Interview konnte nicht gelöscht werden." });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (!caseFile) return <div className="p-12 text-center text-slate-500 italic">Falldatei nicht gefunden oder Zugriff verweigert.</div>;
            ...
            <div className="shrink-0 relative overflow-hidden border-b border-white/5 bg-linear-to-b from-emerald-950/25 via-slate-950/80 to-slate-950 z-10">
                            ...
                            <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-2 mt-0.5">
                                ...
                                {isFetching['hr'] && (
                                    <span className="ml-2 text-emerald-300 animate-pulse text-[10px] font-bold flex items-center gap-1">
                                        <i className="fa-solid fa-arrows-rotate fa-spin"></i>Synchronisierung…</span>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex items-center gap-3">
                    {!isCompleted && (
                        ...
                        >
                            {(isSaving || vettingLoading) ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-floppy-disk"></i>Fortschritt speichern</>}
                        </button>
                    )}
            ...
            <div className="flex flex-col md:flex-row flex-1 min-h-0 overflow-hidden">
                ...
                <div className="hidden md:flex w-60 bg-slate-900/40 border-r border-slate-800/60 flex-col shrink-0">
                    <div className="p-3 space-y-0.5">
                        <p className="px-3 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2">Fallabschnitte</p>
                        {[
                            { id: 'overview', label: 'Überblick', icon: 'fa-file-contract' },
                            { id: 'background', label: 'Background', icon: 'fa-magnifying-glass' },
                            { id: 'interviews', label: 'Interviews', icon: 'fa-microphone-lines' },
                            { id: 'log', label: 'Audit-Protokoll', icon: 'fa-list-ul' },
                            { id: 'adjudication', label: 'Entscheidung', icon: 'fa-gavel' }
                        ].map(tab => (
                            <button
                                key={tab.id}
                        ...
                        {canManage && (
                            ...
                            >
                                ...
                                <span className="truncate">Admin Aktionen</span>
                            </button>
                        )}
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 bg-slate-900/50">
                    ...
                    <div className="md:hidden mb-6 flex flex-col gap-4">
                        ...
                        <div className="relative">
                            ...
                            >
                                <option value="overview">Fallübersicht</option>
                                <option value="background">Hintergrundüberprüfung</option>
                                <option value="interviews">Interviews</option>
                                <option value="log">Audit-Protokoll</option>
                                <option value="adjudication">Entscheidung</option>
                                {canManage && <option value="admin">Admin Aktionen</option>}
                            </select>
                    ...
                    {activeTab === 'overview' && (
                        <div className="max-w-4xl space-y-6">
                            ...
                            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-lg font-bold text-white mb-4 border-b border-slate-700 pb-2">Falldetails</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <p className="text-xs text-slate-500 uppercase font-bold mb-1">Sachbearbeiter</p>
                                    ...
                                    {caseConfig.type === 'TRANSFER' && transferRequest && (
                                        <div className="col-span-1 md:col-span-2 bg-amber-900/10 p-4 rounded-sm border border-amber-500/20">
                                            <div className="flex justify-between items-center text-sm">
                                                <span>Aus:<strong>{linkedMember?.unit?.name || 'Unassigned'}</strong></span>
                                                <i className="fa-solid fa-arrow-right text-amber-500"></i>
                                                <span>Zu:<strong>{(transferRequest as any).targetUnit?.name}</strong></span>
                                            </div>
                                    ...
                                    {caseConfig.type === 'VETTING' && (
                                        <div className="col-span-1 md:col-span-2 bg-indigo-900/10 p-4 rounded-sm border border-indigo-500/20">
                                            <div className="flex justify-between items-center text-sm">
                                                <span>Aktuell:<strong>{linkedMember?.clearanceLevel?.name || 'None'}</strong></span>
                                                <i className="fa-solid fa-arrow-right text-indigo-500"></i>
                                                <span>Angefordert:<strong>{requestedLevel?.name || 'Review'}</strong></span>
                                            </div>
                                        </div>
                                    )}

                                    <div className="col-span-1 md:col-span-2">
                                        <p className="text-xs text-slate-500 uppercase font-bold mb-2">Erste Stellungnahme/Bericht</p>
                            ...
                            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6 flex flex-col h-[400px]">
                                <h3 className="text-lg font-bold text-white mb-4">Ermittlungsnotizen</h3>
                                <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar mb-4">
                                    ...
                                    {notesList.length === 0 && <p className="text-slate-500 italic text-sm text-center mt-10">Keine zusätzlichen Notizen aufgezeichnet.</p>}
                                </div>
                                {!isCompleted && (
                    ...
                    {activeTab === 'background' && (
                        <div className="max-w-4xl space-y-6">
                            ...
                            {userRecord && (
                                <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                    <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
                                        <i className="fa-solid fa-id-card-clip text-emerald-300"></i>Mitgliedsstatus</h3>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="bg-slate-900/60 p-4 rounded-lg border border-slate-700 text-center">
                                            <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Reputations-Score</p>
                                            <p className={`text-2xl font-black ${userRecord.reputation >= 50 ? 'text-green-400' : 'text-amber-400'}`}>{userRecord.reputation}</p>
                                        </div>
                                        <div className="bg-slate-900/60 p-4 rounded-lg border border-slate-700 text-center">
                                            <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Kundenzufriedenheit</p>
                            ...
                            {caseConfig.type === 'VETTING' && (
                                <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                    <h3 className="text-lg font-bold text-white mb-4">Überprüfungskontrollen</h3>
                            ...
                            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-lg font-bold text-white mb-4">Überprüfung der inneren Angelegenheiten</h3>
                                {activeInvestigations.length > 0 ? (
                                    <div className="bg-red-900/20 border border-red-500/30 p-4 rounded-lg">
                                        <div className="flex items-center gap-3 text-red-400 font-bold mb-2">
                                            <i className="fa-solid fa-triangle-exclamation text-xl"></i>
                                            <span>Aktive Untersuchungen gefunden</span>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            {activeInvestigations.map(inv => (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-amber-400 font-bold uppercase text-xs">Details anzeigen<i className="fa-solid fa-arrow-right ml-1"></i></span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-green-900/10 border border-green-500/20 p-4 rounded-lg text-center">
                                        ...
                                        <p className="text-green-300 font-bold">Keine aktiven Ermittlungen</p>
                                        <p className="text-green-200/60 text-xs">Der Kandidat hat keine offenen internen Akten.</p>
                                    </div>
                                )}
                            </div>
                            ...
                            {/* Internal Conduct */}
                            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-lg font-bold text-white mb-4">Verhaltensprotokoll</h3>
                                {userRecord && userRecord.conductRecord && userRecord.conductRecord.length > 0 ? (
                                    ...
                                    <div className="p-4 bg-green-900/10 border border-green-500/20 rounded-lg text-center">
                                        <p className="text-green-400 font-bold text-sm">Saubere Aufzeichnung</p>
                                        <p className="text-green-300/70 text-xs">Es wurden keine internen Verhaltenseinträge in der Datei gefunden.</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                    <h3 className="text-lg font-bold text-white mb-4">Zertifizierungen</h3>
                                    ...
                                    ) : <p className="text-sm text-slate-500 italic">Keiner.</p>}
                                </div>
                                <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                    <h3 className="text-lg font-bold text-white mb-4">Belobigungen</h3>
                                ...
                                {dossier ? (
                                    <div className="space-y-4">
                                        {dossier.warrants.filter(w => w.status === WarrantStatus.Active || w.status === WarrantStatus.Standing).length > 0 ? (
                                            <div className="bg-red-900/20 border-l-4 border-red-500 p-4 rounded-r">
                                                <div className="flex items-center gap-2 text-red-400 font-bold mb-1">
                                                    ...
                                                    <span>Aktiver Hinweis gefunden</span>
                                                </div>
                                                <p className="text-sm text-red-200/80">Target verfügt über aktive Warnungen im System.</p>
                                            </div>
                                        ) : (
                                        ...
                                        {dossier.reports.length > 0 ? (
                                            ...
                                            <p className="text-sm text-slate-500 italic text-center">Keine Geheimdienstberichte eingereicht.</p>
                                        )}
                                    </div>
                                ) : (
                                    <p className="text-sm text-slate-500">Dossier wird geladen…</p>
                                )}
                            </div>
                        </div>
                    )}
                        ...
                        <div className="max-w-4xl space-y-6">
                            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-lg font-bold text-white">Interviewaufzeichnungen</h3>
                                    {!isCompleted && (
                                        <button
                                        ...
                                        >
                                            <i className="fa-solid fa-plus mr-2"></i>Zeitplan</button>
                                    )}
                                </div>
                                {caseFile.interviews.length > 0 ? (
                                    ...
                                    <p className="text-sm text-slate-500 italic text-center py-6">Keine Vorstellungsgespräche geplant.</p>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'log' && (
                        <div className="max-w-4xl space-y-4">
                            <h3 className="text-xl font-bold text-white mb-4">Aktivitätsprotokoll</h3>
                            {localLogs.map(log => (
                            ...
                            {localLogs.length === 0 && <p className="text-slate-500 italic text-sm">Keine Aktivität protokolliert.</p>}
                        </div>
                    )}
                    ...
                    {activeTab === 'adjudication' && (
                        <div className="max-w-4xl space-y-6">
                            ...
                            {caseConfig.type === 'VETTING' && linkedMember && (
                                <div className="bg-indigo-900/10 border border-indigo-500/20 rounded-xl p-6">
                                    <h4 className="text-sm font-bold text-indigo-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                                        <i className="fa-solid fa-shield-halved"></i>Freigabeauftrag</h4>
                                    <div className="flex items-center gap-4 mb-5 bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
                                        <div className="text-center flex-1">
                                            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Aktuell</p>
                                        ...
                                        <i className="fa-solid fa-arrow-right text-indigo-500"></i>
                                        <div className="text-center flex-1">
                                            <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Bewerben</p>
                                    ...
                                    {!isCompleted && (
                                        <>
                                            <div className="mb-4">
                                                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Freigabeebene</label>
                                            ...
                                            {limitingMarkers.length > 0 && (
                                                <div>
                                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Begrenzungsmarkierungen</label>
                            ...
                            {caseConfig.type === 'TRANSFER' && (
                                <div className="bg-amber-900/10 border border-amber-500/20 rounded-xl p-6">
                                    <h4 className="text-sm font-bold text-amber-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                                        <i className="fa-solid fa-right-left"></i>Übertragungsaktion</h4>
                                    {transferRequest ? (
                                        <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
                                            <div className="flex items-center gap-4 text-sm">
                                                <div className="text-center flex-1">
                                                    <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Aus</p>
                                    ...
                                    <p className="text-[10px] text-amber-400/60 mt-3">
                                        <i className="fa-solid fa-circle-info mr-1"></i>Durch die Genehmigung wird dieses Mitglied zur Zieleinheit verschoben.</p>
                                </div>
                            )}
                            ...
                            {caseConfig.type === 'JOB' && (
                                <div className="bg-blue-900/10 border border-blue-500/20 rounded-xl p-6">
                                    <h4 className="text-sm font-bold text-blue-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                                        <i className="fa-solid fa-briefcase"></i>Positionszuweisung</h4>
                                    <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
                                        <div className="flex items-center gap-4 text-sm">
                                            <div className="text-center flex-1">
                                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Aktuelle Position</p>
                                            ...
                                            <div className="text-center flex-1">
                                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Neue Position</p>
                            ...
                            {caseConfig.type === 'RECRUITMENT' && caseFile.referralSource === 'WEBSITE_APPLICATION' && (
                                <div className="bg-emerald-500/5 border border-emerald-500/30 rounded-xl p-6">
                                    <h4 className="text-sm font-bold text-emerald-300 uppercase tracking-wider mb-4 flex items-center gap-2">
                                        <i className="fa-solid fa-user-plus"></i>Mitgliedschaftsförderung</h4>
                                    <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
                                        <div className="flex items-center gap-4 text-sm">
                                            <div className="text-center flex-1">
                                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Aktuelle Rolle</p>
                                                <p className="text-white font-bold">{linkedMember?.role || 'Client'}</p>
                                            </div>
                                            <i className="fa-solid fa-arrow-right text-emerald-300"></i>
                                            <div className="text-center flex-1">
                                                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-wider mb-1">Neue Rolle</p>
                                                <p className="text-emerald-300 font-bold">Mitglied</p>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {caseConfig.type === 'RECRUITMENT' && !isCompleted && (
                                <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                    <h4 className="text-sm font-bold text-slate-300 uppercase tracking-wider mb-4 flex items-center gap-2">
                                        <i className="fa-solid fa-user-gear text-slate-500"></i>Neueinstellung<span className="text-[10px] font-normal normal-case text-slate-500 ml-1">(Optional)</span>
                                    </h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Einheit</label>
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Rang</label>
                                        ...
                                        {limitingMarkers.length > 0 && (
                                            <div>
                                                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Begrenzungsmarkierungen</label>
                            ...
                            {(caseConfig.type === 'RECRUITMENT' && caseFile.referralSource !== 'WEBSITE_APPLICATION' || caseConfig.type === 'JOB') && hrConfig.probationDays && hrConfig.probationDays > 0 && (
                                caseFile.linkedUserId ? (
                                    ...
                                    <div className="bg-slate-800/50 border border-slate-600/40 rounded-xl p-4 flex items-start gap-2.5">
                                        ...
                                        <div>
                                            <p className="text-xs font-bold text-slate-300">Die Bewährung kann nicht automatisch beantragt werden</p>
                            ...
                            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-lg font-bold text-white mb-4">Endgültige Entscheidung</h3>

                                <div className="mb-6">
                                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Entscheidungsbegründung / Schlussbemerkung</label>
                                    <textarea
                                        value={finalNotes}
                    ...
                    {activeTab === 'admin' && canManage && (
                        <div className="max-w-xl space-y-6">
                            ...
                            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-lg font-bold text-white mb-2">Überschreiben des Fallstatus</h3>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    >Aktualisierungsstatus</button>
                                </div>
                            </div>

                            <div className="bg-red-900/10 border border-red-500/30 rounded-xl p-6">
                                <h3 className="text-lg font-bold text-red-400 mb-2">Gefahrenzone</h3>
                                ...
                                >
                                    {isDeleting ? <><i className="fa-solid fa-spinner animate-spin mr-2"></i>Löschen…</> : 'Delete Case File'}
                                </button>
                            </div>
                        </div>
                    )}
