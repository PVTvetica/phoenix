...
const UnitFeed: React.FC<{ unitId: number }> = ({ unitId }) => {
    ...
    const handlePost = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setContent('');
            fetchFeed();
        } catch { addToast("Beitrag fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Dein Beitrag konnte nicht veröffentlicht werden." }); } finally { setIsPosting(false); }
    };
    ...
    const handleDelete = async (postId: string) => {
        ...
        try {
            ...
            fetchFeed();
        } catch { addToast("Löschen fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beitrag konnte nicht gelöscht werden." }); }
    };
    ...
    return (
        <div className="flex flex-col h-[600px] bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-700/50 overflow-hidden">
            <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex items-center gap-3 shrink-0">
                ...
                <h3 className="font-bold text-white text-sm uppercase tracking-wider">Einheitskommunikation</h3>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                {isLoading ? (
                    <div className="text-center text-slate-500 italic py-10">Feed wird geladen…</div>
                ) : posts.length === 0 ? (
                    <div className="flex items-center justify-center h-full">
                        <EmptyState icon="fa-comment-slash" accent="emerald" heading="Noch keine Beiträge" description="Sei der Erste, der postet." compact />
                    </div>
                ) : (
                    posts.map(post => {
        ...
        <div className="bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-700/50 overflow-hidden flex flex-col h-full max-h-[600px]">
            <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-bold text-white text-sm uppercase tracking-wider">Aktiver Dienstplan</h3>
                </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                {members.map(m => (
                    <div key={m.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-800/50 transition-colors group">
                        ...
                        {m.unit?.leaderId === m.id && <i className="fa-solid fa-crown text-amber-400 text-xs" title="Einheitsführer"></i>}
                    </div>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-700/50 overflow-hidden flex flex-col h-full max-h-[600px]">
            <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex items-center gap-3 shrink-0">
                ...
                <h3 className="font-bold text-white text-sm uppercase tracking-wider">Bereitstellungsplan</h3>
            </div>
            <div className="p-4 space-y-3 flex-1 overflow-y-auto custom-scrollbar">
                {upcoming.length > 0 ? upcoming.map(op => (
                    ...
                    <div className="flex items-center justify-center h-full">
                        <EmptyState icon="fa-calendar-xmark" accent="emerald" heading="Keine anstehenden Einsätze" compact />
                    </div>
                )}
            </div>
        </div>
            ...
            <div className="flex items-center justify-center py-20 animate-fade-in">
                <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/30 max-w-md w-full">
                    <EmptyState
                        icon="fa-users-slash"
                        accent="emerald"
                        heading="Keine Einheitszuweisung"
                        description="Du bist im General Pool. Kontaktiere HR oder Command für eine operative Einheit."
                    />
                </div>
            </div>
        );
            ...
            <div className="relative rounded-xl overflow-hidden border border-slate-700/50 shadow-lg bg-slate-900 group">
                ...
                <div className="relative z-10 p-6 md:p-8 flex flex-col md:flex-row items-center md:items-end gap-6 text-center md:text-left">
                    ...
                    {isLeader && (
                        ...
                        >
                            <i className="fa-solid fa-pen-to-square"></i>Profil bearbeiten</button>
                    )}
                </div>
            </div>
