...
const ProbationTab: React.FC = () => {
    ...
    const handleSaveConfig = useCallback(async () => {
        ...
        try {
            ...
            addToast("Konfiguration gespeichert", <i className="fa-solid fa-check"></i>, "bg-emerald-500/10 text-emerald-400 border-emerald-500/50");
        } catch {
    ...
    const handleConfirmProbation = useCallback(async (userId: number, userName: string) => {
        ...
        try {
            ...
            addToast("Bewährung geklärt", <i className="fa-solid fa-check"></i>, "bg-emerald-500/10 text-emerald-400 border-emerald-500/50", { description: `${userName} is now a full member.` });
        } catch {
    ...
    const handleDemote = useCallback(async (userId: number, userName: string) => {
        ...
        try {
            ...
            addToast("Mitglied herabgestuft", <i className="fa-solid fa-arrow-down"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: `${userName} has been demoted to Client.` });
        } catch {
            addToast("Fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50");
        } finally {
            setSaving(false);
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-xl font-black text-white flex items-center gap-3 uppercase tracking-tight">
                        <i className="fa-solid fa-hourglass-half text-emerald-300"></i>Bewährungs-Tracker</h2>
                    <p className="text-slate-400 text-sm mt-1">Überwachen und überprüfen du neue Mitglieder auf Bewährung.</p>
                </div>
            ...
            {canAdmin && (
                <div className="bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex items-center gap-3">
                        ...
                        <h3 className="font-bold text-white text-sm uppercase tracking-wider">Bewährungseinstellungen</h3>
                    </div>
                    <div className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        {editingConfig ? (
                            ...
                            >
                                <i className="fa-solid fa-pen"></i>Konfigurieren</button>
                        )}
                    </div>
                </div>
            )}
            ...
            {probationMembers.length === 0 ? (
                <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/30 p-10 text-center">
                    ...
                    <h3 className="text-lg font-bold text-white mb-1">Keine Mitglieder auf Bewährung</h3>
                                ...
                                <div className="p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                                    ...
                                    <div className="flex items-center gap-4 shrink-0">
                                        ...
                                        {(canRecruit || canAdmin) && (
                                            ...
                                            >
                                                <i className="fa-solid fa-clipboard-check"></i>Rezension</button>
                                        )}
                                    </div>
                                </div>
                                ...
                                {isReviewing && (
                                    <div className="border-t border-white/5 p-4 bg-slate-950/40 space-y-3">
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                                            <div>
                                                <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Startdatum</p>
                                                <p className="text-slate-300 font-mono text-xs">{member.probationStart ? fmt.date(member.probationStart) : 'N/A'}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Enddatum</p>
                                                <p className="text-slate-300 font-mono text-xs">{fmt.date(member.probationEnd!)}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Rolle</p>
                                                <p className="text-slate-300 text-xs">{member.role}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Einheit</p>
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-2 pt-2">
                                            ...
                                            >
                                                <i className="fa-solid fa-check"></i>Bestätigen – Vollmitglied</button>
                                            <button
                                            ...
                                            >
                                                <i className="fa-solid fa-arrow-down"></i>Zum Clients herabstufen</button>
                                        </div>
                                    </div>
                                )}
                            </div>
