...
const MyInterviewsTab: React.FC = () => {
    ...
    const InterviewTable: React.FC<{ interviews: (HydratedHRInterview & { applicantName?: string })[] }> = ({ interviews }) => (
        <div className="bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-700/50 overflow-hidden">
            <table className="w-full text-left hidden md:table">
                <thead>
                    <tr className="bg-white/5 border-b border-white/5 text-slate-500 text-[10px] uppercase tracking-widest font-black">
                        ...
                        <th className="p-4 font-bold">Protokoll</th>
                            ...
                            <td className="p-4 text-right">
                                <div className="flex justify-end gap-2">
                                    {int.status !== 'Completed' && (
                                        ...
                                        >
                                            <i className="fa-solid fa-pen-to-square mr-1"></i>Bearbeiten</button>
                                    )}
                                    <button
            ...
            {interviews.length === 0 && (
                 <div className="p-8 text-center text-slate-500 italic">Keine Interviews gefunden.</div>
            )}
        </div>
    );
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-xl font-black text-white flex items-center gap-3 uppercase tracking-tight">
                        <i className="fa-solid fa-clipboard-user text-emerald-300"></i>Meine Interviews</h2>
                    <p className="text-slate-400 text-sm mt-1">dir zugewiesene bevorstehende geplante Protokolle.</p>
                </div>
                <button
                ...
                >
                    <i className="fa-solid fa-calendar-plus"></i>Planen du ein Vorstellungsgespräch</button>
            </div>

            <div className="space-y-3">
                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Demnächst</h3>
                <InterviewTable interviews={upcoming as any} />
            </div>

            <div className="space-y-3">
                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1">Abgeschlossene Geschichte</h3>
