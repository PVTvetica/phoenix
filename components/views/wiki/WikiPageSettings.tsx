    ...
    return (
        <PortalCard variant="dashed" padding="md" className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Klassifizierungsstufe</label>
                    <select
                    ...
                    >
                        {securityClearances.length > 0 ? (
                            ...
                            <>
                                <option value={0}>Stufe 0 – Nicht klassifiziert</option>
                                <option value={1}>Stufe 1 – Eingeschränkt</option>
                                <option value={2}>Stufe 2 – Vertraulich</option>
                                <option value={3}>Stufe 3 – Geheimnis</option>
                                <option value={4}>Stufe 4 – Streng geheim</option>
                            </>
                        )}
                    </select>
                </div>
                ...
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Übergeordnete Seite</label>
                    <select
                        value={parentPageId || ''}
                    ...
                    >
                        <option value="">Root (kein übergeordnetes Element)</option>
                        {validParents.map((p) => (
                    ...
                    {menuStructureLocked && (
                        <p className="text-[10px] text-amber-400/80 mt-1.5">
                            <i className="fa-solid fa-lock mr-1"></i>Menüposition gesperrt. Unten entsperren zum Umhängen.</p>
                    )}
                </div>
            </div>
                ...
                <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-700/50 bg-slate-900/40 cursor-pointer hover:border-amber-500/30 transition-colors">
                    ...
                    <div className="min-w-0">
                        <span className="text-xs font-bold text-white block">
                            <i className="fa-solid fa-lock mr-1.5 text-amber-400/80"></i>Menüposition sperren</span>
            ...
            {limitingMarkers.length > 0 && (
                <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] mb-2">Begrenzungsmarkierungen</label>
