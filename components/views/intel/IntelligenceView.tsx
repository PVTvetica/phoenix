...
const IntelligenceView: React.FC = () => {
    ...
    const intelTableColumns: TableColumn<FeedItem>[] = useMemo(() => [
        ...
        { key: 'subjectType', label: 'Typ', sortable: true, width: '90px', render: (item) => <span className="text-[10px] uppercase tracking-wider text-slate-400">{s(item.data.subjectType)}</span> },
        { key: 'threatLevel', label: 'Gefahr', sortable: true, width: '80px', render: (item) => <span className={`text-[10px] font-black uppercase ${threatColor(s(item.data.threatLevel))}`}>{s(item.data.threatLevel)}</span> },
        { key: 'summary', label: 'Zusammenfassung', render: (item) => <span className="text-xs text-slate-400 truncate block">{s(item.data.summary).substring(0, 80)}{s(item.data.summary).length > 80 ? '...' : ''}</span> },
        ...
        { key: 'createdAt', label: 'Datum', sortable: true, width: '90px', render: (item) => <span className="text-[10px] text-slate-500 font-mono">{s(item.data.createdAt).substring(0, 10)}</span> },
    ], []);
        ...
        <div id="intel-hub-scroller" className="flex flex-col h-full overflow-y-auto overflow-x-hidden bg-slate-950">
            ...
            <div className="w-full px-4 sm:px-6 lg:px-8 space-y-8 pb-12 grow mt-6">
                ...
                <div className="flex flex-col lg:flex-row gap-3">
                    ...
                    {searchTerm.trim() && (
                        <button onClick={() => setSearchTerm('')} className="bg-slate-900 px-4 py-2.5 rounded-lg text-[10px] font-black uppercase text-slate-400 hover:text-white transition-colors border border-slate-700 tracking-wider shrink-0 w-fit">Suche löschen</button>
                    )}
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mr-1">Gefahr</span>
                    ...
                    {viewMode === 'table' && selectedIds.size > 0 && hasPermission('intel:manage') && (
                        <div className="flex items-center gap-3 p-3 bg-sky-500/10 border border-sky-500/20 rounded-lg animate-fade-in">
                            ...
                            <button onClick={handleBulkDelete} className="text-[10px] font-bold uppercase tracking-wider text-red-400 hover:text-red-300 px-3 py-1.5 rounded-sm bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 transition-colors">
                                <i className="fa-solid fa-trash mr-1.5"></i>Löschen</button>
                            <button onClick={handleBulkUpdateAffiliation} className="text-[10px] font-bold uppercase tracking-wider text-amber-400 hover:text-amber-300 px-3 py-1.5 rounded-sm bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/20 transition-colors">
                                <i className="fa-solid fa-building mr-1.5"></i>Zugehörigkeit festlegen</button>
                            <button onClick={handleBulkAddTags} className="text-[10px] font-bold uppercase tracking-wider text-emerald-400 hover:text-emerald-300 px-3 py-1.5 rounded-sm bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 transition-colors">
                                <i className="fa-solid fa-tags mr-1.5"></i>Tags hinzufügen</button>
                            <button onClick={() => setSelectedIds(new Set())} className="ml-auto text-[10px] text-slate-500 hover:text-slate-300 transition-colors">Auswahl löschen</button>
                        </div>
                    )}
                            ...
                            <>
                                ...
                                {(isLoadingMore || hasMore) && (
                                    <div className="flex items-center justify-center py-6 text-slate-500 font-mono text-[10px] uppercase tracking-[0.3em]">
                                        {isLoadingMore ? (
                                            <><i className="fa-solid fa-spinner animate-spin mr-2" aria-hidden />Lade mehr</>
                                        ) : (
                                            <span className="opacity-60">Scrollen du für weitere Datensätze</span>
                                        )}
                                    </div>
                                )}
                            </>
