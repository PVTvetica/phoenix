...
const LegislationDetailView: React.FC<LegislationDetailViewProps> = ({ legislation, onBack }) => {
    ...
    const handleAdvanceStatus = async (action: string) => {
        ...
        try {
            ...
            addToast("Status aktualisiert", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            await refreshGovernment();
        } catch (err: any) {
            addToast("Aktualisierung fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Operation failed.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleVote = async (vote: 'for' | 'against' | 'abstain') => {
        ...
        try {
            ...
            addToast("Stimmabgabe", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50', { description: `You voted "${vote}" on this bill.` });
            await refreshGovernment();
        } catch (err: any) {
            addToast("Abstimmung fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to cast vote.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleVeto = async () => {
        ...
        try {
            ...
            addToast("Veto gegen Gesetzgebung eingelegt", <i className="fa-solid fa-ban" />, 'bg-amber-500/10 text-amber-400 border-amber-500/50', { description: 'Gesetzgebung wurde vetoed.' });
            await refreshGovernment();
        } catch (err: any) {
            addToast("Veto gescheitert", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to veto.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleAddComment = async () => {
        ...
        try {
            ...
            addToast("Kommentar fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to add comment.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <button onClick={onBack} className="flex items-center gap-1 text-xs text-slate-400 hover:text-white transition-colors">
                <i className="fa-solid fa-arrow-left"></i>Zurück zur Gesetzgebung</button>
                ...
                <div className="flex items-start justify-between mb-2">
                    <div>
                        ...
                        {legislation.isConstitutionalAmendment && (
                            <span className="text-[10px] px-2 py-0.5 rounded-sm bg-amber-500/20 text-amber-400 font-bold">Verfassungsänderung</span>
                        )}
                    </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                    {canManage && legislation.status === 'Draft' && (
                        <button onClick={() => handleAdvanceStatus('gov:propose_legislation')} disabled={isLoading} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-md hover:bg-amber-500/20 transition-colors disabled:opacity-40">
                            <i className="fa-solid fa-paper-plane"></i>Vorschlagen</button>
                    )}
                    {canManage && legislation.status === 'Proposed' && (
                        <button onClick={() => handleAdvanceStatus('gov:start_legislation_debate')} disabled={isLoading} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-purple-400 bg-purple-500/10 border border-purple-500/20 rounded-md hover:bg-purple-500/20 transition-colors disabled:opacity-40">
                            <i className="fa-solid fa-comments"></i>Offene Debatte</button>
                    )}
                    {canManage && ['Proposed', 'Debate'].includes(legislation.status) && (
                        <button onClick={() => handleAdvanceStatus('gov:start_legislation_vote')} disabled={isLoading} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-md hover:bg-emerald-500/20 transition-colors disabled:opacity-40">
                            <i className="fa-solid fa-check-to-slot"></i>Abstimmung starten</button>
                    )}
                    {canManage && legislation.status === 'Voting' && (
                        <button onClick={() => handleAdvanceStatus('gov:conclude_legislation_vote')} disabled={isLoading} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-sky-400 bg-sky-500/10 border border-sky-500/20 rounded-md hover:bg-sky-500/20 transition-colors disabled:opacity-40">
                            <i className="fa-solid fa-gavel"></i>Abstimmung abschließen</button>
                    )}
            ...
            {(isVoting || ['Passed', 'Failed'].includes(legislation.status)) && totalVotes > 0 && (
                <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4">
                    <h3 className="text-xs font-semibold text-white mb-3">Abstimmungsliste</h3>
                    <div className="grid grid-cols-3 gap-3 mb-3">
                        ...
                        <div className="text-center"><div className="text-lg font-bold text-red-400">{legislation.votesAgainst}</div><div className="text-[10px] text-slate-400">Gegen</div></div>
                        <div className="text-center"><div className="text-lg font-bold text-slate-400">{legislation.votesAbstain}</div><div className="text-[10px] text-slate-400">Verzichten</div></div>
                    </div>
                    {totalVotes > 0 && (
            ...
            {isVoting && canVoteOnLegislation && myVotingPositions.length > 0 && !myVote && (
                <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-lg p-4">
                    <h3 className="text-xs font-semibold text-white mb-3">Gib Deine Stimme ab</h3>
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={() => handleVote('against')} disabled={isLoading} className="flex-1 py-2 text-xs font-bold text-red-400 bg-red-500/10 border border-red-500/20 rounded-md hover:bg-red-500/20 transition-colors disabled:opacity-40">Gegen</button>
                        <button onClick={() => handleVote('abstain')} disabled={isLoading} className="flex-1 py-2 text-xs font-bold text-slate-400 bg-slate-500/10 border border-slate-500/20 rounded-md hover:bg-slate-500/20 transition-colors disabled:opacity-40">Verzichten</button>
                    </div>
                </div>
            )}
            ...
            {/* Bill Body */}
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-5">
                <h3 className="text-xs font-semibold text-white mb-3">Rechnungstext</h3>
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-5">
                ...
                {legislation.comments && legislation.comments.length > 0 ? (
                    ...
                    <p className="text-xs text-slate-500 mb-4">Noch keine Kommentare.</p>
                )}
            ...
            >
                <div className="flex flex-col h-full">
                    <div className="p-6 space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Grund (optional)</label>
                    ...
                    <div className="flex justify-end items-center p-4 bg-slate-900/50 border-t border-white/5 shrink-0 gap-3">
                        <button type="button" onClick={() => setShowVetoForm(false)} disabled={isLoading} className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors disabled:opacity-50">Abbrechen</button>
                        <button
                            type="button"
                            onClick={handleVeto}
                            disabled={isLoading}
