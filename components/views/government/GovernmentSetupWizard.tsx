...
const TEMPLATE_OPTIONS: { type: GovernmentType; name: string; icon: string; description: string; color: string }[] = [
    { type: GovernmentType.MilitaryJunta, name: 'Military Junta', icon: 'fa-solid fa-helmet-safety', description: 'Oberbefehlshaber mit ernanntem Beratungsrat. Direkte Befehlskette.', color: 'red' },
    { type: GovernmentType.CorporateBoard, name: 'Corporate Board', icon: 'fa-solid fa-building', description: 'CEO führt die Exekutive. Vorstand von Mitgliedern gewählt.', color: 'blue' },
    { type: GovernmentType.DemocraticRepublic, name: 'Democratic Republic', icon: 'fa-solid fa-landmark-dome', description: 'Gewählter Präsident, Senat als Legislative und ernannte Judikative.', color: 'sky' },
    { type: GovernmentType.ConstitutionalMonarchy, name: 'Constitutional Monarchy', icon: 'fa-solid fa-crown', description: 'Erbmonarch mit gewähltem Parlament.', color: 'amber' },
    { type: GovernmentType.Westminster, name: 'Westminster Parliament', icon: 'fa-solid fa-landmark', description: 'Zweikammerparlament mit vom Haus gewähltem PM. Verhältniswahl.', color: 'emerald' },
    { type: GovernmentType.Technocracy, name: 'Technocracy', icon: 'fa-solid fa-microchip', description: 'Verdienstbasierter Expertenrat. Kompetenz vor Beliebtheit.', color: 'purple' },
    { type: GovernmentType.PirateCode, name: 'Pirate Code', icon: 'fa-solid fa-skull-crossbones', description: 'Kapitän von der Crew gewählt. Quartiermeister hält die Ordnung.', color: 'orange' },
    { type: GovernmentType.Custom, name: 'Custom', icon: 'fa-solid fa-puzzle-piece', description: 'Von null starten. Baue eigene Zweige und Positionen.', color: 'slate' },
];
        ...
        try {
            if (selectedType === GovernmentType.Custom) {
                ...
                await rpcAction('gov:upsert_config', {
                    config: { governmentType: 'custom', name: 'Government', description: 'Individuelle Regierungsstruktur' }
                });
            } else {
                await rpcAction('gov:apply_template', { templateType: selectedType });
            }
            addToast("Regierung gegründet", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50', { description: 'Deine Regierungsstruktur wurde erstellt. Du kannst sie im Admin-Panel weiter anpassen.' });
            onComplete();
        } catch (err: any) {
            addToast("Einrichtung fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to create government structure.' });
        } finally {
            setIsApplying(false);
        }
    };
    ...
    return (
        <div className="bg-slate-900 border border-slate-700/50 rounded-xl p-6 max-w-4xl mx-auto">
            <div className="text-center mb-6">
                ...
                <h2 className="text-xl font-bold text-white">Regierung gründen</h2>
            ...
            <div className="flex justify-end">
                ...
                >
                    {isApplying ? (
                        <>
                            <div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full"></div>Festlegung…</>
                    ) : (
                        <>
                            <i className="fa-solid fa-gavel"></i>Regierung gründen</>
                    )}
                </button>
            </div>
        </div>
