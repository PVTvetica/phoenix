...
const STATUS_STYLES: Record<Order['status'], { label: string; pill: string; dot: string }> = {
    draft:    { label: 'Entwurf',    pill: 'bg-slate-500/10 text-slate-300 border-slate-500/30', dot: 'bg-slate-500' },
    active:   { label: 'Aktiv',   pill: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30', dot: 'bg-emerald-500' },
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h2 className="text-xl font-bold text-white">Durchführungsverordnungen</h2>
                    <p className="text-sm text-slate-400 mt-0.5">Verbindliche Weisungen von autorisierten Positionsinhabern.</p>
                </div>
                {canIssue && (
                    <button onClick={() => setShowCreate(true)}
                        ...
                        <i className="fa-solid fa-gavel mr-2"></i>Auftrag erteilen</button>
                )}
            </div>
    ...
    <div className="bg-slate-900/30 border border-dashed border-slate-700 rounded-xl p-12 text-center">
        ...
        {canIssue && (
            <button onClick={onCreate}
                ...
                <i className="fa-solid fa-plus mr-2"></i>Entwerfen du eine Bestellung</button>
        )}
    </div>
);
...
const OrderEditorModal: React.FC<{
    ...
    const submit = async (status: 'draft' | 'active') => {
        ...
        try {
            ...
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>,
        ...
        >
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-5">
                    ...
                    {/* Position */}
                    <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Ausgabeposition</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-3">
                        <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Nummer (optional)</label>
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Titel</label>
                    ...
                    {/* Preamble */}
                    <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Präambel (optional)</label>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Operatives Gremium</label>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Begründung (Compliance)</label>
                    ...
                    {/* Expiry */}
                    <div>
                        <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Ablaufdatum (optional)</label>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 shrink-0 flex items-center justify-end gap-2">
                    <button onClick={onClose}
                        className="px-5 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white">Abbrechen</button>
...
const OrderDetailModal: React.FC<{
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            addToast("Entwurf gelöscht", <i className="fa-solid fa-trash"></i>, 'bg-slate-500/10 text-slate-400 border-slate-500/50');
            onRefresh();
            onClose();
        } catch (err: any) {
            addToast("Löschen fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
        }
    };
    ...
    const handleRevoke = async () => {
        ...
        try {
            ...
            addToast("Bestellung widerrufen", <i className="fa-solid fa-ban"></i>, 'bg-red-500/10 text-red-400 border-red-500/50');
            onRefresh();
            onClose();
        } catch (err: any) {
            addToast("Widerruf fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
        } finally {
            setRevoking(false);
        }
    };
    ...
    return (
        ...
        >
            <div className="flex flex-col h-full">
                ...
                <div className="p-6 space-y-5">
                    <div>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Operativer Text</p>
                        <p className="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed">{order.body}</p>
                    </div>

                    {order.rationale && (
                        <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4">
                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Begründung & Einhaltung</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-4 border-t border-slate-800">
                        {order.issued_at && <MetaCell label="Ausgegeben" value={fmt.date(order.issued_at)} />}
                        {order.effective_at && <MetaCell label="Effective" value={fmt.date(order.effective_at)} />}
                        {order.expires_at && <MetaCell label="Läuft ab" value={fmt.date(order.expires_at)} />}
                    ...
                    {order.issuer && (
                        <div className="flex items-center gap-3 pt-4 border-t border-slate-800">
                            ...
                            <div>
                                <p className="text-xs text-slate-500">Ausgestellt von</p>
                    ...
                    {order.status === 'revoked' && (
                        <div className="bg-red-500/5 border border-red-500/20 rounded-lg p-4">
                            <p className="text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2">Widerruf</p>
                ...
                {showRevoke && (
                    <div className="p-6 border-t border-slate-800 bg-red-500/5">
                        <label className="block text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2">Widerrufsgrund (optional)</label>
                        ...
                        <div className="flex items-center justify-end gap-2 mt-3">
                            <button onClick={() => setShowRevoke(false)}
                                className="px-4 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white">Abbrechen</button>
                    ...
                    <div className="flex items-center gap-2">
                        {canDelete && (
                            <button onClick={handleDelete}
                                ...
                                <i className="fa-solid fa-trash mr-2"></i>Entwurf löschen</button>
                        )}
                        {canRevoke && !showRevoke && (
                            <button onClick={() => setShowRevoke(true)}
                                className="px-4 py-2 text-xs font-bold uppercase tracking-widest text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-lg">
                                <i className="fa-solid fa-ban mr-2"></i>Bestellung widerrufen</button>
                        )}
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={onClose}
                            className="px-5 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white">Schließen</button>
                        {canEdit && (
                            <button onClick={onEdit}
                                ...
                                <i className="fa-solid fa-pen mr-2"></i>Bearbeiten</button>
                        )}
                    </div>
                </div>
            </div>
