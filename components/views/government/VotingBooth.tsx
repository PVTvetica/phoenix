...
const VotingBooth: React.FC<VotingBoothProps> = ({ election, onSubmit, onCancel }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast("Abstimmung fehlgeschlagen", err.message || 'Failed to cast your vote.', 'error');
        } finally {
    ...
    return (
        <div className="p-6">
            <div className="text-center mb-6">
                ...
                <h2 className="text-lg font-bold text-white">Wahlkabine</h2>
                <p className="text-xs text-slate-400 mt-1">{election.title}</p>
                <p className="text-[10px] text-slate-500 mt-0.5">Deine Stimme ist geheim und kann nicht dir zugeordnet werden.</p>
            </div>

            {/* Simple Majority / Plurality / Proportional — Radio selection */}
            {!isPreferential && !isApproval && (
                <div className="space-y-2 mb-6">
                    <p className="text-xs text-slate-400 mb-2">Wähle Deinen Wunschkandidaten aus:</p>
                    {candidates.map(c => (
                        <button
                            key={c.id}
            ...
            {isApproval && (
                <div className="space-y-2 mb-6">
                    <p className="text-xs text-slate-400 mb-2">Wähle alle Kandidaten aus, die du gutheißen:</p>
                    {candidates.map(c => (
                        <button
                            key={c.id}
                ...
                <div className="mb-6">
                    ...
                    {candidates.filter(c => !rankedCandidates.includes(c.id)).length > 0 && (
                        <div className="space-y-1">
                            <p className="text-[10px] text-slate-500 mb-1">Klicke hier, um zum Ranking hinzuzufügen:</p>
            ...
            <div className="flex justify-end gap-2 border-t border-slate-700/50 pt-4">
                <button onClick={onCancel} className="px-4 py-2 text-xs text-slate-400 hover:text-white">Abbrechen</button>
                <button
                ...
                >
                    {isSubmitting ? (
                        <><div className="animate-spin w-3 h-3 border-2 border-white border-t-transparent rounded-full"></div>Einreichen…</>
                    ) : (
                        <><i className="fa-solid fa-check"></i>Stimme abgeben</>
                    )}
                </button>
            </div>
        </div>
