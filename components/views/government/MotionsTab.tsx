...
const MotionsTab: React.FC = () => {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            addToast("Bewegung erstellt", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50', { description: 'Antrag wurde erstellt.' });
            await refreshGovernment();
        } catch (err: any) {
            addToast("Erstellen fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to create motion.' });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleStartVote = async (motionId: number) => {
        ...
        try {
            ...
            addToast("Die Abstimmung hat begonnen", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            await refreshGovernment();
        } catch (err: any) { addToast("Start fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to start voting.' }); }
    ...
    const handleCastVote = async (motionId: number, vote: 'for' | 'against' | 'abstain') => {
        ...
        try {
            ...
            addToast("Stimmabgabe", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            await refreshGovernment();
        } catch (err: any) { addToast("Abstimmung fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to cast vote.' }); }
        finally { setIsLoading(false); }
    };
    ...
    const handleConclude = async (motionId: number) => {
        ...
        try {
            ...
            addToast("Antrag abgeschlossen", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            await refreshGovernment();
        } catch (err: any) { addToast("Abschluss fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to conclude motion.' }); }
        finally { setIsLoading(false); }
    };
    ...
    const handleCancel = async (motionId: number) => {
        ...
        try {
            ...
            addToast("Antrag abgebrochen", <i className="fa-solid fa-ban" />, 'bg-slate-500/10 text-slate-300 border-slate-500/40');
            await refreshGovernment();
        } catch (err: any) { addToast("Abbrechen fehlgeschlagen", <i className="fa-solid fa-circle-exclamation" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err.message || 'Failed to cancel motion.' }); }
        finally { setIsLoading(false); }
    };
        ...
        return (
            <div key={motion.id} className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4">
                ...
                <div className="flex items-center gap-3 text-[10px] text-slate-500 mb-3">
                    ...
                    {motion.isSecretBallot && <span className="text-purple-400"><i className="fa-solid fa-mask mr-0.5"></i>Geheimnis</span>}
                    {motion.restrictedToPositionIds && motion.restrictedToPositionIds.length > 0 && (
                        <span className="text-amber-400"><i className="fa-solid fa-lock mr-0.5"></i>Eingeschränkt</span>
                    )}
                </div>
                ...
                {isVoting && canParticipate && !motion.hasVoted && (
                    <div className="flex gap-1.5">
                        ...
                        <button onClick={() => handleCastVote(motion.id, 'against')} disabled={isLoading} className="flex-1 py-1.5 text-[10px] font-bold text-red-400 bg-red-500/10 border border-red-500/20 rounded-sm hover:bg-red-500/20 transition-colors disabled:opacity-40">Gegen</button>
                        <button onClick={() => handleCastVote(motion.id, 'abstain')} disabled={isLoading} className="flex-1 py-1.5 text-[10px] font-bold text-slate-400 bg-slate-500/10 border border-slate-500/20 rounded-sm hover:bg-slate-500/20 transition-colors disabled:opacity-40">Verzichten</button>
                    </div>
                )}
                ...
                {canManage && (
                    <div className="flex gap-1.5 mt-2">
                        {motion.status === 'Open' && (
                            <button onClick={() => handleStartVote(motion.id)} disabled={isLoading} className="text-[10px] text-emerald-400 hover:underline disabled:opacity-40">Abstimmung starten</button>
                        )}
                        {isVoting && (
                            <button onClick={() => handleConclude(motion.id)} disabled={isLoading} className="text-[10px] text-sky-400 hover:underline disabled:opacity-40">Abschließen</button>
                        )}
                        {['Open', 'Voting'].includes(motion.status) && (
                            <button onClick={() => handleCancel(motion.id)} disabled={isLoading} className="text-[10px] text-red-400 hover:underline disabled:opacity-40">Abbrechen</button>
                        )}
                    </div>
                )}
            </div>
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <h2 className="text-sm font-bold text-white">Anträge</h2>
                {canManage && (
                    <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-purple-400 bg-purple-500/10 border border-purple-500/20 rounded-md hover:bg-purple-500/20 transition-colors">
                        <i className="fa-solid fa-plus"></i>Neuer Antrag</button>
                )}
            </div>

            {activeMotions.length > 0 && (
                <div>
                    <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Aktiv</h3>
            ...
            {pastMotions.length > 0 && (
                <div>
                    <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Vergangenheit</h3>
            ...
            {governmentMotions.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-sm text-slate-400">Es wurden noch keine Anträge gestellt.</p>
                </div>
            )}
                ...
                <div className="flex flex-col h-full">
                    <div className="p-6 space-y-4">
                        ...
                        <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                            <input type="checkbox" checked={formSecretBallot} onChange={e => setFormSecretBallot(e.target.checked)} className="rounded-sm bg-slate-800 border-slate-600 text-purple-500 focus:ring-purple-500" disabled={isLoading} />Geheime Abstimmung (Abstimmung erfolgt anonym)</label>
                        {governmentPositions.length > 0 && (
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Abstimmung auf Positionen beschränken (optional)</label>
                    ...
                    <div className="flex justify-end items-center p-4 bg-slate-900/50 border-t border-white/5 shrink-0 gap-3">
                        <button type="button" onClick={() => setShowCreateModal(false)} disabled={isLoading} className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors disabled:opacity-50">Abbrechen</button>
                        <button
                            type="button"
