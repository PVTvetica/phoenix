...
const NewUserSetupView: React.FC<NewUserSetupViewProps> = ({ pendingUser, onSetupComplete, isAdminSetup, brandingConfig }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-950 p-6 relative overflow-hidden font-sans">
            ...
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(239,68,68,0.1)_0%,transparent_70%)] pointer-events-none"></div>
            ...
            <div className="w-full max-w-lg relative z-10 animate-fade-in-up space-y-6">
                ...
                <div className="bg-slate-900/60 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden relative">
                    <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-red-800 via-red-600 to-red-800"></div>

                    <div className="p-8 md:p-10 text-center">
                        {/* Logo/Icon Area */}
                        <div className="w-16 h-16 bg-sky-500/10 rounded-2xl flex items-center justify-center mx-auto mb-6 border border-sky-500/20 shadow-[0_0_30px_rgba(239,68,68,0.2)]">
                        ...
                        <div className="mb-10">
                            ...
                            <div className="relative inline-block group">
                                <div className="absolute -inset-1 bg-linear-to-r from-red-600 to-red-800 rounded-full blur-sm opacity-25 group-hover:opacity-50 transition duration-1000"></div>
                        ...
                        {step === 'INPUT' ? (
                            <form onSubmit={handleNext} className="space-y-6 text-left animate-fade-in">
                                ...
                                <button type="submit" className="w-full py-4 bg-sky-600 hover:bg-sky-500 text-white font-black uppercase tracking-[0.2em] rounded-xl shadow-lg shadow-sky-900/20 transition-all active:scale-95 text-xs">Nächster Schritt<i className="fa-solid fa-arrow-right ml-2"></i>
                                </button>
                            </form>
                        ) : (
                            <div className="space-y-6 text-left animate-fade-in">
                                <div className="bg-black/40 border border-slate-800 rounded-xl p-5">
                                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-1">Zielidentität</p>
                                    <p className="text-lg font-bold text-white font-mono">{rsiHandle}</p>
                                </div>

                                <div className="space-y-4">
                                    <p className="text-[10px] font-black text-sky-500 uppercase tracking-widest">Verifizierungssequenz</p>
                                    <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-4 space-y-4 text-xs leading-relaxed">
                                        <div className="flex gap-3">
                                            ...
                                            <p className="text-slate-300">Besuchen<a href="https://robertsspaceindustries.com/account/profile" target="_blank" rel="noopener noreferrer" className="text-sky-400 hover:underline">rsi.com/account/profile</a></p>
                                        </div>
                                        <div className="flex gap-3">
                                            <span className="text-sky-500 font-mono">02</span>
                                            <div className="flex-1 space-y-2">
                                                <p className="text-slate-300">Kopieren du diesen Whitelabel-Bestätigungscode:</p>
                                        ...
                                        <div className="flex gap-3">
                                            ...
                                            <p className="text-slate-300">Füge den Code in Dein ein<strong>"Bio"</strong> section and save.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-3 pt-2">
                                    <button onClick={() => setStep('INPUT')} className="flex-1 py-4 bg-slate-800 hover:bg-slate-700 text-slate-400 font-bold uppercase tracking-widest rounded-xl text-xs transition-all">Zurück</button>
                                    <button onClick={handleSubmit} disabled={isLoading} className="flex-2 py-4 bg-sky-600 hover:bg-sky-500 text-white font-black uppercase tracking-[0.2em] rounded-xl shadow-lg shadow-sky-900/20 transition-all active:scale-95 text-xs disabled:opacity-50">
                                        {isLoading ? <><i className="fa-solid fa-circle-notch animate-spin mr-2" />Synchronisierung…</> : "Verify & Link"}
                                    </button>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="bg-black/20 p-4 border-t border-white/5 flex justify-between items-center text-[10px] text-slate-600 font-mono uppercase tracking-wider">
                        <span className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-sky-500 animate-pulse"></div>Sichere Anmeldung</span>
