        ...
        >
            <div className="p-5 space-y-4">
                ...
                {member ? (
                    ...
                    <div>
                        <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400">Ausgabe an</span>
                        <input
                            type="text"
                            value={memberSearch}
                        ...
                        <div className="mt-1 max-h-44 overflow-y-auto space-y-1 border border-white/5 rounded-lg bg-slate-950/60 p-1">
                            {filteredMembers.length === 0 && <div className="text-xs text-slate-500 p-2">Keine Übereinstimmungen.</div>}
                ...
                <div>
                    <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400">Artikel im Kit</span>
                        {lines.length > 0 && !pickerOpen && (
                            ...
                            >
                                <i className="fa-solid fa-plus mr-1" />Artikel hinzufügen</button>
                        )}
                    </div>
                    {lines.length > 0 && (
                        ...
                        <div className="bg-slate-950/60 border border-white/5 rounded-lg p-2">
                            ...
                            <div className="max-h-44 overflow-y-auto space-y-1">
                                {pickerLoading && availableForPicker.length === 0 && (
                                    <div className="text-xs text-slate-500 p-2"><i className="fa-solid fa-spinner animate-spin mr-2" />Suche…</div>
                                )}
                                {!pickerLoading && availableForPicker.length === 0 && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label className="block">
                        <span className="text-[10px] font-mono uppercase tracking-widest text-slate-400">Fällig zurück (optional)</span>
                        <input
                            type="datetime-local"
                            value={dueBackAt}
                            ...
                            className="mt-1 w-full bg-slate-900 border border-white/10 rounded-lg px-3 py-2 text-sm text-white"
                        />
                        <span className="text-[10px] text-slate-600 mt-1 block">Gilt für jede Zeile</span>
                    </label>
                    <label className="block">
