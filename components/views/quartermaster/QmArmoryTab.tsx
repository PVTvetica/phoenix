...
export default function QmArmoryTab({ locations, canManage, canRequest, onIssue, onCreate, refreshKey }: Props) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (seq !== requestSeq.current) return;
            addToast("Das Laden des Inventars ist fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
        } finally {
    ...
    const requestItem = async (item: QmInventoryItem) => {
        ...
        if (!Number.isFinite(qty) || qty <= 0) {
            addToast("Ungültige Menge", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50');
            return;
        }
        ...
        try {
            ...
            addToast("Antrag eingereicht", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50', {
                description: 'Ein Offizier erledigt die Anfrage.',
            });
            load();
        } catch (err: any) {
            addToast("Die Anfrage ist fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
        }
    };
    ...
    const exportCsv = async () => {
        try {
            ...
            addToast("Der Export ist fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
        }
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                >
                    <i className="fa-solid fa-file-csv" />CSV exportieren</button>
                {onCreate && (
                    <button
                        onClick={onCreate}
                    ...
                    >
                        <i className="fa-solid fa-plus" />Lagerbestand hinzufügen</button>
                )}
            </div>
                                ...
                                <div className="flex-1 p-4 flex flex-col min-w-0">
                                    ...
                                    <div className="flex items-baseline gap-3 mt-1">
                                        <div>
                                            ...
                                            <div className="text-[10px] font-mono uppercase tracking-widest text-slate-500">Zur Hand</div>
                                        </div>
                                        {it.quantityOnIssue > 0 && (
                                            <div>
                                                <div className="text-lg font-bold font-mono text-sky-300">{it.quantityOnIssue}</div>
                                                <div className="text-[10px] font-mono uppercase tracking-widest text-slate-500">Zum Thema</div>
                                            </div>
                                        )}
                                    </div>
                                    <div className="flex-1" />
                                    <div className="flex items-center gap-2 pt-3 mt-3 border-t border-white/5">
                                        ...
                                        {canManage && (
                                            <button
                                                ...
                                                title="Adjust stock"
                                            >
                                                <i className="fa-solid fa-sliders mr-1" />Anpassen</button>
                                        )}
            ...
            {totalCount > PAGE_SIZE && (
                <div className="flex justify-end items-center gap-2 text-xs text-slate-400">
                    <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}
                        ...
                        <i className="fa-solid fa-chevron-left mr-1" />Vorher</button>
                    <span>Page {page + 1} / {totalPages}</span>
                    <button onClick={() => setPage((p) => p + 1)} disabled={page >= totalPages - 1}
                        className="px-3 py-1.5 bg-slate-800 border border-white/10 rounded-sm text-xs font-bold disabled:opacity-30 hover:bg-slate-700">Nächste<i className="fa-solid fa-chevron-right ml-1" />
                    </button>
                </div>
            )}
