...
const TABS: readonly { key: Tab; label: string; icon: string; permission?: string }[] = [
    ...
    { key: 'armory',    label: 'Waffenkammer',    icon: 'fa-boxes-stacked' },
    { key: 'issuances', label: 'Emissionen', icon: 'fa-clipboard-list' },
    { key: 'catalog',   label: 'Katalog',   icon: 'fa-book' },
    { key: 'settings',  label: 'Einstellungen',  icon: 'fa-gear', permission: 'qm:manage' },
];
        ...
        return (
            <div className="p-8">
                <EmptyState
                    icon="fa-lock"
                    accent="orange"
                    heading="Du hast keinen Zugriff auf den Quartiermeister"
                    description="Bitte einen Admin, dir die Berechtigung qm:view zu gewähren."
                />
            </div>
        );
    }
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden bg-slate-950 text-white animate-fade-in">
            <HeroShell
                ...
                chipAccent="orange"
                title="Org Armoury"
                subtitle="Verfolge physische Ingame-Assets an deinen Standorten. Kit an Mitglieder für Einsätze ausgeben und Rückgaben protokollieren — das Bewegungs-Ledger hält Bestände ehrlich."
                actions={canManage && !firstRun && (
                    <HeroActionButton onClick={() => setCreateInventoryOpen(true)} accent="orange" icon="fa-plus">Lagerbestand hinzufügen</HeroActionButton>
                )}
                ...
                {!isLoading && firstRun && (
                    <div className="rounded-2xl border border-white/10 bg-slate-900/40 p-10 space-y-6">
                        <EmptyState
                            icon="fa-warehouse"
                            accent="orange"
                            heading="Richte dein Arsenal ein"
                            description="Lege zuerst einen Lagerort an (z. B. „Main Hangar“), dann deinen ersten Bestandseintrag. Katalogartikel sind optional — auch Freitext-Namen möglich."
                        />
                        {canManage && (
                            <div className="flex flex-wrap items-center justify-center gap-3">
                                ...
                                >
                                    <i className="fa-solid fa-map-location-dot" />Erstelle den ersten Standort</button>
                            </div>
                        )}
                    </div>
                )}
                    ...
                    <QmIssuancesTab
                        ...
                        onFulfil={async (issuanceId) => {
                            try {
                                ...
                                addToast("Ausgabe erfüllt", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
                                refresh();
                            } catch (err: any) {
                                addToast("Die Erfüllung ist fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
                            }
                        }}
