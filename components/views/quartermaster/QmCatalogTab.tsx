...
export default function QmCatalogTab({ catalog, canAdmin, onRefresh }: Props) {
    ...
    const createItem = async () => {
        ...
        try {
            ...
            addToast("Katalogelement hinzugefügt", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            onRefresh();
        } catch (err: any) {
            addToast("Erstellen ist fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const removeItem = async (item: QmCatalogItem) => {
        ...
        try {
            ...
            addToast("Katalogelement gelöscht", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
            onRefresh();
        } catch (err: any) {
            addToast("Das Löschen ist fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message });
        }
    };
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                    <h2 className="text-sm font-bold text-white uppercase tracking-widest">Katalog</h2>
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/30 p-3 flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <label className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-slate-400 cursor-pointer select-none whitespace-nowrap">
                    ...
                    />Plattformkatalog einbinden</label>
            </div>
                    ...
                    <table className="w-full text-left text-sm">
                        <thead className="bg-black/30 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                            <tr>
                                ...
                                <th className="px-3 py-2">Unterkategorie</th>
                                <th className="px-3 py-2">Quelle</th>
                                <th className="px-3 py-2 w-16 text-right">Aktionen</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            ...
                            {customFiltered.length === 0 && !debouncedSearch && catalog.length === 0 && (
                                <tr><td colSpan={5} className="px-3 py-4 text-center text-xs text-slate-500 italic">Keine organisationsspezifischen Elemente.</td></tr>
                            )}
                            ...
                            <table className="w-full text-left text-sm">
                                <tbody className="divide-y divide-white/5">
                                    ...
                                    {!searchLoading && !searchError && platformResults.length === 0 && (
                                        <tr><td colSpan={5} className="px-3 py-4 text-center text-xs text-slate-500 italic">Keine Plattformübereinstimmungen.</td></tr>
                                    )}
