    ...
    return (
        <DashboardCard title="Priority Dispatch" icon="fa-solid fa-tower-broadcast" className="h-full">
            {priorityRequests.length > 0 ? (
                ...
                <div className="flex items-center justify-center h-full text-slate-500 italic text-xs">Keine ausstehenden Anfragen.</div>
            )}
        </DashboardCard>
    );
};
    ...
    return (
        <DashboardCard title="Active Operations" icon="fa-solid fa-person-military-rifle" className="h-full">
            {activeOps.length > 0 ? (
                    ...
                    {activeOps.map(op => (
                        <div key={op.id} onClick={() => viewOperationDetails(op)} className="p-3 bg-slate-800/50 rounded-sm border border-slate-700 hover:border-green-500/30 cursor-pointer transition-colors group">
                            <div className="flex justify-between items-center mb-1">
                                ...
                                <span className="text-[10px] bg-green-500/10 text-green-400 px-2 py-0.5 rounded-sm border border-green-500/20 uppercase font-bold">Aktiv</span>
                            </div>
                ...
                <div className="flex items-center justify-center h-full text-slate-500 italic text-xs">Keine Vorgänge aktiv.</div>
            )}
        </DashboardCard>
    );
};
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-full">
                    {vacancies.map(job => (
                        <div key={job.id} className="p-4 bg-slate-800/30 rounded-sm border border-slate-700 flex flex-col justify-between h-full">
                            ...
                            <button onClick={() => openApplyJobModal(job)} className="mt-3 text-xs bg-slate-700 hover:bg-slate-600 text-white py-1.5 rounded-sm transition-colors w-full uppercase font-bold">Anwenden</button>
                        </div>
                    ))}
                </div>
            ) : (
                <div className="flex items-center justify-center h-full text-slate-500 italic text-xs">Keine offenen Stellen aufgeführt.</div>
            )}
        </DashboardCard>
    );
};
...
const ClientMissionTracker: React.FC<{ request: HydratedServiceRequest }> = ({ request }) => {
    ...
    const steps = [
        { status: ServiceRequestStatus.Submitted, label: 'Eingereicht', icon: 'fa-paper-plane' },
        { status: ServiceRequestStatus.Triaged, label: 'Akzeptiert', icon: 'fa-clipboard-check' },
        { status: ServiceRequestStatus.Accepted, label: 'Zugewiesen', icon: 'fa-user-group' },
            ...
            <div className="flex flex-col h-full justify-between gap-6">
                <div>
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <button onClick={() => viewRequestDetails(request)} className="text-xs bg-slate-800 hover:bg-slate-700 text-sky-400 px-3 py-1.5 rounded-sm border border-slate-600 transition-colors">Details anzeigen</button>
                    </div>
                ...
                <div className="bg-sky-900/20 border border-sky-500/20 p-4 rounded-lg flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-sky-300 font-bold text-sm">Die Mitarbeiter koordinieren.</p>
    ...
    if (currentUser && currentUser.reputation <= 10) {
        return (
            <div className="flex flex-col items-center justify-center py-6 text-center space-y-4 animate-fade-in">
                ...
                <div>
                    <h3 className="text-white font-bold text-base uppercase tracking-wider">Service eingeschränkt</h3>
                ...
                >
                    <i className="fa-brands fa-discord mr-2 text-sm group-hover:animate-pulse"></i>Fordern du eine Überprüfung an</a>
            </div>
        );
    }
    ...
    if (onDutyCount === 0) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center p-8 space-y-4 bg-slate-900/50 rounded-lg border border-slate-700/50">
                ...
                <div>
                    <h3 className="text-white font-bold text-xl">Dienste nicht verfügbar</h3>
                ...
                >
                    <i className="fa-brands fa-discord mr-2"></i>Kontaktiere Command</a>
            </div>
        );
    }
        ...
        <>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Gefahr</label>
                ...
                <button type="submit" disabled={!tosAgreed} className="w-full bg-sky-600 hover:bg-sky-500 text-white font-bold py-3 rounded-sm shadow-lg shadow-sky-900/20 disabled:bg-slate-700 disabled:text-slate-500 transition-all">Anfrage senden</button>
            </form>
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center z-200 animate-fade-in p-4">
                    <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
                        <div className="flex justify-between items-center p-5 border-b border-slate-700">
                            <h2 className="text-xl font-bold text-white">Nutzungsbedingungen</h2>
                        ...
                        <div className="p-6 overflow-y-auto custom-scrollbar">
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(brandingConfig.termsOfService || '<p>Nutzungsbedingungen nicht konfiguriert.</p>') }}
                            />
                        </div>
...
const StaffDashboard: React.FC<{
    ...
    const SectionHeader: React.FC<{ title: string; icon: string; count?: number; viewAllTarget?: string }> = ({ title, icon, count, viewAllTarget }) => (
        <div className="flex items-center justify-between mb-4">
            ...
            {viewAllTarget && (
                <button onClick={() => setActiveView(viewAllTarget)} className="text-[10px] font-bold text-slate-500 hover:text-sky-400 uppercase tracking-wider transition-colors">Alle anzeigen<i className="fa-solid fa-arrow-right ml-1"></i>
                </button>
            )}
        </div>
    );
                                ...
                                <div className="flex flex-col gap-6 h-full">
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex gap-4 text-xs font-mono text-slate-500">
                                            <span className="flex items-center"><i className="fa-solid fa-shield-halved mr-2 text-sky-500"></i>Sicher</span>
                                            <span className="flex items-center"><i className="fa-solid fa-bolt mr-2 text-yellow-500"></i>Schnell</span>
                                            <span className="flex items-center"><i className="fa-solid fa-check-circle mr-2 text-green-500"></i>Zuverlässig</span>
                                        </div>
                                    </div>
                        ...
                        {pendingRating && (
                            <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-xl flex items-center justify-between animate-fade-in">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-white font-bold">Feedback erforderlich</h4>
                                        <p className="text-amber-200/60 text-xs">Please rate your recent {pendingRating.serviceType} service.</p>
                                    </div>
                                </div>
                                <button onClick={() => openRateRequestModal(pendingRating)} className="bg-amber-500 hover:bg-amber-400 text-black font-bold px-4 py-2 rounded-sm text-xs uppercase tracking-wide transition-colors">Jetzt bewerten</button>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-5 flex flex-col gap-4">
                            <div className="flex items-center gap-3 mb-1">
                                ...
                                <div>
                                    <h3 className="font-bold text-white text-sm uppercase tracking-wide">Treten du unserer Organisation bei</h3>
                                    <p className="text-xs text-slate-400">Werden du Mitglied unserer Organisation.</p>
                                </div>
                            </div>
                            ...
                            {myApplication ? (
                                <div className="bg-slate-900/50 p-3 rounded-sm border border-slate-700/50">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="text-[10px] font-black uppercase text-slate-500 tracking-wider">Bewerbungsstatus</span>
