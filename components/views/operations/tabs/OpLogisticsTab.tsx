        ...
        <div className="p-4 md:p-6 space-y-6">
            <div className="flex items-center justify-between">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <i className="fa-solid fa-boxes-stacked text-slate-500"></i>Logistik</h3>
                {canManage && (
                    <button onClick={() => setShowForm(!showForm)} className="text-[10px] font-bold text-purple-300 hover:text-purple-200 uppercase">
                        <i className="fa-solid fa-plus mr-1"></i>Artikel hinzufügen</button>
                )}
            </div>
            ...
            {items.length > 0 && (
                <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Gesamterfüllung</span>
            ...
            {showForm && (
                <div className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-4 space-y-3 animate-fade-in">
                    ...
                    <div className="flex justify-end gap-2">
                        <button onClick={() => setShowForm(false)} className="text-xs text-slate-400 hover:text-white px-3 py-1.5">Abbrechen</button>
            ...
            {items.length === 0 && !canManage && (
                <div className="flex flex-col items-center justify-center h-32 text-slate-600 opacity-50">
                    ...
                    <p className="text-xs italic">Keine Logistikartikel verfolgt.</p>
                </div>
            )}
        </div>
    );
