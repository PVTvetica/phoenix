        ...
        <div className="p-6 lg:p-8 space-y-4">
            <div className="flex items-center justify-between">
                <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                    <i className="fa-solid fa-tower-broadcast text-purple-400/70"></i>Kommunikationsplan</p>
                {canManage && !isEditing && (
                    <button onClick={openEditor}
                        ...
                        <i className="fa-solid fa-pen-to-square"></i>Bearbeiten</button>
                )}
            </div>
            ...
            {isEditing ? (
                ...
                <div className="text-center py-12">
                    ...
                    <p className="text-slate-600 text-xs italic">Kein Kommunikationsplan definiert.</p>
                </div>
            )}
        </div>
    );
            ...
            <div className="rounded-lg border border-slate-700/40 bg-slate-800/20 px-4 py-3">
                <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="text-[9px] text-slate-600 uppercase tracking-widest mt-1.5">Legacy-Eintrag – zum Konvertieren bearbeiten</div>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="space-y-3">
            ...
            <button onClick={onAddRow} className="text-xs text-purple-300 hover:text-purple-200">
                <i className="fa-solid fa-plus mr-1"></i>Kanal hinzufügen</button>
            <div className="flex gap-3 justify-end">
                <button onClick={onCancel} className="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors">Abbrechen</button>
...
const DiscordChannelPicker: React.FC<{
    ...
    if (error) {
        return (
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-[11px] text-amber-200 leading-relaxed">
                ...
                {error}
                <button onClick={onRefresh} className="ml-2 underline hover:text-amber-100">Wiederholen</button>
            </div>
        );
    }
    ...
    if (loading && channels.length === 0) {
        return (
            <div className="text-[11px] text-slate-500"><i className="fa-solid fa-circle-notch fa-spin mr-2"></i>Kanäle werden geladen…</div>
        );
    }

    if (filtered.length === 0) {
        return (
            <div className="rounded-lg border border-slate-700/40 bg-slate-800/30 px-3 py-2 text-[11px] text-slate-400">Keine passenden Kanäle gefunden.<button onClick={onRefresh} className="ml-2 underline text-purple-300 hover:text-purple-200">Aktualisieren</button>
            </div>
        );
    }
...
const ELEMENT_TOOLS: { type: OperationBoardElement['elementType']; icon: string; color: string; label: string }[] = [
    { type: 'unit', icon: 'fa-solid fa-users', color: '#3b82f6', label: 'Einheit' },
    { type: 'waypoint', icon: 'fa-solid fa-location-dot', color: '#f59e0b', label: 'Waypoint' },
    { type: 'ship', icon: 'fa-solid fa-rocket', color: '#06b6d4', label: 'Schiff' },
    ...
    { type: 'icon', icon: 'fa-solid fa-icons', color: '#a855f7', label: 'Symbol' },
];
            ...
            {canManage && (
                <div className="shrink-0 px-3 py-2 bg-slate-900/80 border-b border-slate-700/30 flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className="text-[9px] text-slate-600 font-mono">
                        {activeTool === 'draw' ? <span className="text-purple-300">Freihand zeichnen · Esc zum Abbrechen</span> :
                ...
                <div ref={containerRef} className="relative flex-1 min-h-0 overflow-hidden bg-slate-950"
                    ...
                    {activeTool && (
                        <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 bg-purple-500/10 border border-purple-500/30 text-purple-300 text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg backdrop-blur-xs flex items-center gap-2 pointer-events-none animate-pulse">
                            {activeTool === 'draw' ? (
                                <><i className="fa-solid fa-pencil"></i>Zum Zeichnen klicken und ziehen · Esc zum Abbrechen</>
                            ) : activeTool === 'zone' || activeTool === 'line' ? (
                                <><i className={`${ELEMENT_ICONS[activeTool]?.icon}`}></i>Zum Platzieren klicken oder auf Größe ziehen · Esc zum Abbrechen</>
                            ) : (
                                <><i className={`${ELEMENT_ICONS[activeTool]?.icon}`}></i>Zum Platzieren klicken · Esc zum Abbrechen</>
                            )}
                        </div>
                    )}
                    ...
                    {elements.length === 0 && !activeTool && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div className="text-center text-slate-600">
                                ...
                                <p className="text-sm font-medium opacity-50">Taktisches Brett</p>
                ...
                {selectedElement && (
                    <div className="w-56 shrink-0 bg-slate-900/80 border-l border-slate-700/30 overflow-y-auto custom-scrollbar">
                        <div className="p-3 border-b border-slate-700/30 flex items-center justify-between">
                            <p className="text-[9px] text-slate-500 uppercase font-black tracking-widest">Eigenschaften</p>
                        ...
                        <div className="p-3 space-y-3">
                            <div>
                                <label className="text-[9px] text-slate-500 uppercase font-bold tracking-wider block mb-1">Typ</label>
                            ...
                            <div>
                                <label className="text-[9px] text-slate-500 uppercase font-bold tracking-wider block mb-1">Etikett</label>
                            ...
                            <div>
                                <label className="text-[9px] text-slate-500 uppercase font-bold tracking-wider block mb-1">Farbe</label>
                                <div className="flex items-center gap-2">
                            ...
                            {savingEdit && <p className="text-[9px] text-purple-300"><i className="fa-solid fa-spinner animate-spin mr-1"></i>Sparen…</p>}
                            {canManage && (
                                <div className="pt-2 border-t border-slate-700/30">
                                    <button onClick={() => handleDeleteElement(selectedElement.id)}
                                        ...
                                        <i className="fa-solid fa-trash"></i>Element löschen</button>
                                </div>
                            )}
                        </div>
                    </div>
...
const OpCommandSignalsTab: React.FC<OpCommandSignalsTabProps> = ({ operation, canManage, isParticipant, onRefresh }) => {
    ...
    const subTabs: { key: SubTab; label: string; icon: string }[] = [
        ...
        { key: 'tactical-board', label: 'Taktisches Brett', icon: 'fa-solid fa-map' },
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="shrink-0 px-6 lg:px-8 pt-6 lg:pt-8 pb-4">
                <div className="flex items-center justify-between gap-4">
                    <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                        <i className="fa-solid fa-tower-broadcast text-purple-400/70"></i>Befehl und Signale</p>
