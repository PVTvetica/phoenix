...
const OpAdministerTab: React.FC<OpAdministerTabProps> = ({ operation, canManage, onRefresh, onDeleteOperation, isDeleting, loadingAction, onAction }) => {
    ...
    const handleRepostAnnouncement = useCallback(async () => {
        if (!pickerChannelId) {
            addToast("Wähle einen Kanal", <i className="fa-solid fa-triangle-exclamation"></i>, 'bg-amber-500/10 text-amber-400 border-amber-500/50', { description: 'Wähle einen Discord-Kanal, bevor du postest.' });
            return;
        }
        setReposting(true);
        try {
            ...
            if (result?.ok) {
                ...
                addToast("Beitrag fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: result?.error || 'Discord did not accept the post.' });
            }
        } catch (err: any) {
            addToast("Beitrag fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message || 'Could not reach the server.' });
        } finally {
            setReposting(false);
        }
        ...
        try {
            ...
            if (!payload?.phases?.length) {
                addToast(
                    ...
                    { description: 'Dieser Einsatz hat keine Phasen, Aufgaben oder Meilensteine als Vorlage.' }
                );
                return;
            }
            await createOperationTemplate({ name: name.trim(), payload });
            addToast("Vorlage gespeichert", <i className="fa-solid fa-check"></i>, 'bg-green-500/10 text-green-400 border-green-500/30');
        } catch (err: any) {
            addToast(
                'Save Failed',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (result?.discordEventFailed) {
                ...
                addToast("Vorgang aktualisiert", <i className="fa-solid fa-check"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: "Einsatzeinstellungen wurden gespeichert." });
            }
            onRefresh();
        } catch (err) {
            console.error("Failed to update:", err);
            addToast("Aktualisierung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Einsatzeinstellungen konnten nicht aktualisiert werden." });
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleInviteAlly = async () => {
        ...
        try {
            ...
            addToast("Verbündeter eingeladen", <i className="fa-solid fa-check"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: "Die verbündete Org wurde zu diesem Einsatz eingeladen." });
            setInvitePeerId('');
            onRefresh();
        } catch {
            addToast("Einladung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Verbündeter konnte nicht eingeladen werden. Stelle sicher, dass gemeinsame Einsätze für diesen Peer aktiv sind." });
        } finally {
        ...
        <div className="p-6 lg:p-8 space-y-6">
            ...
            <div className="flex items-center justify-between p-4 bg-linear-to-r from-slate-800/60 to-slate-900/40 rounded-xl border border-slate-700/40 animate-fade-in-down">
                <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                    <i className="fa-solid fa-gear text-purple-400/70"></i>Vorgang Administration</p>
                {!isEditing && (
                    <button onClick={startEdit}
                        ...
                        <i className="fa-solid fa-pen-to-square"></i>Bearbeiten du alle Details</button>
                )}
            </div>
                ...
                <div className={cardClass}>
                    <div className="px-5 py-3 bg-purple-500/5 border-b border-purple-500/10">
                        <p className="text-[10px] text-purple-300/80 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                            <i className="fa-solid fa-pen-to-square"></i>Vorgang bearbeiten</p>
                    </div>
                    <div className="p-6 space-y-6">
                        {/* Core Details */}
                        <div className="space-y-4">
                            <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest flex items-center gap-2"><i className="fa-solid fa-info-circle"></i>Kerndetails</p>
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className={labelClass}>Maximale Teilnehmerzahl</label>
                                ...
                                <div>
                                    <label className={labelClass}>Host-Einheit</label>
                                ...
                                <div>
                                    <label className={labelClass}>Standort / AO</label>
                        ...
                        <div className="border-t border-slate-700/30 pt-5 space-y-4">
                            <p className="text-[10px] text-amber-400/70 uppercase font-black tracking-widest flex items-center gap-2"><i className="fa-solid fa-clock"></i>Zeitplan</p>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className={labelClass}>Geplanter Start</label>
                                ...
                                <div>
                                    <label className={labelClass}>Geplantes Ende</label>
                        ...
                        {/* Configuration Flags */}
                        <div className="border-t border-slate-700/30 pt-5 space-y-4">
                            <p className="text-[10px] text-purple-400/70 uppercase font-black tracking-widest flex items-center gap-2"><i className="fa-solid fa-sliders"></i>Konfiguration</p>
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                <button type="button" onClick={() => setEditIsSpecial(!editIsSpecial)}
                                    ...
                                    <span className="text-[10px] font-bold uppercase tracking-wider">Klassifiziert</span>
                                ...
                                <button type="button" onClick={() => setEditIsJoint(!editIsJoint)}
                                    ...
                                    <span className="text-[10px] font-bold uppercase tracking-wider">Gemeinsame Operation</span>
                                ...
                                <button type="button" onClick={() => setEditIsTraining(!editIsTraining)}
                                    ...
                                    <span className="text-[10px] font-bold uppercase tracking-wider">Trainingsübung</span>
                                ...
                                <button type="button" onClick={() => setEditTracksUec(!editTracksUec)}
                                    ...
                                    <span className="text-[10px] font-bold uppercase tracking-wider">UEC verfolgen</span>
                            ...
                            {editIsSpecial && (
                                <div>
                                    <label className={labelClass}>Beitrittscode / PIN</label>
                        ...
                        {/* Security */}
                        <div className="border-t border-slate-700/30 pt-5 space-y-4">
                            <p className="text-[10px] text-red-400/70 uppercase font-black tracking-widest flex items-center gap-2"><i className="fa-solid fa-shield-halved"></i>Sicherheitsklassifizierung</p>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className={labelClass}>Freigabeebene</label>
                                ...
                                <div>
                                    <label className={labelClass}>Begrenzungsmarkierungen</label>
                                    <div className="space-y-1.5 max-h-28 overflow-y-auto pr-1 custom-scrollbar bg-black/20 border border-slate-700/50 rounded-lg p-2.5">
                                        ...
                                        )) : <span className="text-xs text-slate-600 italic">Keine Markierungen konfiguriert.</span>}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex gap-3 justify-end pt-3 border-t border-slate-700/30">
                            <button onClick={() => setIsEditing(false)} className="px-5 py-2.5 text-xs font-bold uppercase text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors">Abbrechen</button>
                            <button onClick={handleSave} disabled={saving || !editName.trim()}
                                ...
                                {saving ? <><i className="fa-solid fa-spinner animate-spin mr-2"></i>Sparen…</> : 'Save Changes'}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className={cardClass}>
                        <div className="px-5 py-3 bg-slate-800/40 border-b border-slate-700/30">
                            <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                                <i className="fa-solid fa-info-circle text-purple-400/70"></i>Betriebsdetails</p>
                        </div>
                        <div className="p-5 grid grid-cols-2 lg:grid-cols-3 gap-4">
                            <InfoItem label="Name" value={operation.name} />
                            <InfoItem label="Typ" value={operation.type} />
                            <InfoItem label="Status" value={operation.status} />
                            <InfoItem label="Host-Einheit" value={operation.unit?.name || 'None'} />
                            <InfoItem label="Standort" value={operation.locationText || operation.location?.name || 'None'} />
                            <InfoItem label="Maximale Teilnehmerzahl" value={operation.maxParticipants ? String(operation.maxParticipants) : 'Unlimited'} />
                            <InfoItem label="Geplanter Start" value={operation.scheduledStart ? formatOpDateTimeWithZone(operation.scheduledStart) : 'Not set'} />
                            <InfoItem label="Geplantes Ende" value={operation.scheduledEnd ? formatOpDateTimeWithZone(operation.scheduledEnd) : 'Not set'} />
                            <InfoItem label="Freigabeebene" value={securityClearances.find(c => c.level === (operation.clearanceLevel || 0))?.name || `Level ${operation.clearanceLevel || 0}`} />
                        </div>
                    </div>
                    ...
                    <div className={cardClass}>
                        <div className="px-5 py-3 bg-slate-800/40 border-b border-slate-700/30">
                            <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                                <i className="fa-solid fa-sliders text-purple-400/70"></i>Konfigurationsflags</p>
                        </div>
                        <div className="p-5 flex flex-wrap gap-3">
                            <FlagChip label="Klassifiziert" active={operation.isSpecial} color="red" icon="fa-solid fa-lock" />
                            <FlagChip label="Gemeinsame Operation" active={operation.isJoint} color="cyan" icon="fa-solid fa-handshake" />
                        ...
                        <div className={cardClass}>
                            <div className="px-5 py-3 bg-cyan-950/20 border-b border-cyan-500/10 flex items-center justify-between">
                                <p className="text-[10px] text-cyan-400/80 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                                    <i className="fa-solid fa-handshake"></i>Alliierte Organisationen</p>
                            </div>
                            <div className="p-5 space-y-3">
                                ...
                                {(operation.alliedOrgs || []).length === 0 && (
                                    <p className="text-xs text-slate-600 italic">Keine verbündeten Organisationen eingeladen.</p>
                                )}
                                <div className="flex gap-2 pt-2">
                                    <select value={invitePeerId} onChange={e => setInvitePeerId(e.target.value)}
                                        ...
                                        <option value="">Wähle einen Verbündeten…</option>
                                    ...
                                    <button onClick={handleInviteAlly} disabled={invitingAlly || !invitePeerId}
                                        ...
                                        {invitingAlly ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-plus"></i>Einladen</>}
                                    </button>
                                </div>
                            </div>
                        </div>
            ...
            {onAction && (
                <div className={cardClass}>
                    <div className="px-5 py-3 bg-slate-800/40 border-b border-slate-700/30">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                            <i className="fa-solid fa-rocket text-purple-400/70"></i>Missionskontrolle</p>
                    </div>
                    <div className="p-5 flex flex-wrap gap-3">
            ...
            {canManage && hasPermission('operations:create') && (
                <div className="bg-purple-950/10 rounded-xl border border-purple-500/15 overflow-hidden">
                    <div className="px-5 py-3 bg-purple-950/20 border-b border-purple-500/10">
                        <p className="text-[10px] text-purple-300/80 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                            <i className="fa-solid fa-clipboard-list"></i>Vorlagen</p>
                    </div>
                    <div className="p-5 flex items-center justify-between gap-4 flex-wrap">
                        <div className="max-w-xl">
                            ...
                            {sourceTemplate && (
                                <p className="text-[10px] text-slate-500 mt-1 flex items-center gap-1.5">
                                    <i className="fa-solid fa-bookmark text-purple-400/70"></i>Erstellt aus Vorlage:<span className="text-purple-300 font-bold">{sourceTemplate.name}</span>
                                </p>
                            )}
                        </div>
                        <button
                        ...
                        >
                            {savingTemplate
                                ? <><i className="fa-solid fa-spinner animate-spin"></i>Sparen…</>
                                : <><i className="fa-solid fa-bookmark"></i>Als Vorlage speichern</>}
                        </button>
                    </div>
                </div>
            )}
                ...
                <div className="bg-indigo-950/10 rounded-xl border border-indigo-500/15 overflow-hidden">
                    <div className="px-5 py-3 bg-indigo-950/20 border-b border-indigo-500/10">
                        <p className="text-[10px] text-indigo-300/80 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                            <i className="fa-brands fa-discord"></i>Discord Ankündigung</p>
                    </div>
                        ...
                        <div className="flex flex-wrap items-end gap-3">
                            <div className="flex-1 min-w-[260px]">
                                <div className="flex items-center justify-between gap-2 mb-1.5">
                                    <label className="text-[10px] text-slate-400 uppercase font-black tracking-widest">Kanal</label>
                                    <button
                                        type="button"
                            ...
                            >
                                {reposting
                                    ? <><i className="fa-solid fa-spinner animate-spin"></i>Posten…</>
                                    : operation.discordAnnouncementMessageId
                                        ? <><i className="fa-solid fa-rotate"></i>Repost-Ankündigung</>
                                        : <><i className="fa-brands fa-discord"></i>Auf Discord posten</>}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-red-950/10 rounded-xl border border-red-500/15 overflow-hidden">
                <div className="px-5 py-3 bg-red-950/20 border-b border-red-500/10">
                    <p className="text-[10px] text-red-400/80 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                        <i className="fa-solid fa-triangle-exclamation"></i>Gefahrenzone</p>
                </div>
                <div className="p-5 flex flex-wrap gap-3">
                    <button onClick={onDeleteOperation} disabled={isDeleting}
                        ...
                        {isDeleting ? (
                            <><i className="fa-solid fa-spinner animate-spin"></i>Vorgang wird gelöscht…</>
                        ) : (
                            <><i className="fa-solid fa-trash-can"></i>Vorgang löschen</>
                        )}
                    </button>
                </div>
            </div>
