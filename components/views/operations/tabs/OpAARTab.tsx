...
const categoryConfig: Record<string, { icon: string; color: string; label: string; bgClass: string }> = {
    observation: { icon: 'fa-solid fa-eye', color: 'text-sky-400', label: 'Beobachtungen', bgClass: 'bg-sky-500/10 border-sky-500/20' },
    ...
    action_item: { icon: 'fa-solid fa-clipboard-check', color: 'text-purple-400', label: 'Aktionselemente', bgClass: 'bg-purple-500/10 border-purple-500/20' },
};
    ...
    const handleGenerateDraft = async () => {
        ...
        try {
            ...
            addToast(
                ...
                { description: 'Prüfe und bearbeite den Entwurf vor dem Absenden.' }
            );
        } catch (err: any) {
            ...
            if (cleanMsg.includes('AAR_COOLDOWN_ACTIVE') || err?.code === 'AAR_COOLDOWN_ACTIVE') {
                ...
                addToast(
                    ...
                    { description: 'Entwürfe sind auf einen alle 3 Stunden pro Einsatz begrenzt. Versuche es später erneut.' }
                );
                return;
            }
        ...
        <div className="p-6 lg:p-8 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 p-4 bg-linear-to-r from-slate-800/60 to-slate-900/40 rounded-xl border border-slate-700/40">
                <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                    ...
                    {isSubmitted && <span className="text-[8px] bg-green-500/10 text-green-400 border border-green-500/20 px-1.5 py-0.5 rounded-sm ml-1">Eingereicht</span>}
                </p>
                ...
                {canContribute && !isSubmitted && (
                    <button onClick={() => setShowForm(!showForm)}
                        ...
                        <i className="fa-solid fa-plus"></i>Eintrag hinzufügen</button>
                )}
                {canManage && !isSubmitted && !editingSummary && (
                    <button onClick={() => { setSummaryText(operation.aarSummary || ''); setLessonsText(operation.aarLessonsLearned || ''); setEditingSummary(true); }}
                        ...
                        <i className="fa-solid fa-pen-to-square"></i>Zusammenfassung schreiben</button>
                )}
            </div>
                ...
                <div className="bg-green-950/15 rounded-xl border border-green-500/15 overflow-hidden">
                    <div className="px-5 py-3 bg-green-950/20 border-b border-green-500/10">
                        <p className="text-[10px] text-green-400/80 uppercase font-black tracking-[0.15em] flex items-center gap-2">
                            <i className="fa-solid fa-check-circle"></i>AAR eingereicht</p>
                    </div>
                    <div className="p-5 flex items-center justify-between gap-4 flex-wrap">
                        ...
                        {canReopen && (
                            ...
                            >
                                {reopening
                                    ? <><i className="fa-solid fa-spinner animate-spin"></i>Wiedereröffnung…</>
                                    : <><i className="fa-solid fa-rotate-left"></i>AAR erneut öffnen</>}
                            </button>
                        )}
                    </div>
                </div>
            ...
            {(editingSummary || operation.aarSummary || operation.aarLessonsLearned) && (
                <div className="bg-slate-900/60 rounded-xl border border-slate-700/30 overflow-hidden">
                    <div className="px-5 py-3 bg-slate-800/40 border-b border-slate-700/30">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em]">AAR-Zusammenfassung</p>
                    </div>
                    <div className="p-5 space-y-4">
                        {editingSummary ? (
                            <div className="space-y-4">
                                <div className="flex items-center justify-between gap-3 p-3 rounded-lg border border-purple-500/20 bg-purple-500/5">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[10px] uppercase font-black tracking-widest text-purple-300 flex items-center gap-2">
                                            <i className="fa-solid fa-wand-magic-sparkles"></i>AI Entwurf</p>
                                    ...
                                    >
                                        {generatingDraft
                                            ? <><i className="fa-solid fa-spinner animate-spin"></i>Abfassung…</>
                                            : onCooldown
                                                ? <><i className="fa-solid fa-hourglass-half"></i> {formatRemaining(cooldownRemaining)}</>
                                                : <><i className="fa-solid fa-wand-magic-sparkles"></i>Entwurf generieren</>}
                                    </button>
                                </div>
                                <div>
                                ...
                                <div>
                                    <label className={labelClass}>Gelernte Lektionen</label>
                                ...
                                <div className="flex gap-3 justify-end">
                                    <button onClick={() => setEditingSummary(false)} className="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors">Abbrechen</button>
                            ...
                            <>
                                {operation.aarSummary ? (
                                    ...
                                    <p className="text-xs text-slate-600 italic">Noch keine Zusammenfassung geschrieben.</p>
                                )}
                                {operation.aarLessonsLearned && (
                                    <div className="pt-3 border-t border-slate-700/30">
                                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mb-2">Gelernte Lektionen</p>
            ...
            {showForm && (
                <div className="bg-slate-900/60 rounded-xl border border-slate-700/30 overflow-hidden">
                    <div className="px-5 py-3 bg-slate-800/40 border-b border-slate-700/30">
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-[0.15em]">Neuer Eintrag</p>
                    </div>
                    <div className="p-5 space-y-4">
                        <div>
                        ...
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setShowForm(false)} className="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-lg hover:bg-slate-800 transition-colors">Abbrechen</button>
            ...
            {entries.length === 0 && !canContribute && (
                <div className="flex flex-col items-center justify-center py-16 text-slate-600">
                    ...
                    <p className="text-sm font-medium opacity-50">Noch keine AAR-Einträge eingereicht.</p>
                </div>
            )}
        </div>
    );
