            ...
            >
                ...
                {!compact && (
                    <div className="flex items-center gap-2 mt-0.5 ml-3">
                        ...
                        {isConcluded && <span className="text-[9px] text-slate-600 italic">Abgeschlossen</span>}
                    </div>
                )}
            </div>
        );
        ...
        <div className="flex flex-col h-full bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={goToToday} className="ml-1 px-3 py-1.5 rounded-lg bg-slate-900/60 border border-slate-700 text-slate-300 hover:text-white hover:border-purple-500/40 hover:bg-purple-500/10 transition-colors text-[10px] font-black uppercase tracking-wider">Heute</button>
                </div>
                            ...
                            return (
                                <div key={i} className={`flex flex-col rounded-xl border ${isToday(date) ? 'border-purple-500/50 bg-purple-500/5 shadow-lg shadow-purple-900/20' : 'border-slate-700/50 bg-slate-900/60'}`}>
                                    ...
                                    <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1.5">
                                        ...
                                        {dayOps.length === 0 && (
                                            <p className="text-[9px] text-slate-600 text-center italic mt-4">Keine OPs</p>
                                        )}
                                    </div>
                                </div>
                            );
                        ...
                        if (dayOps.length === 0) {
                            return (
                                <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/30 p-10 text-center">
                                    ...
                                    <h3 className="text-lg font-bold text-white mb-1">Keine Operationen geplant</h3>
                                    <p className="text-sm text-slate-500">Versuche es mit einem anderen Tag oder erstellen du einen neuen Vorgang.</p>
                                </div>
                            );
                        }
                        return (
                                            ...
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                        ...
                                                        {isOverdue && (
                                                            <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-sm bg-red-500/20 text-red-400 border border-red-500/30 animate-pulse">ÜBERFÄLLIG</span>
                                                        )}
                                                    </div>
                                                ...
                                                <div className="text-right shrink-0 ml-4 space-y-1">
                                                    ...
                                                    {isOverdue && hasPermission('operations:manage') && (
                                                        ...
                                                        >
                                                            <i className="fa-solid fa-rocket mr-1"></i>Start</button>
                                                    )}
                                                </div>
                                            </div>
                                            {op.owner && (
