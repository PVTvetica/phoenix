...
const ServiceRequestsView: React.FC<ServiceRequestsViewProps> = ({
    ...
    const tabs: { key: Tab; label: string; icon: string; badge?: number; urgent?: boolean }[] = isStaff ? [
        { key: 'pending', label: 'Ausstehend', icon: 'fa-inbox', badge: counts.submitted || undefined, urgent: counts.submitted > 0 },
        { key: 'unassigned', label: 'Nicht zugewiesen', icon: 'fa-user-clock', badge: counts.triaged || undefined, urgent: counts.triaged > 0 },
        { key: 'in_progress', label: 'In Bearbeitung', icon: 'fa-bolt', badge: counts.inProgress || undefined },
        { key: 'mine', label: 'Mir zugewiesen', icon: 'fa-briefcase', badge: counts.mineActive || undefined },
        { key: 'resolved', label: 'Erledigt', icon: 'fa-check-double' },
        { key: 'all', label: 'Alle', icon: 'fa-list-ul' },
    ] : [
        { key: 'all', label: 'Meine Wünsche', icon: 'fa-list-ul' },
    ];

    return (
        <div className="h-full flex flex-col overflow-hidden animate-fade-in">
            <HeroShell
                chipLabel="MODUL · SERVICEANFRAGEN"
                chipIcon="fa-file-invoice"
                chipAccent="sky"
                title="Serviceanfragen"
                subtitle="Missionskontrolle und Disposition. Live-Haftbefehl und Intel-Querverweis bei jeder Anfrage."
                ...
                stats={isStaff ? <>
                    <HeroStat icon="fa-inbox" label="Ausstehend" value={counts.submitted} accent="sky" emphasize={counts.submitted > 0} />
                    <HeroStat icon="fa-bolt" label="In Bearbeitung" value={counts.inProgress} accent="emerald" />
                    <HeroStat icon="fa-circle-check" label="Erledigt (7T)" value={counts.resolved7d} accent="emerald" />
                    <HeroStat icon="fa-briefcase" label="Meine aktiven" value={counts.mineActive} accent="amber" emphasize={counts.mineActive > 0} />
                </> : undefined}
                tabs={tabs.map(tab => (
                    <button
                        key={tab.key}
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                ...
                <div className="relative mb-4 max-w-2xl">
                    ...
                    <input
                        type="search"
                        placeholder="Anfragen, IDs oder Einsatzkräfte suchen…"
                        value={searchTerm}
                ...
                {filteredRequests.length > 0 ? (
                    ...
                    <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/30 mt-4">
                        <EmptyState
                            icon="fa-folder-open"
                            accent="sky"
                            heading="Keine passenden Anfragen"
                            description={searchTerm ? 'Probiere einen anderen Suchbegriff oder setze Filter zurück.' : 'Neue Anfragen erscheinen hier, sobald Clients sie einreichen.'}
                        />
                    </div>
                )}
            </div>
