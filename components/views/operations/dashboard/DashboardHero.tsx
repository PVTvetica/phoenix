                ...
                <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="min-w-0">
                        ...
                        <div className="mt-1 flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-slate-500">
                            {variant === 'client' ? (
                                <span>Rufstatus ·<span className="text-sky-300">{currentUser.reputation}</span></span>
                            ) : (
                                <span>
                                    {currentUser.rank?.name || 'Einsatzkraft'}
                                    <span className="text-slate-700 mx-1.5">·</span>
                                    {currentUser.unit?.name || 'Allgemeiner Pool'}
                                </span>
                            )}
                            {isSyncing && (
                                <span className="ml-2 inline-flex items-center gap-1.5 text-sky-400">
                                    <span className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse" />Synchronisierung</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                        ...
                        {variant === 'staff' && (
                            <button
                                ...
                                title={isDuty ? 'Vom Dienst gehen' : 'Zum Dienst antreten'}
                            >
                                <span className={`w-1.5 h-1.5 rounded-full ${isDuty ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600'}`} />
                                {toggling ? '…' : isDuty ? 'Im Dienst' : 'Nicht im Dienst'}
                            </button>
                        )}
                    </div>
                </div>
