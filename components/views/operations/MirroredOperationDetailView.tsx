...
const RSVP_BUTTONS = [
    ...
    { status: RSVPStatus.Declined, label: 'Abfall', icon: 'fa-circle-xmark', active: 'bg-red-500/20 text-red-400 border-red-500/40' },
] as const;
...
const MirroredOperationDetailView: React.FC<Props> = ({ mirror: initialMirror, onBack }) => {
    ...
    const handleRsvp = async (status: string) => {
        ...
        try {
            ...
            addToast("RSVP Aktualisiert", <i className="fa-solid fa-check"></i>, 'bg-green-500/10 text-green-400 border-green-500/50', { description: `You're marked "${status}" for this operation.` });
            await refresh();
        } catch {
            addToast("RSVP Fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: 'Host-Instanz war nicht erreichbar.' });
        } finally { setRsvping(false); }
    };

    const backBtn = (
        <button onClick={onBack} className="text-xs text-slate-400 hover:text-white flex items-center gap-2">
            <i className="fa-solid fa-arrow-left"></i>Zurück zum Betrieb</button>
    );
    ...
    return (
        <div className="p-6 lg:p-8 space-y-6">
            ...
            {(op.roe || op.commanderNotes) && (
                <Section title="Bestellungen" icon="fa-scroll">
                    {op.roe && <div className="mb-3"><p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Einsatzregeln</p><p className="text-sm text-slate-300 whitespace-pre-wrap">{op.roe}</p></div>}
                    {op.commanderNotes && <div><p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Notizen des Kommandanten</p><p className="text-sm text-slate-300 whitespace-pre-wrap">{op.commanderNotes}</p></div>}
                </Section>
            )}
            ...
            <Section title="Participants" icon="fa-users">
                <div className="space-y-1.5">
                    ...
                    {!(op.participants?.length || op.alliedParticipants?.length) && <p className="text-xs text-slate-600 italic">Noch keine Teilnehmer.</p>}
                </div>
            </Section>

            {/* Tactical board — read-only render of the synced snapshot. */}
            {!!op.boardElements?.length && (
                <Section title="Taktisches Brett" icon="fa-chess-board">
