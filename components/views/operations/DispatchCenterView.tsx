            ...
            <div className="flex items-center justify-between border-t border-white/5 pt-2">
                <div className="flex -space-x-1.5 overflow-hidden">
                    {request.assignedMembers.length > 0 ? request.assignedMembers.map(m => (
                        ...
                        <span className="text-[9px] text-slate-600 italic">Nicht zugewiesen</span>
                    )}
                </div>
        ...
        >
            ...
            <div className="text-right">
                <div className={`text-[10px] font-bold px-2 py-0.5 rounded-sm border ${liveChannel ? 'bg-sky-500/10 text-sky-400 border-sky-500/20' : 'bg-slate-700/20 text-slate-500 border-slate-700'}`}>
                    {liveChannel?.replace(/^radio-/, '').toUpperCase() || 'KEIN FUNK'}
                </div>
            </div>
        </div>
    );
...
const DispatchCenterView: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden animate-fade-in">
            <HeroShell
                chipLabel="MODUL · DISPATCH"
                chipIcon="fa-headset"
                chipAccent="cyan"
                title="Dispatch-Konsole"
                subtitle="Live-Warteschlange, Einheitenkoordination und Personensuche. CAD für den Raum, MDT für das Feld."
                titleBreakpoint="lg"
                actions={
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 lg:min-w-[380px] w-full sm:w-auto">
                        <form onSubmit={handleMdtSubmit} className="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder="MDT · RSI-Handle suchen…"
                ...
                stats={<>
                    <HeroStat icon="fa-inbox" label="Eingehend" value={stats.incoming} accent="red" emphasize={stats.incoming > 0} />
                    <HeroStat icon="fa-bolt" label="Aktiv" value={stats.active} accent="sky" />
                    <HeroStat icon="fa-users" label="Im Dienst" value={stats.onDuty} accent="emerald" />
                    <HeroStat icon="fa-clock" label="Überfällig" value={stats.overdue} accent="amber" emphasize={stats.overdue > 0} />
                </>}
                tabs={<>
                    <button
                        onClick={() => setTab('cad')}
                    ...
                    >
                        <i className="fa-solid fa-tower-broadcast"></i>CAD · Versand</button>
                    <button
                        onClick={() => setTab('mdt')}
                        className={`flex items-center gap-2 px-4 py-3 text-xs font-bold uppercase tracking-widest border-b-2 transition-colors whitespace-nowrap ${
            ...
            <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
                ...
                <div className="w-full lg:w-1/3 border-b lg:border-b-0 lg:border-r border-slate-800 flex flex-col h-1/3 lg:h-auto">
                    <div className="h-12 bg-slate-800/50 border-b border-white/5 flex items-center justify-between px-4 shrink-0">
                        <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wide">Aktive Vorfälle ({activeRequests.length})</h3>
                        <button onClick={() => setIsCreateModalOpen(true)} className="text-sky-400 hover:text-sky-300 transition-colors text-xs font-bold uppercase">
                            <i className="fa-solid fa-plus mr-1"></i>Neue Anfrage</button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3">
                        {activeRequests.length > 0 ? activeRequests.map(request => {
                            ...
                            <EmptyState
                                icon="fa-inbox"
                                accent="cyan"
                                heading="Keine aktiven Vorfälle"
                                description="Die Warteschlange ist leer — eingehende Anfragen erscheinen hier."
                                compact
                            />
                        )}
                    </div>
                ...
                <div className="w-full lg:w-1/3 border-b lg:border-b-0 lg:border-r border-slate-800 flex flex-col h-1/3 lg:h-auto">
                    <div className="h-12 bg-slate-800/50 border-b border-white/5 flex items-center px-4 shrink-0">
                        <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wide">Einheiten im Dienst ({onDutyUnits.length})</h3>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-3">
                        {onDutyUnits.length > 0 ? onDutyUnits.map(user => (
                            ...
                            <EmptyState
                                icon="fa-user-clock"
                                accent="emerald"
                                heading="Keine Einheiten im Dienst"
                                description="Personal erscheint hier, sobald es in den Dienst geht."
                                compact
                            />
                        )}
                    </div>
                ...
                <div className="w-full lg:w-1/3 flex flex-col h-1/3 lg:h-auto">
                    <div className="h-12 bg-slate-800/50 border-b border-white/5 flex items-center px-4 shrink-0 justify-between">
                        <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wide">Kommunikationsmatrix</h3>
                        <button
                        ...
                        {!radioConfig.configured && (
                            <div className="bg-amber-500/5 border border-amber-500/20 rounded-lg p-3 flex items-start gap-2.5">
                                ...
                                <p className="text-[10px] text-slate-500">
                                    <strong className="text-amber-400">LiveKit nicht konfiguriert.</strong> Live-Funkdaten sind nicht verfügbar. Die Kanalbelegung unten bleibt daher leer.
                                </p>
                            </div>
                        )}
                            ...
                            return (
                                ...
                                >
                                    <div className="px-3 py-2 bg-slate-800/80 border-b border-white/5 flex justify-between items-center">
                                        ...
                                        <span className="text-[10px] font-mono text-slate-500">{occupants.length} Nutzer</span>
                                    </div>
                                    <div className="p-2 min-h-[40px] transition-colors group-hover:bg-white/5">
                                        <div className="flex flex-wrap gap-2">
                                            ...
                                            {occupants.length === 0 && (
                                                <span className="text-[9px] text-slate-600 italic px-2">Kanal leer</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
