...
const PHASE_STEPS = [
    ...
    { key: OperationStatus.Scheduled, label: 'Geplant', icon: 'fa-solid fa-clock', color: 'amber' },
    { key: OperationStatus.Active, label: 'Aktiv', icon: 'fa-solid fa-bolt', color: 'green' },
    { key: OperationStatus.Concluded, label: 'Abgeschlossen', icon: 'fa-solid fa-flag-checkered', color: 'slate' },
] as const;
        ...
        if (diff <= 0) {
            return (
                <div className="flex items-center gap-2 animate-pulse">
                    ...
                    <span className="text-red-400 font-mono text-sm font-bold">ÜBERFÄLLIG</span>
                </div>
            );
        }
...
const OperationDetailView: React.FC<OperationDetailViewProps> = ({ operation: initialOperation, onBack }) => {
    ...
    const handleAction = async (action: () =>Promise<void>, actionName: string) => {
        ...
        try {
            ...
            if (actionName !== 'join' && error.message) addToast("Vorgang fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: error.message });
        } finally {
    ...
    const handleDeleteOperation = async () => {
        ...
        if (confirmed) {
            ...
            try {
                ...
                addToast("Löschen fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Löschen des Einsatzes ist ein Fehler aufgetreten." });
                setIsDeleting(false);
            }
        }
    };
                        ...
                        <div>
                            <h1 className="text-lg font-black text-white tracking-tight uppercase flex items-center gap-2">
                                ...
                                <span className="bg-red-500/10 text-red-400 border border-red-500/30 text-[10px] font-black px-2 py-0.5 rounded-sm uppercase tracking-widest flex items-center gap-1">
                                    <i className="fa-solid fa-lock text-[8px]"></i>Klassifiziert</span>
                            </h1>
                        </div>
                    </div>
                </div>
                <div className="flex-1 flex flex-col items-center justify-center p-6 relative overflow-hidden">
                    ...
                    <div className="max-w-md w-full bg-slate-900/80 border border-red-500/20 p-8 rounded-2xl shadow-2xl relative backdrop-blur-md animate-scale-in">
                        ...
                        <div className="text-center mb-8">
                            ...
                            <h2 className="text-2xl font-black text-white uppercase tracking-widest mb-2">Eingeschränkter Zugriff</h2>
                            <p className="text-red-400/80 text-sm font-mono">Autorisierungscode erforderlich.</p>
                        </div>
                        <form onSubmit={(e) => { e.preventDefault(); handleAction(() => joinOperationWithShip(operation.id, { joinCode: joinCodeRef.current }), 'join'); }} className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase mb-2 tracking-wider text-center">Autorisierungscode (PIN)</label>
                            ...
                            <button type="submit" disabled={!!loadingAction}
                                ...
                                {loadingAction === 'join' ? (<><i className="fa-solid fa-circle-notch animate-spin mr-2"></i>Überprüfung…</>) : (<><i className="fa-solid fa-unlock-keyhole mr-2"></i>Authentifizieren und beitreten</>)}
                            </button>
                        </form>
                    </div>
                </div>
    ...
    const navGroups = [
        ...(isActive ? [{
            ...
            items: [
                ...
                { id: 'live-overview' as OpSection, label: 'Überblick', icon: 'fa-solid fa-users-rectangle' },
                ...(canManage ? [{ id: 'live-command' as OpSection, label: 'Befehl', icon: 'fa-solid fa-tower-broadcast' }] : []),
            ],
        }] : []),
        {
            title: 'Operations',
                                ...
                                <div className="flex items-center gap-2 flex-wrap mt-2">
                                    ...
                                    {operation.isJoint && (
                                        <span className="bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 text-[10px] font-black px-2.5 py-0.5 rounded-sm uppercase tracking-wider flex items-center gap-1.5">
                                            <i className="fa-solid fa-handshake text-[8px]"></i>Gemeinsam</span>
                                    )}
                    ...
                    <div className="flex items-center gap-2">
                        {radioConfig.configured ? (
                            ...
                            <span className="px-3 py-1.5 text-slate-600 text-[10px] font-black uppercase tracking-widest">
                                <i className="fa-solid fa-lock text-[9px] mr-1.5"></i>Admin Einrichtung erforderlich</span>
                        )}
                        <button
                                ...
                                >
                                    {opRadio.isTransmitting ? (
                                        <span className="flex items-center justify-center gap-3">
                                            <span className="w-2.5 h-2.5 rounded-full bg-black/40 animate-ping" />LIVE-TX</span>
                                    ) : (
