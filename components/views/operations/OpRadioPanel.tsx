    ...
    return (
        <div className={`bg-slate-950/70 backdrop-blur-md border border-amber-500/20 rounded-xl shadow-[0_8px_24px_rgba(0,0,0,0.4)] ${compact ? 'p-2.5 space-y-2.5' : 'p-3.5 space-y-3'}`}>
                ...
                <div className="flex items-center gap-2">
                    {isConnected ? (
                        <span className="flex items-center gap-1.5 text-[10px] font-bold text-green-300 uppercase tracking-widest">
                            <span className="w-1.5 h-1.5 rounded-full bg-green-400 shadow-[0_0_6px_rgba(34,197,94,0.7)] animate-pulse" />Verlinkt</span>
                    ) : isConnecting ? (
                        <span className="text-[10px] font-bold text-amber-300 uppercase tracking-widest animate-pulse">Verlinkung…</span>
                    ) : (
            ...
            {!isConnected && !isConnecting && (
                ...
                >
                    <i className="fa-solid fa-rotate-right mr-1" />Wieder verbinden</button>
            )}
        </div>
    );
};
