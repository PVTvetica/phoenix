...
const DutyRosterView: React.FC = () => {
    ...
    const viewModeTabs: Array<{ key: 'on-duty' | 'off-duty' | 'all'; label: string; icon: string; count: number }> = [
        { key: 'on-duty', label: 'Im Dienst', icon: 'fa-bolt', count: onDutyCount },
        { key: 'off-duty', label: 'Außer Dienst', icon: 'fa-moon', count: members.length - onDutyCount },
        { key: 'all', label: 'Alle', icon: 'fa-users', count: members.length },
    ];
                ...
                <div className="@container/roster bg-slate-900/60 backdrop-blur-md rounded-xl border border-slate-700/50 overflow-hidden flex-1 min-h-0 flex flex-col relative">
                    {rosterMode === 'flat' ? (
                        ...
                        <div className="flex bg-white/5 border-b border-white/5 px-4 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest shrink-0">
                            <div className="flex-1">Identität</div>
                            <div className="w-48 hidden @3xl/roster:block">Rang</div>
                    ...
                    {isLoading ? (
                        <div className="absolute inset-0 flex items-center justify-center z-10 bg-slate-900/60 backdrop-blur-xs">
                            <div className="flex flex-col items-center">
                                ...
                                <span className="text-[10px] font-black text-emerald-300 uppercase tracking-widest">Dienstplan wird geladen…</span>
                            </div>
                        </div>
                    ) : rosterMode === 'flat' ? (
                            ...
                            {flatRoster.length > 0 ? (
                                ...
                                <div className="absolute inset-0 flex items-center justify-center p-6">
                                    <EmptyState
                                        icon="fa-user-slash"
                                        accent="emerald"
                                        heading="Kein Personal gefunden"
                                        description="Probiere einen anderen Suchbegriff oder setze Filter zurück."
                                        compact
                                    />
                                </div>
                            )}
...
const FlatColumnHeaders: React.FC<{
    ...
    <div className="flex items-center bg-white/5 border-b border-white/5 px-4 py-3 shrink-0">
        ...
        <div className="w-44 hidden @3xl/roster:block">
            <SortableColumnHeader label="Rang" sortKey="rank" activeKey={sortKey} sortDir={sortDir} onSort={onSort} accent="emerald" showInactiveIndicator />
        </div>
        ...
        <div className="w-44 hidden @5xl/roster:block">
            <SortableColumnHeader label="Einheit" sortKey="unit" activeKey={sortKey} sortDir={sortDir} onSort={onSort} accent="emerald" showInactiveIndicator />
        </div>
        ...
        <div className="w-24 text-right text-[10px] font-black text-slate-500 uppercase tracking-widest">Aktionen</div>
    </div>
);
...
const FlatMemberRow: React.FC<{
    ...
    <div className="flex items-center px-4 h-full hover:bg-slate-800/50 transition-colors border-b border-slate-700/30 group">
        ...
        <div className="w-44 hidden @5xl/roster:flex items-center text-xs text-slate-400 truncate">
            {member.unit?.name || <span className="text-slate-600 italic">Nicht zugewiesen</span>}
        </div>
