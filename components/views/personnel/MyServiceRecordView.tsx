...
const TIMELINE_ACCENTS: Record<RecordType, { border: string; bg: string; text: string; icon: string; label: string }> = {
    REQUEST: { border: 'border-sky-500/30', bg: 'bg-sky-500/10', text: 'text-sky-300', icon: 'fa-clipboard-list', label: 'Serviceanfrage' },
    OPERATION: { border: 'border-purple-500/30', bg: 'bg-purple-500/10', text: 'text-purple-300', icon: 'fa-bullseye', label: 'Betrieb' },
...
const FILTERS: Array<{ key: FilterKey; label: string }> = [
    { key: 'all', label: 'Alle' },
    { key: 'op', label: 'Operationen' },
    { key: 'req', label: 'Anfragen' },
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden animate-fade-in">
            <HeroShell
                ...
                statsCols={4}
                stats={<>
                    <HeroStat icon="fa-star" label="Ruf" value={user.reputation ?? 0} accent="amber" emphasize={(user.reputation ?? 0) > 0} />
                    ...
                    <HeroStat icon="fa-certificate" label="Zertifizierungen" value={isHydrating ? '—' : certCount} accent="emerald" emphasize={!isHydrating && certCount > 0} />
                    <HeroStat icon="fa-medal" label="Belobigungen" value={isHydrating ? '—' : commendCount} accent="amber" emphasize={!isHydrating && commendCount > 0} />
                </>}
            />
            ...
            <div className={`flex-1 min-h-0 overflow-y-auto custom-scrollbar p-4 sm:p-6 ${activeTab === 'dossier' ? 'lg:overflow-hidden' : ''}`}>
                {activeTab === 'dossier' ? (
                                    ...
                                    <div className="space-y-4 animate-fade-in">
                                        <div className="flex items-center justify-between gap-3">
                                            <div className="min-w-0">
                                                <p className="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-black mb-1">Einstufung</p>
                                        ...
                                        <div>
                                            <p className="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-black mb-2">Begrenzungsmarkierungen</p>
                                            {(user.limitingMarkers && user.limitingMarkers.length > 0) ? (
                                                ...
                                                <p className="text-xs text-slate-500 italic">Es wurden keine einschränkenden Markierungen angewendet.</p>
                                            )}
                                        </div>
                                    </div>
                                ) : (
                                    <p className="text-xs text-slate-500 italic">Keine Freigabe zugewiesen.</p>
                                )}
                            </SectionCard>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <SectionCard title="Spezialisierungen" icon="fa-tags" accent="sky">
                            {isHydrating ? (
                                <div className="flex flex-wrap gap-2">
                            ...
                            ) : <p className="text-slate-500 italic text-sm">Keine Spezialisierungen festgelegt.</p>}
                        </SectionCard>

                        <SectionCard title="Zertifizierungen" icon="fa-certificate" accent="emerald">
                            {isHydrating ? (
                        ...
                        <SectionCard title="Belobigungen" icon="fa-medal" accent="amber">
                            {isHydrating ? (
                        ...
                        {canSeeConduct && (
                            <SectionCard title="Verhaltensprotokoll" icon="fa-gavel" accent="rose">
                                <p className="text-[10px] text-slate-500 italic flex items-center gap-1.5 -mt-1">
                                    <i className="fa-solid fa-lock text-slate-600" aria-hidden />Nur für du und Administratoren sichtbar.</p>
                                {isHydrating ? (
                                    <div className="space-y-2">
                                        <Skeleton className="h-20" />
                                    </div>
                                    ...
                                    <p className="text-slate-500 italic text-sm">Es liegen keine Verhaltenseinträge vor.</p>
                                )}
                            </SectionCard>
                        )}
                    </div>
                        ...
                        <SectionCard title="Position History" icon="fa-briefcase" accent="sky">
                            {positionHistory === null ? (
                                ...
                                <EmptyState
                                    icon="fa-briefcase"
                                    accent="sky"
                                    heading="Keine Positionszuweisungen hinterlegt"
                                    description="HR- und Regierungspositionen erscheinen hier nach und nach. Sprich mit deinem Command, wenn du hier Einträge erwartest."
                                    compact
                                />
                            ) : (() => {
                                    ...
                                    <>
                                        <div className="grid grid-cols-3 gap-3 mb-5">
                                            <div className="bg-slate-950/40 border border-slate-800 rounded-lg p-3 text-center">
                                                <div className="text-[10px] font-black uppercase tracking-widest text-slate-500">Aktuell</div>
                                            ...
                                            <div className="bg-slate-950/40 border border-slate-800 rounded-lg p-3 text-center">
                                                <div className="text-[10px] font-black uppercase tracking-widest text-slate-500">Regierung</div>
                                                        ...
                                                        <div className="bg-slate-950/40 border border-slate-800 p-3 rounded-lg">
                                                            <div className="flex items-start justify-between gap-3 mb-1">
                                                                <div className="flex items-center gap-2 flex-wrap min-w-0">
                                                                    ...
                                                                    {!entry.endedAt && (
                                                                        <span className="text-[10px] px-1.5 py-0.5 rounded-sm uppercase font-black tracking-widest bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">Aktuell</span>
                                                                    )}
                                                                </div>
