...
const ProfileView: React.FC = () => {
    ...
    const savePreferences = async () => {
        ...
        if (!tzChanged && !fmtChanged) {
            addToast("Keine Änderungen zum Speichern", <i className="fa-solid fa-circle-info"></i>, 'bg-slate-500/10 text-slate-300 border-slate-500/40');
            return;
        }
        ...
        try {
            ...
            addToast("Zeiteinstellungen gespeichert", <i className="fa-solid fa-check"></i>, 'bg-green-500/10 text-green-400 border-green-500/50');
        } catch (err: any) {
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message || 'Could not save preferences.' });
        } finally {
    ...
    const confirmSave = async () => {
        ...
        try {
            ...
            console.error(err);
            addToast("Aktualisierung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Dein RSI-Handle konnte nicht aktualisiert werden." });
            setRsiHandle(currentUser.rsiHandle);
            setIsEditing(false);
            setIsSaving(false);
        }
    ...
    const handleSyncRoles = async () => {
        ...
        try {
            ...
            if (typeof result === 'string' && result.startsWith('SYNC_COOLDOWN:')) {
                ...
                addToast("Abklingzeit synchronisieren", <i className="fa-solid fa-clock"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: `Try again in ${remainingMin} minute${remainingMin !== 1 ? 's' : ''}.` });
                return;
            }
            ...
            addToast("Synchronisierung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Synchronisieren deiner Rollen ist ein Fehler aufgetreten. Bitte versuche es erneut." });
        } finally {
            setIsSyncing(false);
        }
    };
    ...
    const runDiagnostics = async () => {
        ...
        try {
            ...
            addToast("Gerät registriert", <i className="fa-solid fa-circle-check"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: "Push-Benachrichtigungen sind auf diesem Gerät aktiv." });
    ...
    const handleTestSignal = async () => {
        addToast("Testsignal", <i className="fa-solid fa-satellite-dish"></i>, "bg-sky-500/10 text-sky-400 border-sky-500/50", { description: "Testsignal vom Command angefordert…" });
        try {
            await rpcAction('user:test_push', { userId: currentUser.id });
        } catch (e: any) {
            addToast("Signal fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: `${e.message || 'Unknown network error'}. Ensure you have 'Registered' this specific browser first.` });
        }
    };
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 max-w-5xl mx-auto w-full">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden">
                        <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex items-center gap-3">
                            ...
                            <h3 className="font-bold text-white text-sm uppercase tracking-wider">Kontoeinstellungen</h3>
                        </div>
                        ...
                        <div className="p-5 space-y-6">
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    {!isEditing && (
                                        <button onClick={() => setIsEditing(true)} className="text-sky-300 hover:text-sky-200 text-[10px] font-black uppercase tracking-widest transition-colors">Ändern</button>
                                    )}
                                </div>
                                {isEditing ? (
                                    <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-700 space-y-3 animate-fade-in">
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={initiateSave} disabled={isSaving} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-white bg-sky-600 hover:bg-sky-500 border border-sky-500/40 rounded-lg shadow-lg shadow-sky-900/30 transition disabled:opacity-50">
                                                {isSaving ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-check"></i>Überprüfen und speichern</>}
                                            </button>
                                            <button onClick={() => { setIsEditing(false); setRsiHandle(currentUser.rsiHandle); }} className="flex-1 px-4 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-300 bg-slate-900/60 border border-slate-700 rounded-lg hover:bg-slate-800 transition-colors">Abbrechen</button>
                                        </div>
                                        <p className="text-[10px] text-amber-400 leading-tight">
                                            <i className="fa-solid fa-triangle-exclamation mr-1"></i>Wenn du Deinen Benutzernamen ändern, ist eine erneute Überprüfung erforderlich.</p>
                                    </div>
                                ) : (
                            ...
                            <div className="pt-4 border-t border-slate-800">
                                <div className="flex justify-between items-center mb-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Anzeigename</label>
                                ...
                                {isEditingDisplayName ? (
                                    <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-700 space-y-3 animate-fade-in">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                {isSavingDisplayName ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-check"></i>Speichern</>}
                                            </button>
                                    ...
                                    <div className="bg-slate-950/40 p-3 rounded-lg border border-slate-700/50 flex items-center justify-between gap-3">
                                        <div className="min-w-0">
                                            ...
                                            {currentUser.displayName ? (
                                                <p className="text-[10px] text-slate-500 mt-0.5">Benutzerdefinierte Überschreibung · Discord:<span className="font-mono text-slate-400">{currentUser.discordName || '—'}</span>
                                                </p>
                                            ) : (
                                                <p className="text-[10px] text-slate-500 mt-0.5">Legen du unter Discord einen benutzerdefinierten Namen fest, der überschrieben werden soll</p>
                                            )}
                                        </div>
                                    </div>
                                )}
                            ...
                            {/* Discord Sync */}
                            <div className="pt-4 border-t border-slate-800">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Rangsynchronisierung</label>
                    ...
                    <div className="bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden">
                        <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex items-center justify-between gap-3">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="font-bold text-white text-sm uppercase tracking-wider">Kommunikations-Uplink</h3>
                            </div>
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    <i className="fa-solid fa-satellite-dish"></i>Testsignal</button>
                            </div>
                ...
                <div className="bg-slate-900/60 backdrop-blur-md border border-slate-700/50 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-white/5 bg-white/5 flex items-center gap-3">
                        ...
                        <h3 className="font-bold text-white text-sm uppercase tracking-wider">Zeitpräferenzen</h3>
                    </div>
                    <div className="p-5 space-y-6">
                        <p className="text-xs text-slate-400 leading-relaxed bg-slate-950/40 p-3 rounded-lg border border-slate-700/50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-2">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Zeitzone</label>
                                    {tzDraft !== browserTz && (
                                        <button onClick={resetTzToBrowser} className="text-sky-300 hover:text-sky-200 text-[10px] font-black uppercase tracking-widest transition-colors">Vom Browser erkennen</button>
                                    )}
                                </div>
                                <select
                                    value={tzDraft}
                                ...
                                <p className="text-[10px] text-slate-500 mt-2 leading-tight">Browser erkannt:<span className="font-mono text-slate-400">{browserTz}</span>
                                </p>
                            </div>

                            {/* Date format */}
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Datumsformat</label>
                        ...
                        <div className="flex justify-end pt-2 border-t border-slate-800">
                            ...
                            >
                                {isSavingPrefs ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-check"></i>Einstellungen speichern</>}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-red-950/10 border border-red-500/20 rounded-xl overflow-hidden">
                    <div className="px-5 py-4 border-b border-red-500/10 bg-red-500/5 flex items-center gap-3">
                        ...
                        <h3 className="font-black text-red-300 text-sm uppercase tracking-widest">Gefahrenzone</h3>
                    </div>
                    <div className="p-5">
                        <button
                        ...
                        >
                            <i className="fa-solid fa-trash"></i>Konto löschen</button>
                        {currentUser.role === 'Admin' && <p className="text-[10px] text-red-400/60 text-center mt-2 italic">Admins können ihr eigenes Konto nicht löschen.</p>}
                    </div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center z-100 animate-fade-in p-4">
                    <div className="bg-slate-900/90 backdrop-blur-md border border-amber-500/30 rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
                        <div className="bg-amber-950/30 p-5 border-b border-amber-500/20 flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-base font-black text-white uppercase tracking-tight">Bestätige die Handle-Änderung</h3>
                                <p className="text-amber-300/80 text-[10px] font-mono uppercase tracking-widest mt-0.5">Sicherheitsprotokoll</p>
                            </div>
                        </div>
                        <div className="p-5 space-y-4">
                            <p className="text-slate-300 text-sm">Handle wirklich ändern zu<strong className="text-white font-mono">{rsiHandle}</strong>?
                            </p>
                        ...
                        <div className="p-4 bg-slate-950/60 border-t border-white/5 flex justify-end gap-3 rounded-b-xl">
                            ...
                            >
                                <i className="fa-solid fa-check"></i>Bestätigen und verifizieren</button>
                        </div>
                    </div>
                </div>
            )}
