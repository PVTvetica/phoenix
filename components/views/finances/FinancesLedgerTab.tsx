...
export default function FinancesLedgerTab({
    ...
    const handleExport = async () => {
        try {
            ...
            addToast("Exportbereit", <i className="fa-solid fa-check" />, 'bg-emerald-500/10 text-emerald-400 border-emerald-500/50');
        } catch (err: any) {
            addToast("Der Export ist fehlgeschlagen", <i className="fa-solid fa-xmark" />, 'bg-red-500/10 text-red-400 border-red-500/50', {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Rows */}
            {filtered.length === 0 ? (
                <div className="rounded-xl border border-white/5 bg-slate-900/30 p-10 text-center text-slate-500 text-sm">Keine Einträge entsprechen den aktuellen Filtern.</div>
            ) : (
