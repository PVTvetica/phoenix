        ...
        >
            ...
            <div className="w-48 pl-2 flex flex-col justify-center border-r border-slate-700/50 pr-4">
                ...
                <div className="flex items-center gap-2 mt-1">
                    ...
                    {isRequest && <span className="text-[9px] bg-red-900/30 text-red-500 border border-red-500/20 px-1 rounded-sm uppercase font-black">Aktive Op</span>}
                </div>
            </div>
                            ...
                            >
                                ...
                                {canManage && transferMenuOpen === identity && (
                                    <div className="absolute top-full left-0 mt-1 z-50 bg-slate-800 border border-slate-600 rounded-lg shadow-xl w-48 max-h-48 overflow-y-auto custom-scrollbar py-1">
                                        <p className="px-3 py-1 text-[9px] text-slate-500 uppercase font-black tracking-wider">Übertragen an</p>
                                        ...
                                        {transferTargets.length === 0 && (
                                            <p className="px-3 py-2 text-[10px] text-slate-600 italic">Keine anderen Kanäle</p>
                                        )}
                                    </div>
                                )}
                            </div>
...
const RadioControlView: React.FC = () => {
    ...
    const handleTuneIn = (channel: RadioChannel) => {
        setChannel(channel);
        addToast("Kanal abgestimmt", <i className="fa-solid fa-headphones"></i>, 'bg-sky-500/10 text-sky-400 border-sky-500/50', { description: `Tuning into ${channel.name}.` });
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            addToast("Häufigkeit gelöscht", <i className="fa-solid fa-trash"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: "Die Funkfrequenz wurde entfernt." });
        } catch {
            addToast("Löschen fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Funkkanal konnte nicht gelöscht werden." });
        }
    };
    ...
    const handleReboot = async () => {
        ...
        try {
            ...
            addToast("Netzwerk neu gestartet", <i className="fa-solid fa-power-off"></i>, 'bg-green-500/10 text-green-400 border-green-500/50', { description: `${result.count} rooms cleared.` });
            setTimeout(fetchStatus, 1000);
        } catch (e: any) {
            console.error(e);
            addToast("Neustart fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: e.message || "Failed to reboot the network." });
        } finally {
    ...
    const handleSignalUser = async (identity: string, channel: RadioChannel) => {
        try {
            ...
            addToast("Ping gesendet", <i className="fa-solid fa-signal"></i>, 'bg-amber-500/10 text-amber-400 border-amber-500/50', { description: 'Funk-Ping wurde gesendet.' });
        } catch (error) {
            console.error("Failed to send ping:", error);
            addToast("Ping fehlgeschlagen", <i className="fa-solid fa-triangle-exclamation"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: "Funk-Ping konnte nicht gesendet werden." });
        }
    };
    ...
    const handleTransferUser = useCallback(async (identity: string, currentRoom: string, targetChannel: RadioChannel) => {
        ...
        try {
            ...
            addToast("Übertragung fehlgeschlagen", <i className="fa-solid fa-circle-exclamation"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: "Operator konnte nicht verschoben werden." });
        }
    ...
    const handleBroadcastAlert = async () => {
        ...
        try {
            ...
            console.error(e);
            addToast("Übertragung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Rundfunkalarm konnte nicht gesendet werden." });
        } finally {
    ...
    const handleDrop = async (e: React.DragEvent, targetChannel: RadioChannel) => {
        ...
        if (draggedUser && draggedUser.currentRoom !== targetRoomName) {
            addToast(
                ...
                <i className="fa-solid fa-arrow-right-arrow-left"></i>,
                'bg-green-500/10 text-green-400 border-green-500/50',
                { description: 'Operator wird in den Zielkanal verschoben…' }
            );
            ...
            try {
                ...
                addToast("Übertragung fehlgeschlagen", <i className="fa-solid fa-circle-exclamation"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: "Operator konnte nicht in den Zielkanal verschoben werden." });
            }
        }
        setDraggedUser(null);
    };
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden animate-fade-in">
            <HeroShell
                ...
                subtitle="Spectrum-Überwachung und Disposition. Kanäle hören, Operatoren transferieren, Alarme senden."
                actions={<>
                    <HeroActionButton onClick={() => setIsBroadcastModalOpen(true)} accent="amber" icon="fa-bullhorn">Broadcast-Alarm</HeroActionButton>
                    <HeroActionButton onClick={openIssueEamModal} accent="red" icon="fa-radiation">Notfall senden</HeroActionButton>
                    {canManage && (
                        <>
                            <HeroActionButton onClick={handleCreate} accent="amber" icon="fa-plus">Neue Freq</HeroActionButton>
                            <HeroActionButton
                            ...
                            >Starten du Net neu</HeroActionButton>
                        </>
                    )}
                </>}
                stats={<>
                    <HeroStat icon="fa-tower-broadcast" label="Channels" value={allChannels.length} accent="amber" />
                    <HeroStat icon="fa-bolt" label="Aktiv" value={activeChannelsCount} accent="emerald" emphasize={activeChannelsCount > 0} />
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
                ...
                {!radioConfig.configured && (
                    <div className="bg-amber-500/5 border border-amber-500/30 rounded-xl p-3 flex items-center gap-3">
                        ...
                        <p className="text-xs text-slate-400">
                            <strong className="text-amber-300">LiveKit Nicht konfiguriert</strong> — Live comms data is unavailable. Set the LIVEKIT_* credentials in your server's .env file to enable real-time radio monitoring.
                        </p>
                    </div>
                )}
                ...
                <div className="flex gap-2">
                    <div className="relative flex-1 max-w-2xl">
                        ...
                        />
                        <button onClick={fetchStatus} disabled={isRefreshing} className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 text-slate-500 hover:text-amber-300 transition-colors" title="Aktualisieren">
                ...
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center h-64 opacity-60">
                        ...
                        <p className="text-[10px] uppercase tracking-widest font-black text-amber-300">Spektrum scannen…</p>
                    </div>
                ) : (
                    <div>
                        ...
                        {allChannels.length === 0 && !hideEmpty && (
                            <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/30">
                                <EmptyState
                                    ...
                                    accent="amber"
                                    heading="Keine Frequenzen gefunden"
                    ...
                    <div className="bg-slate-900 border border-amber-500 rounded-xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden">
                        <div className="bg-amber-900/20 p-5 border-b border-amber-500/30 flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-white uppercase tracking-tight">Broadcast-Alarm</h2>
                                    <p className="text-[10px] text-amber-400 font-mono uppercase tracking-wide">Alle Kanäle</p>
                                </div>
                            </div>
                        ...
                        <div className="p-6">
                            ...
                            <label className="block text-[10px] text-slate-500 uppercase font-black mb-2 tracking-widest">Nachricht</label>
                            <textarea
                        ...
                        <div className="p-4 bg-slate-950/60 border-t border-white/5 flex justify-end gap-3 rounded-b-xl">
                            <button onClick={() => setIsBroadcastModalOpen(false)} className="px-4 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white transition-colors">Abbrechen</button>
                            <button
