            ...
            <aside className="hidden md:flex flex-col w-72 shrink-0 bg-slate-900/60 border-r border-white/5 px-6 py-8">
                <div className="flex items-center gap-3 mb-10">
                    ...
                    <div>
                        ...
                        <div className="text-[11px] text-slate-500">Erstmaliges Setup</div>
                    </div>
                </div>
                ...
                <div className="mt-auto pt-8 text-[11px] text-slate-600">Selbstgehostete Star Citizen-Organisationsoperationen.</div>
            </aside>
...
function WelcomeStep({ onNext }: { onNext: () => void }) {
    return (
        <Shell icon="fa-rocket" title="Willkommen bei Open MyRSI.org">
            ...
            <PrimaryBtn onClick={onNext}>Lass uns gehen<i className="fa-solid fa-arrow-right ml-1" /></PrimaryBtn>
        </Shell>
    );
}
...
function PreflightStep({ onNext }: { onNext: () => void }) {
    ...
    return (
        <Shell icon="fa-clipboard-check" title="Preflight-Check" subtitle="Ein kurzer Blick auf deine Umgebung. Wir zeigen niemals Werte — nur, ob jedes Element konfiguriert ist.">
            <div className="space-y-2 mb-6">
                {loading && <p className="text-slate-500 text-sm"><i className="fa-solid fa-spinner fa-spin mr-2" />Überprüfung…</p>}
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="ml-auto flex items-center gap-2">
                                    {!ok && c.requiresRestart && (
                                        <span className="text-[10px] uppercase tracking-widest text-amber-500/80"><i className="fa-solid fa-rotate-right mr-1" />Neustart erforderlich</span>
                                    )}
            ...
            {!loading && status && !criticalOk && (
                <p className="text-rose-400/90 text-sm mb-4"><i className="fa-solid fa-circle-exclamation mr-2" />Löse die oben genannten erforderlichen Punkte, um fortzufahren.</p>
            )}
            ...
            <div className="flex items-center gap-3">
                <PrimaryBtn onClick={onNext} disabled={!criticalOk}>Weitermachen<i className="fa-solid fa-arrow-right ml-1" /></PrimaryBtn>
                <button onClick={check} className="px-4 py-3 text-slate-400 hover:text-white text-sm font-medium"><i className="fa-solid fa-rotate mr-2" />Überprüfen du es noch einmal</button>
            </div>
        </Shell>
    );
}
...
function DiscordStep({ onSignIn }: { onSignIn: () => void }) {
    return (
        <Shell icon="fa-discord" title="Konto erstellen" subtitle="Open MyRSI.org nutzt Discord zum Anmelden. Du wirst das erste Mitglied und Admin der Organisation.">
            <button onClick={onSignIn}
                ...
                <i className="fa-brands fa-discord text-xl" />Mit Discord anmelden</button>
...
function ClaimStep({ redeem, pendingName }: { redeem: (code: string) =>Promise<string>; pendingName?: string }) {
    ...
    return (
        <Shell icon="fa-key" title={pendingName ? `Welcome, ${pendingName}` : 'Claim the Admin seat'}
            subtitle="Dein Konto ist erstellt. Sichere den Admin-Platz mit dem Einmalcode aus der Server-Konsole beim ersten Start.">
            <div className="rounded-lg border border-white/5 bg-slate-900/50 p-4 mb-5 text-xs text-slate-400">
                <i className="fa-solid fa-terminal mr-2 text-slate-500" />Suchen du nach dem<span className="text-sky-300 font-mono">OPEN MYRSI.ORG</span> banner in your server logs — your code looks like <span className="font-mono text-slate-300">SETUP-XXXXXXXX</span>.
            </div>
...
function RsiStep({ finalize }: { finalize: (rsiHandle: string, verificationCode?: string, skipVerification?: boolean) =>Promise<void> }) {
    ...
    if (phase === 'input') {
        return (
            <Shell icon="fa-id-badge" title="RSI-Handle verknüpfen" subtitle="Nenne uns deinen Star Citizen (RSI)-Handle, damit die Org dich erkennt.">
                ...
                <PrimaryBtn onClick={() => setPhase('verify')} disabled={!handle.trim()}>Nächste<i className="fa-solid fa-arrow-right ml-1" /></PrimaryBtn>
            </Shell>
        );
    }

    return (
        <Shell icon="fa-id-badge" title="Handle verifizieren" subtitle={`Confirm you own "${handle.trim()}" by adding this code to your RSI bio.`}>
            <ol className="space-y-3 mb-5 text-sm text-slate-300">
                <li><span className="text-slate-500 mr-2 font-mono">01</span>Öffne Dein RSI-Profil →<span className="text-slate-400">Profil bearbeiten → Kurzbiografie</span>.</li>
                <li><span className="text-slate-500 mr-2 font-mono">02</span>Füge diesen Code an einer beliebigen Stelle in deiner Biografie ein und speichere:<div className="mt-2 flex items-center gap-2">
                        <code className="px-3 py-2 bg-slate-900 border border-sky-500/30 rounded text-sky-300 font-mono">{code}</code>
                        <button onClick={() => navigator.clipboard?.writeText(code)} className="text-slate-400 hover:text-white text-xs"><i className="fa-solid fa-copy mr-1" />Kopie</button>
                    </div>
                </li>
                <li><span className="text-slate-500 mr-2 font-mono">03</span>Kommen du zurück und überprüfen du es.</li>
            </ol>
            {error && <p className="text-rose-400 text-sm mb-3">{error}</p>}
            <div className="flex flex-wrap items-center gap-3">
                ...
                <button onClick={() => doFinalize(true)} disabled={busy}
                    className="px-4 py-3 text-slate-400 hover:text-white text-sm font-medium">Später überprüfen – ich bin offline</button>
            </div>
...
function ImportStep({ onContinue, selfDiscordId }: { onContinue: () => void; selfDiscordId?: string }) {
    ...
    return (
        <Shell icon="fa-file-import" title="Import von MyRSI.org (optional)"
            subtitle="Migration von der gehosteten Plattform? Importiere deinen Org-Export jetzt oder später unter Admin → Import.">
            {!running && !done && (
                <div className="rounded-lg border border-dashed border-white/15 bg-slate-900/40 p-5 mb-4 text-center">
                    ...
                    <p className="text-[11px] text-slate-600 mt-3">Exportieren von<span className="text-slate-400">manage.myrsi.org → Manage org → Export</span>.</p>
                </div>
            )}
            ...
            {manifest && !running && !done && (
                <div className="rounded-lg border border-white/5 bg-slate-900/50 px-4 py-3 mb-4 text-xs text-slate-300">
                    <div>Quellorganisation:<span className="text-white">{manifest.org || '—'}</span></div>
            ...
            {manifest && users.length > 0 && !running && !done && (
                <div className="rounded-lg border border-sky-500/20 bg-sky-500/5 px-4 py-3 mb-4">
                    <label className="block text-xs font-semibold text-sky-300 mb-1">Wer davon bist du?</label>
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                {done && <PrimaryBtn onClick={onContinue}>Weitermachen<i className="fa-solid fa-arrow-right ml-1" /></PrimaryBtn>}
                {!running && !done && !error && <button onClick={onContinue} className="px-4 py-3 text-slate-400 hover:text-white text-sm font-medium">Überspringen – das mache ich später</button>}
                {error && <button onClick={() => window.location.assign('/')} className="px-4 py-3 text-slate-400 hover:text-white text-sm font-medium"><i className="fa-solid fa-rotate-left mr-2" />Neu laden</button>}
            </div>
        </Shell>
    );
}
...
function CongratsStep({ name }: { name?: string }) {
    ...
    return (
        <Shell icon="fa-circle-check" title={name ? `You're all set, ${name}!` : "You're all set!"}
            subtitle="Deine Organisation ist bereit. Willkommen an Bord — und danke fürs Self-Hosting von Open MyRSI.org.">
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 mb-8 text-sm text-slate-300">
                ...
                configurable from the <span className="text-emerald-300 font-medium">Admin Konsole</span> inside your dashboard.
            </div>
