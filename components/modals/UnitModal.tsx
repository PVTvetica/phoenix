    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && unit) {
                ...
                addToast("Einheit aktualisiert", <i className="fa-solid fa-check"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: "Einheitsdetails erfolgreich gespeichert." });
            } else {
                await addUnit(unitData);
                addToast("Einheit erstellt", <i className="fa-solid fa-check"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: "Neue Einheit wurde der Organisationsstruktur hinzugefügt." });
            }
            onClose();
        } catch (err) {
            console.error("Failed to save unit:", err);
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Einheit konnte nicht gespeichert werden. Bitte versuche es erneut." });
            setIsLoading(false);
        }
                ...
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            {!canManageStructure && <p className="text-[9px] text-slate-500 mt-1 italic">Wenden du sich an den Administrator, um die Hierarchie zu ändern.</p>}
                        </div>
                    </div>

                    <div>
                        <label className={labelClass}>Einheitsführer</label>
                    ...
                    <div className="border-t border-slate-700/50 pt-6">
                        <h4 className="text-sm font-bold text-white mb-4">Branding und Identität</h4>
                        <div>
                        ...
                        <div className="mt-4">
                            <label className={labelClass}>Leitbild/Beschreibung</label>
                    ...
                    <div className="border-t border-slate-700/50 pt-6">
                        <h4 className="text-sm font-bold text-white mb-4">Funkkonfiguration</h4>
                        <label className="flex items-center gap-3 cursor-pointer group">
                            <input
                                ...
                                disabled={isLoading}
                            />
                            <span className="text-sm text-slate-300 group-hover:text-white transition-colors">Erstelle eine Funkfrequenz für dieses Gerät</span>
                        </label>
                        ...
                        {hasRadioChannel && radioChannels.length > 0 && (
                            <div className="mt-4">
                                <label className={labelClass}>Vorhandenen Kanal verknüpfen (optional)</label>
                                <select
                    ...
                    <div className="border-t border-slate-700/50 pt-6">
                        <h4 className="text-sm font-bold text-white mb-4">Sichtweite</h4>
                        <label className="flex items-start gap-3 cursor-pointer group">
                            ...
                            <div>
                                <span className="text-sm text-slate-300 group-hover:text-white transition-colors">
                                    <i className="fa-solid fa-lock text-amber-400/80 mr-1.5 text-xs"></i>Beschränken du diese Einheit nur auf Mitglieder</span>
                    ...
                    <div>
                        <label className={labelClass}>Sortierreihenfolge (Vorrang)</label>
                        <input
                            type="number"
                            value={sortOrder}
                            ...
                            placeholder="0"
                        />
                        {!canManageStructure && <p className="text-[9px] text-slate-500 mt-1 italic">Wenden du sich an den Administrator, um die Priorität zu ändern.</p>}
                    </div>
                </div>

                {/* Footer */}
                <div className="flex justify-end items-center p-6 bg-slate-900/50 border-t border-white/5 rounded-b-2xl shrink-0 gap-3">
                    <button type="button" onClick={onClose} className="px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors" disabled={isLoading}>Abbrechen</button>
