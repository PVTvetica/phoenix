...
const ServiceTypeModal: React.FC<ServiceTypeModalProps> = ({ isOpen, onClose, config }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: err?.message || "An error occurred while saving the service type. Please try again." });
            setIsLoading(false);
        }
    ...
    return (
        <WindowFrame
            ...
            subtitle="Missionskonfiguration"
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                <div className="p-6 space-y-5">
                    <div>
                        <label className={labelClass}>Gib den Namen ein</label>
                        <input
                            type="text"
                            value={name}
                            ...
                            disabled={isLoading}
                        />
                        <p className="text-[9px] text-slate-500 mt-1">Dies wird in Dropdown-Listen für Clients und Mitglieder angezeigt.</p>
                    </div>

                    <div>
                        <label className={labelClass}>Symbol und Farbe</label>
                    ...
                    <div>
                        <label className={labelClass}>Discord Kanal-ID<span className="text-slate-600 font-normal normal-case ml-2">(Optional)</span>
                        </label>
                        <input
                            type="text"
                    ...
                    <div className="flex items-center space-x-3 pt-2">
                        <label className="flex items-center space-x-2 cursor-pointer group">
                            <input
                                ...
                                disabled={isLoading}
                            />
                            <span className="text-sm text-slate-300 font-bold group-hover:text-white">Aktiv</span>
                        </label>
