...
const ManageSpecializationsModal: React.FC<ManageSpecializationsModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            addToast("Fehler", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Aktualisieren der Spezialisierungen ist ein Fehler aufgetreten. Bitte versuche es erneut." });
        } finally {
            setIsLoading(false);
        }
    };
        ...
        >
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1">
                    <div className="p-3 bg-slate-900/50 border border-slate-700/50 rounded-lg text-xs text-slate-400">Wähle bis zu<strong>four</strong> specializations that best represent your primary skills.
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                        {specializationTags.length > 0 ? (
                            ...
                            <p className="text-slate-500 text-center py-4 text-xs">Keine Spezialisierungen konfiguriert.</p>
                        )}
                    </div>
                </div>
