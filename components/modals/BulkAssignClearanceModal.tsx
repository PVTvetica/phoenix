...
const BulkAssignClearanceModal: React.FC<BulkAssignClearanceModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            addToast("Aktualisierung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50',
    ...
    return (
        <WindowFrame
            isOpen={isOpen}
            onClose={onClose}
            title="Massenzuweisungsfreigabe"
            subtitle="Freigabestufe und Marker auf mehrere Mitglieder anwenden"
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-5">
                    ...
                    <div>
                        ...
                        <div id="bulk-clearance-list" className="bg-slate-950/40 border border-slate-800 rounded-lg max-h-[300px] overflow-y-auto custom-scrollbar">
                            {visibleMembers.length === 0 ? (
                                <p className="p-6 text-center text-xs text-slate-500 italic">Keine Mitglieder entsprechen den aktuellen Filtern.</p>
                            ) : (
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2 flex-wrap gap-3">
                            <label className={labelClass} style={{ marginBottom: 0 }}>Begrenzungsmarkierungen</label>
                        ...
                        {limitingMarkers.length === 0 ? (
                            <p className="text-[11px] text-slate-600 italic">Keine begrenzenden Marker definiert.</p>
                        ) : (
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {limitingMarkers.map(m => {
                                    ...
                                    return (
                                        <button key={m.id} type="button" onClick={() => toggleMarker(m.id)}
                                            ...
                                            {m.syncRestricted && (
                                                <span className="text-[8px] bg-red-500/10 text-red-300 border border-red-500/30 px-1.5 py-0.5 rounded-sm uppercase tracking-wider font-bold shrink-0">Keine Synchronisierung</span>
                                            )}
                                        </button>
                                    );
                                })}
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex items-center justify-between gap-3 rounded-b-xl">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        <button onClick={onClose} disabled={submitting}
                            className="px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-slate-400 hover:text-white transition-colors">Abbrechen</button>
                        <button onClick={handleSubmit} disabled={!canSubmit}
                            ...
                            {submitting
                                ? <><i className="fa-solid fa-spinner animate-spin"></i>Bewerben</>
