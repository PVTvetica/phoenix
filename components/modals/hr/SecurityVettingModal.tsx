            ...
            <div className="flex flex-col md:flex-row h-[75vh]">
                ...
                <div className="w-56 bg-slate-900/50 border-r border-slate-700/50 flex flex-col shrink-0">
                    <div className="p-3 space-y-1">
                        {[
                            { id: 'overview', label: 'Überblick', icon: 'fa-file-lines' },
                    ...
                    <div className="mt-auto p-4 border-t border-slate-800/50">
                        <div className="bg-slate-950/50 p-3 rounded-sm border border-slate-800 text-[10px] text-slate-400 space-y-2 uppercase font-bold tracking-wider">
                            <div className="flex justify-between items-center"><span>RSI Prüfen</span><StatusDot status={vettingData.checks.rsiProfile} /></div>
                            <div className="flex justify-between items-center"><span>Organisationsgeschichte</span><StatusDot status={vettingData.checks.orgHistory} /></div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-slate-900/30 relative">
                    <div className="absolute top-4 right-6 z-10">
                        <button onClick={handleSaveProgress} disabled={isLoading || vettingLoading} className="text-[10px] bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-sm border border-slate-700 font-bold uppercase transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                            {(isLoading || vettingLoading) ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-floppy-disk mr-2"></i>Fortschritt speichern</>}
                        </button>
                    </div>
                    ...
                    {activeTab === 'overview' && (
                        <div className="max-w-3xl space-y-6">
                            <div className="bg-slate-950/30 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4">Daten anfordern</h3>
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div><span className="text-slate-500 block text-[10px] uppercase font-bold">Quelle</span><span className="text-white">{formatReferralSource(applicant.referralSource)}</span></div>
                                    <div><span className="text-slate-500 block text-[10px] uppercase font-bold">Abgelegt</span><span className="text-white font-mono">{fmt.date(applicant.createdAt)}</span></div>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {linkedMember && (
                                    <div className="bg-slate-950/30 border border-slate-700/50 rounded-xl p-4">
                                        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Aktuell</h3>
                                ...
                                {requestedLevel && (
                                    <div className="bg-slate-950/30 border border-slate-700/50 rounded-xl p-4">
                                        <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Angefordert</h3>
                        ...
                        <div className="max-w-4xl space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {[
                                    ...
                                    { key: 'orgHistory', label: 'Organisationsgeschichte' },
                            ...
                            {/* Intel & Warrants */}
                            <div className="bg-slate-950/30 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4">Intel Datenbank</h3>
                                {dossier ? (
                                    <div className="space-y-3">
                                        {dossier.warrants.some(w => w.status === 'Active') && (
                                            <div className="bg-red-500/10 border border-red-500/30 text-red-400 px-4 py-2 rounded-sm text-xs font-bold uppercase tracking-wide flex items-center gap-2">
                                                <i className="fa-solid fa-triangle-exclamation"></i>Aktiver Hinweis gefunden</div>
                                        )}
                                        ...
                                        ) : <p className="text-xs text-slate-500 italic">Keine Berichte gefunden.</p>}
                                    </div>
                                ) : <p className="text-xs text-slate-500 animate-pulse">Datenbank wird gescannt…</p>}
                            </div>
                        </div>
                    )}
                        ...
                        <div className="max-w-3xl space-y-4">
                            <div className="flex justify-between items-center">
                                ...
                                <button onClick={() => openScheduleInterviewModal(applicant)} className="bg-emerald-500/10 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500/20 px-3 py-1.5 rounded-sm text-[10px] font-bold uppercase transition-colors">
                                    <i className="fa-solid fa-plus mr-1"></i>Zeitplan</button>
                            </div>
                                ...
                                <div key={int.id} className="bg-slate-950/30 border border-slate-700/50 rounded-lg p-3 flex justify-between items-center">
                                    ...
                                    <div className="flex gap-2">
                                        {int.status !== 'Completed' && (
                                            <button onClick={() => openEditInterviewModal(int)} className="text-[10px] bg-amber-600/10 hover:bg-amber-600 text-amber-400 hover:text-white px-3 py-1.5 rounded-sm border border-amber-600/30 uppercase font-bold transition-colors">Bearbeiten</button>
                                        )}
                            ...
                            {applicant.interviews.length === 0 && <p className="text-xs text-slate-500 italic border-2 border-dashed border-slate-800 rounded-sm p-4 text-center">Keine Vorstellungsgespräche geplant.</p>}

                            <div className="mt-6 pt-6 border-t border-slate-800/50">
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Interviewfindung</h3>
                    ...
                    {activeTab === 'adjudication' && (
                        <div className="max-w-3xl space-y-6">
                            <div className="bg-slate-950/30 border border-slate-700/50 rounded-xl p-6">
                                <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4">Endgültige Entscheidung</h3>
                                <div className="flex gap-3 mb-6">
                                    <button onClick={() => setFinalStatus('Hired')} className={`flex-1 py-4 rounded-lg border-2 flex flex-col items-center justify-center gap-2 transition-all ${finalStatus === 'Hired' ? 'bg-green-500/10 border-green-500 text-green-400 shadow-[0_0_20px_rgba(34,197,94,0.1)]' : 'bg-slate-900 border-slate-800 text-slate-500 hover:border-slate-700'}`}>
                                        ...
                                        <span className="text-xs font-black uppercase">Genehmigen</span>
                                    </button>
                                    <button onClick={() => setFinalStatus('Rejected')} className={`flex-1 py-4 rounded-lg border-2 flex flex-col items-center justify-center gap-2 transition-all ${finalStatus === 'Rejected' ? 'bg-red-500/10 border-red-500 text-red-400 shadow-[0_0_20px_rgba(220,38,38,0.1)]' : 'bg-slate-900 border-slate-800 text-slate-500 hover:border-slate-700'}`}>
                                        ...
                                        <span className="text-xs font-black uppercase">Ablehnen</span>
                                    </button>
                                </div>
                                ...
                                {(finalStatus === 'Hired' || finalStatus === 'Revoked') && (
                                    <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700 mb-6 animate-fade-in">
                                        <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2">Legen du die Freigabestufe fest</label>
                                ...
                                <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2">Entscheidungsnotizen</label>
                                <textarea
                                    value={finalNotes}
