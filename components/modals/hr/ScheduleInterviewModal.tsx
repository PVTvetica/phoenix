        ...
        try {
            if (isEditMode) {
                if (!templateId || !interviewerId || !scheduledAt) {
                    addToast("Validierungsfehler", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: "Alle Felder sind Pflicht." });
                    setIsLoading(false);
                    return;
                }
                ...
                await refreshHR();
                addToast("Interview aktualisiert", <i className="fa-solid fa-check"></i>, "bg-green-500/10 text-green-400 border-green-500/50", { description: "Interviewdetails erfolgreich gespeichert." });
                onClose();
                return;
            }
            ...
            if (!applicant && mode === 'adhoc') {
                if (!adHocHandle.trim()) {
                    addToast("Validierungsfehler", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: "RSI-Handle ist für Ad-hoc-Interviews Pflicht." });
                    setIsLoading(false);
                    return;
                }
            ...
            addToast("Fehler", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Ein Fehler ist aufgetreten. Bitte versuche es erneut." });
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar max-h-[70vh]">
                    ...
                    {!applicant && !isEditMode && mode === 'existing' && (
                        <div>
                            <label className={labelClass}>Betreff / Kandidat</label>
                            <select
                    ...
                    {!applicant && !isEditMode && mode === 'adhoc' && (
                        <div className="space-y-4 bg-emerald-500/5 p-4 rounded-lg border border-emerald-500/30">
                            ...
                            <div>
                                <label className={labelClass}>Betreffname (optional)</label>
                                <input
                                    type="text"
                                    value={adHocName}
                            ...
                            <div>
                                <label className={labelClass}>Kontext/Dateityp</label>
                                <input
                                    type="text"
                                    value={adHocContext}
                    ...
                    <div>
                        <label className={labelClass}>Protokollvorlage</label>
                        <select
                            value={templateId}
                    ...
                    <div>
                        <label className={labelClass}>Leitender Interviewer</label>
                        <select
                            value={interviewerId}
                    ...
                    {/* Panel Members */}
                    <div>
                        <label className={labelClass}>Panel-Mitglieder<span className="text-slate-600">(Optional)</span></label>
                        ...
                        <p className="text-[10px] text-slate-500 mt-1.5">Panel-Mitglieder können dieses Interview ansehen und daran teilnehmen.</p>
                    </div>

                    <div>
                        <label className={labelClass}>Datum und Uhrzeit</label>
                        <input
                            type="datetime-local"
                            value={scheduledAt}
