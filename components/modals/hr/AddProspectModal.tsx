...
const AddProspectModal: React.FC<AddProspectModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        if (!rsiHandle.trim() || !applicantName.trim()) {
            addToast("Validierungsfehler", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: "RSI-Handle und Name sind Pflicht." });
            return;
        }
        ...
        try {
            ...
            onClose();
            addToast("Akte geöffnet", <i className="fa-solid fa-folder-plus"></i>, "bg-emerald-500/10 text-emerald-400 border-emerald-500/50", { description: "Neue Akte erfolgreich erstellt." });
            refreshHR();
        } catch (err) {
            console.error("Failed to add prospect:", err);
            addToast("Fehler", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Ein Fehler ist aufgetreten. Prüfe, ob dieser RSI-Handle bereits im System ist." });
            setIsLoading(false);
        }
    ...
    return (
        <WindowFrame
            ...
            subtitle="Rekrutierung & Überprüfung"
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div>
                        <label className={labelClass}>Betreffname<span className="text-red-400">*</span></label>
                        <input
                            type="text"
                            value={applicantName}
                    ...
                    <div>
                        <label className={labelClass}>Kontext/Verweisquelle</label>
                        <input
                            type="text"
                            value={referral}
                    ...
                    <div>
                        <label className={labelClass}>Erste Anmerkungen</label>
                        <textarea
                            value={notes}
