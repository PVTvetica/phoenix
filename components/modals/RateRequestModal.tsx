...
const RateRequestModal: React.FC<RateRequestModalProps> = ({ isOpen, onClose, request }) => {
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            addToast("Fehler", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Absenden deiner Bewertung ist ein Fehler aufgetreten. Bitte versuche es erneut." });
        } finally {
            setIsLoading(false);
        }
        ...
        >
            <div className="flex flex-col h-full">
                <div className="p-8 text-center flex-1">
                    ...
                    <h2 className="text-xl font-bold text-white mb-2">Wie war Deine Erfahrung?</h2>
                    ...
                    <div className="text-left">
                        <label className={labelClass}>Zusätzliches Feedback</label>
                        <textarea
                            value={feedback}
                            ...
                            disabled={isLoading}
                        />
                        <p className="mt-3 text-[11px] text-slate-500 leading-relaxed">Dein Feedback hilft<strong className="text-slate-400">{brandingConfig.name}</strong> improve their service. If they have a public landing page enabled, admins may select anonymous excerpts of feedback to display &mdash; your name and identity will never be shown.
                        </p>
                    </div>
                </div>

                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    <button type="button" onClick={onClose} className="px-4 py-2 text-xs font-bold uppercase text-slate-400 hover:text-white transition-colors" disabled={isLoading}>Überspringen</button>
                    <button
                        type="button"
