...
const RequestClearanceModal: React.FC<RequestClearanceModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            onClose();
            addToast("Freigabeantrag eingereicht", <i className="fa-solid fa-lock"></i>, "bg-sky-500/10 text-sky-400 border-sky-500/50", { description: "Deine Freigabeanfrage wurde zur Prüfung eingereicht." });
            refreshHR();

        } catch (err: any) {
            console.error("Failed to submit request:", err);
            addToast("Fehler", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: err.message || "An error occurred while submitting your request." });
            setIsLoading(false);
        }
    ...
    return (
        <WindowFrame
            ...
            subtitle="Zugriffskontrollprotokoll"
            ...
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div className="space-y-4 bg-slate-950/50 p-4 rounded-xl border border-slate-800">
                        <div>
                            <label className={labelClass}>Zielfreigabeniveau (optional)</label>
                            <select
                                value={targetLevelId}
                            ...
                            >
                                ...
                                {availableLevels.length > 0 ? availableLevels.map(lvl => (
                                    ...
                                    <option value="" disabled>Keine höheren Level verfügbar</option>
                                )}
                            </select>
                        </div>

                        <div>
                            <label className={labelClass}>Fachmarkierungen (optional)</label>
                            <div className="space-y-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                                {availableMarkers.length > 0 ? availableMarkers.map(m => (
                                    ...
                                    <p className="text-xs text-slate-500 italic">Keine zusätzlichen Marker verfügbar.</p>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <label className={labelClass}>Begründung / Wissenswertes</label>
                        <textarea
                            value={justification}
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    ...
                    >
                        {isLoading ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-paper-plane"></i>Bewerbung einreichen</>}
                    </button>
                </div>
            </form>
        </WindowFrame>
