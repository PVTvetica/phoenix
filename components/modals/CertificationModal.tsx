...
const CertificationModal: React.FC<CertificationModalProps> = ({ isOpen, onClose, certification }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Speichern der Zertifizierung ist ein Fehler aufgetreten. Bitte versuche es erneut." });
            setIsLoading(false);
        }
    ...
    return (
        <WindowFrame
            ...
            subtitle="Zertifikatsverwaltung"
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                ...
                <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    <div>
                        <label htmlFor="certName" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Zertifizierungsname</label>
                        <input
                            type="text"
                            id="certName"
                            value={name}
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Symbol (optional)</label>
                        <AwardIconInput
                            icon={icon}
                    ...
                    <div>
                        <label htmlFor="certDescription" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Beschreibung (optional)</label>
                        <textarea
                            id="certDescription"
                            value={description}
