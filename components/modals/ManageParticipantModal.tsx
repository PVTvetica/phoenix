...
const ManageParticipantModal: React.FC<ManageParticipantModalProps> = ({ isOpen, onClose, operationId, participant, alliedShips }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            addToast("Aktualisierung fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Aktualisieren der Teilnehmerdetails ist ein Fehler aufgetreten. Bitte versuche es erneut." });
        } finally {
            setIsLoading(false);
        }
    };
        ...
        >
            <div className="flex flex-col h-full">
                <div className="p-6 space-y-5">
                    ...
                    <div>
                        <label className={labelClass}>Zugewiesene Rolle</label>
                        <input
                            type="text"
                            value={role}
                    ...
                    <div>
                        ...
                        {participantShips.length === 0 && (
                            <p className="text-[10px] text-slate-600 mt-1 italic">Keine Schiffe im Hangar – Schiffe im Flottenmanager hinzufügen</p>
                        )}
                    </div>
                    <div>
                        <label className={labelClass}>Anwesenheitsstatus</label>
                        <select
                            value={status}
                        ...
                        >
                            ...
                            <option value="No Show">Keine Show</option>
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex justify-end gap-3 rounded-b-xl">
                    <button onClick={onClose} className="px-4 py-2 text-xs font-bold uppercase text-slate-400 hover:text-white transition-colors" disabled={isLoading}>Abbrechen</button>
                    <button
                        onClick={handleSave}
