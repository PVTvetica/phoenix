    ...
    const handleSubmit = useCallback(async () => {
        ...
        for (const step of STEPS) {
            if (validation[step.key]) {
                addToast("Validierungsfehler", <i className="fa-solid fa-triangle-exclamation"></i>, 'bg-amber-500/10 text-amber-400 border-amber-500/50', { description: validation[step.key]! });
                setStepKey(step.key);
                return;
            }
        }
        ...
        >
            <div className="flex flex-col h-full">
                ...
                <div className="p-4 border-t border-white/5 bg-slate-900/50 flex items-center justify-between gap-3 rounded-b-xl">
                    <button onClick={onClose} className="px-3 py-2 text-xs font-bold uppercase tracking-widest text-slate-400 hover:text-white transition-colors" disabled={isLoading}>Abbrechen</button>
                    <div className="flex items-center gap-2">
                        {stepIndex > 0 && (
                            <button onClick={goBack} disabled={isLoading} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 text-[11px] font-bold uppercase tracking-wider transition-colors disabled:opacity-50">
                                <i className="fa-solid fa-arrow-left"></i>Zurück</button>
                        )}
                        {!isLastStep ? (
                            <button onClick={goNext} disabled={!canAdvance || isLoading}
                                className="flex items-center gap-2 px-5 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 border border-purple-500/40 text-white text-[11px] font-bold uppercase tracking-wider transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Nächste<i className="fa-solid fa-arrow-right"></i>
                            </button>
                        ) : (
...
const BasicsStep: React.FC<{
    ...
    <div className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                <label className={labelClass}>Maximale Teilnehmerzahl (optional)</label>
...
const LocationsStep: React.FC<{
    ...
    return (
        <div className="space-y-5">
            ...
            {/* Primary */}
            <div>
                <label className={labelClass}>Primärer Standort</label>
                <LocationInput
            ...
            <div>
                <div className="flex items-center justify-between mb-2">
                    <label className={labelClass} style={{ marginBottom: 0 }}>Zusätzliche Standorte</label>
                    <button type="button"
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-3">
                    ...
                    <div key={phase._key} className="rounded-xl border border-slate-700/40 bg-slate-900/40 overflow-hidden">
                        <div className="px-4 py-2.5 bg-slate-800/50 border-b border-slate-700/40 flex items-center gap-2">
                            ...
                            <select value={phase.phaseType || 'sequential'}
                                ...
                                <option value="contingency">Kontingenz</option>
                            </select>
                        ...
                        <div className="p-3 space-y-3">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-1.5">
                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Meilensteine</p>
                                    <button type="button" onClick={() => dispatch({ type: 'add_milestone', phaseKey: phase._key })}
                                        ...
                                        disabled={disabled}>
                                        <i className="fa-solid fa-plus mr-1"></i>Meilenstein</button>
                                </div>
                                {(phase.milestones || []).length === 0 ? (
                                    <p className="text-[10px] text-slate-600 italic">Keiner.</p>
                                ) : (
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-1.5">
                                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Aufgaben</p>
                                    <button type="button" onClick={() => dispatch({ type: 'add_task', phaseKey: phase._key })}
                                        className="text-[10px] text-purple-300 hover:text-purple-200 uppercase tracking-wider font-bold"
                                        disabled={disabled}>
                                        <i className="fa-solid fa-plus mr-1"></i>Aufgabe</button>
                                </div>
                ...
                <button type="button" onClick={() => dispatch({ type: 'add_phase' })}
                    ...
                    disabled={disabled}>
                    <i className="fa-solid fa-plus"></i>Phase hinzufügen</button>
            </div>
        </div>
    );
};
    ...
    <div className="space-y-5">
        <div className="bg-slate-900/40 p-4 rounded-xl border border-red-500/15">
            <h4 className="text-xs font-black text-red-400 uppercase tracking-widest mb-3 flex items-center">
                <i className="fa-solid fa-lock mr-2"></i>Zugangskontrolle</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label className={labelClass}>Mindestfreigabestufe</label>
                ...
                <div>
                    <label className={labelClass}>Erforderliche Markierungen</label>
                    {limitingMarkers.length === 0 ? (
                        <p className="text-xs text-slate-500 italic">Keine Markierungen definiert.</p>
                    ) : (
                        <div className="space-y-1 max-h-32 overflow-y-auto custom-scrollbar pr-1 bg-slate-950/40 rounded-lg border border-slate-800 p-2">
                            {limitingMarkers.map(m => (
                                <label key={m.id} className="flex items-center justify-between gap-2 text-xs text-slate-300 cursor-pointer hover:text-white p-1 rounded-sm hover:bg-slate-800/50">
                                    ...
                                    {m.syncRestricted && (
                                        <span className="text-[8px] bg-amber-500/10 text-amber-400 border border-amber-500/30 px-1.5 py-0.5 rounded-sm uppercase tracking-wider font-bold" title="Sync Restricted — blocks Joint Operation sharing">Synchronisierung eingeschränkt</span>
                                    )}
                                </label>
                            ))}
                        </div>
            ...
            {blockingErrors.length > 0 && (
                <div className="rounded-xl border border-red-500/30 bg-red-950/20 p-4 space-y-2">
                    <p className="text-[10px] font-black uppercase tracking-widest text-red-300 flex items-center gap-2">
                        <i className="fa-solid fa-triangle-exclamation"></i>Vor dem Erstellen beheben</p>
