...
const RoleModal: React.FC<RoleModalProps> = ({ isOpen, onClose, role }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Speichern der Rolle ist ein Fehler aufgetreten. Bitte versuche es erneut." });
            setIsLoading(false);
        }
    ...
    return (
        <WindowFrame
            ...
            subtitle="Berechtigungsstufen-Konfiguration"
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                ...
                <div className="p-6 space-y-6">
                    <div>
                        <label htmlFor="roleName" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Name der Systemrolle</label>
                        <input
                            type="text"
                            id="roleName"
                            value={name}
                        ...
                        />
                        {isDefaultRole && <p className="text-[10px] text-slate-500 mt-1 font-bold flex items-center"><i className="fa-solid fa-lock mr-1"></i>Standardsystemrollen können nicht umbenannt werden.</p>}
                    </div>
                    <div>
                        <label htmlFor="roleDescription" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Beschreibung</label>
                        <textarea
                            id="roleDescription"
                            value={description}
