        ...
        >
            <div className="flex flex-col h-full bg-slate-900/50">
                <div className="p-6 max-h-[60vh] overflow-y-auto custom-scrollbar flex-1">
                    {isLoading ? (
                        <div className="text-center text-slate-400 py-10 flex flex-col items-center">
                            ...
                            <span className="text-xs uppercase tracking-widest">Datensatz wird geladen…</span>
                        </div>
                    ) : history.length === 0 ? (
                        <div className="text-center text-slate-500 py-10 italic">Für diesen Benutzer wurde kein Reputationsverlauf gefunden.</div>
                    ) : (
                ...
                <div className="flex justify-end p-4 bg-slate-900/80 border-t border-white/5 rounded-b-xl backdrop-blur-sm">
                    <button type="button" onClick={onClose} className="px-6 py-2 text-xs font-bold uppercase text-slate-400 hover:text-white rounded-lg hover:bg-slate-800 transition-colors">Datensatz schließen</button>
                </div>
            </div>
        </WindowFrame>
    );
