...
const AUDIENCE_CONFIG: { value: AudienceOption; label: string; description: string; icon: string }[] = [
    { value: 'Client', label: 'Client Dashboard', description: 'Alle angemeldeten Clients können diese Mitteilung sehen.', icon: 'fa-solid fa-headset' },
    { value: 'Member', label: 'Members Dashboard', description: 'Alle angemeldeten Mitglieder können diese Mitteilung sehen.', icon: 'fa-solid fa-users' },
    { value: 'Admin', label: 'Admins Only', description: 'Alle angemeldeten Admins sehen diese Mitteilung im Dashboard und in anderen Ansichten.', icon: 'fa-solid fa-shield-halved' },
    { value: 'Login Screen', label: 'Login Screen', description: 'Auf dem Login-Screen für alle Besucher sichtbar.', icon: 'fa-solid fa-right-to-bracket' },
];
...
const NoticeModal: React.FC<NoticeModalProps> = ({ isOpen, onClose, notice }) => {
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        if (!title.trim() || !body.trim() || audience.length === 0) {
            addToast("Validierungsfehler", <i className="fa-solid fa-triangle-exclamation"></i>, "bg-amber-500/10 text-amber-400 border-amber-500/50", { description: "Titel, Text und mindestens eine Zielgruppe sind Pflicht." });
            return;
        }
        ...
        try {
            ...
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Beim Speichern der Mitteilung ist ein Fehler aufgetreten. Bitte versuche es erneut." });
            setIsLoading(false);
        }
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div>
                        <label htmlFor="noticeTitle" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Titel</label>
                    ...
                    <div>
                        <label htmlFor="noticeBody" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Körper</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label htmlFor="noticeType" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Hinweistyp</label>
                        ...
                        <div>
                            <label htmlFor="expiryDate" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Ablaufdatum (optional)</label>
                            <input
                                type="datetime-local"
                                id="expiryDate"
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Publikum</label>
