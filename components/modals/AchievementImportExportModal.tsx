...
const AchievementImportExportModal: React.FC<AchievementImportExportModalProps> = ({ isOpen, onClose, kind }) => {
    ...
    const loadPreview = useCallback(async (parsedItems: any[]) => {
        ...
        try {
            ...
            addToast("Vorschau fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, 'bg-red-500/10 text-red-400 border-red-500/50', { description: err?.message || 'Could not preview the import.' });
            setPreview(null);
        } finally {
            setLoadingPreview(false);
        }
                ...
                {tab === 'export' && (
                    <div className="p-5 space-y-4">
                        ...
                        <div className="flex justify-end">
                            ...
                            >
                                <i className="fa-solid fa-download"></i>Laden du JSON herunter</button>
                        </div>
                    </div>
                )}
                ...
                {tab === 'import' && (
                    <div className="p-5 space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <i className="fa-solid fa-folder-open"></i>Wähle JSON-Datei</button>
                        ...
                        {loadingPreview && (
                            <div className="bg-slate-900/40 rounded-lg p-4 border border-slate-700/30 flex items-center gap-2 text-xs text-slate-400">
                                <i className="fa-solid fa-circle-notch animate-spin"></i>Rechenunterschiede …</div>
                        )}
                        ...
                        <div className="flex justify-end gap-3">
                            {importing ? (
                                ...
                                >
                                    <i className="fa-solid fa-ban"></i>Abbrechen</button>
                            ) : (
                                <button
                                    ...
                                    className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-purple-500/10 text-purple-300 border border-purple-500/30 text-[10px] font-bold uppercase tracking-wider hover:bg-purple-500/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    <i className="fa-solid fa-play"></i>Starten du den Import</button>
                            )}
                        </div>
                    </div>
                )}
