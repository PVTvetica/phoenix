...
const RadioChannelModal: React.FC<RadioChannelModalProps> = ({ isOpen, onClose, channel, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(error);
            addToast("Speichern fehlgeschlagen", <i className="fa-solid fa-xmark"></i>, "bg-red-500/10 text-red-400 border-red-500/50", { description: "Kanal konnte nicht gespeichert werden. Bitte versuche es erneut." });
        } finally {
            setIsLoading(false);
        }
    };
        ...
        >
            <form onSubmit={handleSubmit} className="flex flex-col h-full">
                <div className="p-6 space-y-5">
                    <div>
                        <label className={labelClass}>Kanal-ID</label>
                        <input
                            type="text"
                            value={id}
                            ...
                            required
                        />
                        {!isEditing && <p className="text-[10px] text-slate-600 mt-1 italic">Eindeutiger Bezeichner (Slug), Kleinschreibung, keine Leerzeichen.</p>}
                    </div>
                    ...
                    <div>
                        <label className={labelClass}>Signalfarbe</label>
