...
const SyncUsersModal: React.FC<SyncUsersModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <WindowFrame
            ...
            subtitle="Identität & Rollenverbreitung"
            ...
            <div className="flex flex-col h-full bg-slate-900/50">
                <div className="p-6 space-y-4 flex-1 overflow-hidden flex flex-col">
                    <p className="text-slate-300 text-sm">
                        This process will iterate through all {usersToSync.length} users in the database, updating their <strong className="text-white">Anzeigename</strong>, <strong className="text-white">Avatar</strong>, and syncing their <strong className="text-white">Rang</strong> based on current Discord status.
                    </p>
                    ...
                    >
                        {logs.length === 0 ? (
                            <span className="text-slate-600 italic">Bereit zum Start. Klicke zum Starten auf „Synchronisierung starten“.</span>
                        ) : (
                            logs.map((log, i) => (
                ...
                <div className="flex justify-end p-4 bg-slate-900/80 border-t border-white/5 rounded-b-xl backdrop-blur-sm">
                    {!isRunning ? (
                        <>
                            ...
                            >Starten du die Synchronisierung</button>
                        </>
                    ) : (
                        <button className="px-6 py-2 bg-slate-800 text-slate-400 rounded-lg text-xs font-bold uppercase tracking-wider cursor-wait flex items-center gap-2" disabled>
                            <i className="fa-solid fa-spinner animate-spin"></i>Synchronisierung…</button>
                    )}
                </div>
            </div>
        </WindowFrame>
