        ...
        >
            <div className="flex flex-col h-full bg-slate-900/50">
                <div className="p-10 text-center border-b border-white/5 bg-slate-900/50">
                    ...
                    <div className="text-xs text-slate-500 uppercase tracking-widest font-bold">Gesamtbewertungen</div>
                </div>
                ...
                <div className="p-0 overflow-y-auto custom-scrollbar flex-1 max-h-[60vh]">
                    {isLoading ? (
                        <div className="text-center text-slate-400 p-12 flex flex-col items-center gap-3">
                            ...
                            <span className="text-xs uppercase tracking-widest">Datensätze abrufen…</span>
                        </div>
                    ) : history.length === 0 ? (
                        <div className="text-center text-slate-500 p-12 italic">Für dieses Mitglied wurden keine bewerteten Missionen gefunden.</div>
                    ) : (
                        <table className="w-full text-left border-collapse">
                            <thead className="sticky top-0 bg-slate-900 z-10 shadow-xs">
                                <tr className="border-b border-slate-800">
                                    <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest bg-slate-900">Datum</th>
                                    <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest bg-slate-900">Serviceanfrage</th>
                                    <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest bg-slate-900">Client</th>
                                    <th className="p-4 text-[10px] font-black uppercase text-slate-500 tracking-widest bg-slate-900 text-right">Bewertung</th>
                                </tr>
                            </thead>
                ...
                <div className="flex justify-end p-4 bg-slate-900/80 border-t border-white/5 rounded-b-xl backdrop-blur-sm">
                    <button type="button" onClick={onClose} className="px-6 py-2 text-xs font-bold uppercase text-slate-300 bg-slate-800 hover:bg-slate-700 hover:text-white rounded-lg transition-colors border border-slate-700">Datensatz schließen</button>
                </div>
            </div>
        </WindowFrame>
    );
