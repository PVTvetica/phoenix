                ...
                <div className="flex flex-col select-none">
                    <div className="bg-slate-900 p-3 flex justify-between items-center border-b border-slate-800">
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-600">Taktische Funkeinheit</span>
                        </div>
                    ...
                    <div className="p-6 flex flex-col items-center justify-center space-y-4">
                        ...
                        <div className="flex items-center gap-1.5 text-[9px] text-slate-700 uppercase font-bold tracking-wider">
                            <i className="fa-solid fa-lock text-[8px]"></i>Admin Einrichtung erforderlich</div>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="flex flex-col select-none">
                ...
                <div className="bg-slate-900 p-3 flex justify-between items-center border-b border-slate-800">
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="font-black text-[10px] uppercase tracking-[0.2em] text-slate-400">Taktische Funkeinheit</span>
                    </div>
                ...
                <div className="p-4 space-y-4">
                    ...
                    <div className={`relative rounded-lg p-4 border-2 transition-all duration-500 overflow-hidden min-h-[140px] flex flex-col justify-start ${isEnabled ? 'bg-sky-950/30 border-sky-500/30 shadow-[inset_0_0_30px_rgba(239,68,68,0.15)]' : 'bg-slate-900/50 border-slate-800 justify-center'}`}>
                            ...
                            currentChannel ? (
                                <div className="relative z-10 space-y-2 animate-fade-in flex flex-col h-full">
                                    <div className="flex justify-between items-start shrink-0">
                                        <div className="flex items-center space-x-2">
                                            <p className="text-[9px] text-sky-400/60 uppercase font-black tracking-widest">Aktive Komm</p>
                                ...
                                <div className="flex flex-col items-center justify-center py-4 space-y-2">
                                    ...
                                    {channels.length === 0 && <p className="text-[9px] text-slate-500 uppercase">Keine verfügbaren Kanäle</p>}
                                </div>
                            )
                        ) : (
                        ...
                        {isConnecting && isEnabled && (
                            <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px] flex flex-col items-center justify-center z-20">
                                ...
                                <p className="text-[10px] font-black text-sky-400 uppercase tracking-widest animate-pulse mt-2">Link wird synchronisiert…</p>
                            </div>
                        )}
                    </div>
                    ...
                    {isEnabled && channels.length === 0 && (
                        <div className="text-center py-4 bg-slate-900/30 rounded-sm border border-slate-800 border-dashed">
                            <p className="text-[10px] text-slate-500">Für Deine Rolle sind keine Frequenzen verfügbar.</p>
                        </div>
                    )}
                    ...
                    {isEnabled && isDevicePTTSupported && (
                        <div className="border border-amber-500/20 rounded-lg p-2.5 bg-amber-950/10 space-y-2">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1.5">
                                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">
                                        <i className="fa-solid fa-gamepad mr-1.5 text-[8px]"></i>Gerät PTT</span>
                                ...
                                <div className="flex items-center gap-2">
                                    {pairedDeviceName && !isBinding && (
                                        <button onClick={unpair} className="text-[8px] text-red-400/60 hover:text-red-400 uppercase tracking-wider transition-colors">Entfernen</button>
                                    )}
                                </div>
                            </div>
                            ...
                            >
                                <span className="font-semibold uppercase tracking-wider"><i className="fa-solid fa-circle-info mr-1 text-[8px]"></i>Wie es funktioniert</span>
                            ...
                            {showHIDInfo && (
                                <div className="text-[9px] text-slate-500 bg-slate-900/60 rounded-sm p-2 space-y-1.5 border border-slate-800/50 leading-relaxed">
                                    ...
                                    <p className="text-slate-500"><strong className="text-sky-400/70">Regler</strong> — Xbox, PlayStation, HOTAS, joystick. Works on all browsers. Best for dual-monitor setups where the dashboard stays visible.</p>
                                    <p className="text-slate-500"><strong className="text-sky-400/70">HID-Gerät</strong> — USB foot pedals, macro pads, dedicated PTT hardware. Chrome/Edge only. Can work while another app is focused.</p>
                                </div>
                            )}
                            ...
                            <button
                                ...
                                className="w-full flex items-center justify-between text-[9px] text-slate-500 hover:text-slate-400 transition-colors"
                            >
                                <span className="font-semibold uppercase tracking-wider"><i className="fa-solid fa-triangle-exclamation mr-1 text-[8px] text-amber-500/60"></i>Einschränkungen</span>
                            ...
                            {showHIDLimits && (
                                <div className="text-[9px] text-slate-500 bg-slate-900/60 rounded-sm p-2 space-y-1.5 border border-slate-800/50 leading-relaxed">
                                    ...
                                    <div className="flex flex-col gap-0.5 pl-1">
                                        ...
                                        <span className="text-green-400/70"><i className="fa-solid fa-check text-[7px] mr-1"></i>Funktioniert in allen gängigen Browsern</span>
                                        <span className="text-red-400/70"><i className="fa-solid fa-xmark text-[7px] mr-1"></i>Funktioniert nur, wenn die Dashboard-Registerkarte sichtbar ist</span>
                                    ...
                                    <p className="text-slate-400 font-semibold pt-1">HID-Gerät (WebHID API)</p>
                                    <div className="flex flex-col gap-0.5 pl-1">
                                        <span className="text-green-400/70"><i className="fa-solid fa-check text-[7px] mr-1"></i>Funktioniert, während eine andere App fokussiert ist</span>
                                        <span className="text-green-400/70"><i className="fa-solid fa-check text-[7px] mr-1"></i>Elgato Stream Deck, Elgato Pedal, maßgeschneiderte HID-Hardware</span>
                                        <span className="text-red-400/70"><i className="fa-solid fa-xmark text-[7px] mr-1"></i>Chrome & Nur Kante</span>
                                    ...
                                    <p className="text-slate-400 font-semibold pt-1">Empfohlenes Setup</p>
                                    <p className="text-slate-500 pl-1">Verwenden du a<strong className="text-slate-400">second monitor</strong> with the dashboard visible and bind a controller button. Or use an Elgato Stream Deck / Pedal via HID for true background PTT.</p>
                                </div>
                            )}
                            ...
                            {isBinding ? (
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="flex gap-1.5 pt-0.5">
                                        ...
                                        >Controller neu binden</button>
                                        <button
                                            onClick={bindHID}
                                            className="flex-1 py-1 text-[8px] text-sky-400/60 hover:text-sky-400 uppercase tracking-wider bg-slate-800/30 rounded-sm border border-slate-700/30 hover:border-sky-500/30 transition-all"
                                        >Binden du HID erneut</button>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex gap-1.5">
                                    ...
                                    >
                                        <i className="fa-solid fa-gamepad mr-1.5"></i>Regler</button>
                                    <button
                                        onClick={bindHID}
                                        className="flex-1 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-500 hover:text-sky-400 bg-slate-800/50 hover:bg-slate-800 rounded-sm border border-slate-700/50 hover:border-sky-500/30 transition-all"
                                    >
                                        <i className="fa-solid fa-usb mr-1.5"></i>HID-Gerät</button>
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        >
                            {isTransmitting ? (
                                <span className="flex items-center justify-center">
                                    <span className="w-2.5 h-2.5 rounded-full bg-white animate-ping mr-4"></span>LIVE-TX</span>
                            ) : (
                                'PUSH TO TALK'
                            )}
                        </button>
                        <p className="text-[9px] text-center text-slate-600 uppercase font-black mt-2 tracking-widest">Zum Senden gedrückt halten</p>
                    </div>
                </div>
            </div>
