            ...
            >
                ...
                {items.length > 1 && (
                    <div className="mt-5 flex items-center justify-between">
                        <button type="button" onClick={() => setIndex((i) => (i - 1 + items.length) % items.length)} className="px-3 py-1 text-xs text-slate-400 hover:text-white focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-sky-400 rounded-sm" aria-label="Previous testimonial"><i className="fa-solid fa-chevron-left mr-1" aria-hidden />Vorher</button>
                        <span className="text-[10px] text-slate-500 font-mono">{index + 1} / {items.length}</span>
                        <button type="button" onClick={() => setIndex((i) => (i + 1) % items.length)} className="px-3 py-1 text-xs text-slate-400 hover:text-white focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-sky-400 rounded-sm" aria-label="Next testimonial">Nächste<i className="fa-solid fa-chevron-right ml-1" aria-hidden /></button>
                    </div>
                )}
            </div>
        </CardPanel>
...
const OrgPublicPage: React.FC<{ onLoginClick: () => void }> = ({ onLoginClick }) => {
    ...
    return (
        <div className="relative min-h-dvh bg-slate-950 text-slate-200 font-sans overflow-x-hidden animate-fade-in">
            <a href="#cta-login" className="sr-only focus:not-sr-only fixed top-2 left-2 z-50 bg-slate-900 border border-slate-700 text-white text-xs px-3 py-1.5 rounded-sm">Weiter zur Anmeldung</a>
